...
D-064 当前 W2 候选的非敏感配置冻结为：Provider 标签 `openai-compatible`、API base `https://api.openai.com/v1`、STT `gpt-4o-mini-transcribe`、TTS `gpt-4o-mini-tts`、voice `marin`。API key 只能通过 Gateway 启动终端的隐藏输入注入该进程环境，不得粘贴到本文件、PowerShell 历史、浏览器、Git、日志或 evidence。正式 evidence 前先用同一配置执行一次最短 STT/TTS 探针，确认认证、模型/voice 可用，TTS 响应满足文档化的 raw 24 kHz mono signed PCM16 little-endian 合同，且 Gateway 输出为完整 canonical mono PCM16 WAV；同时记录非敏感的实际输入/输出采样率。探针失败时废弃该 attempt，不得静默换 alias、snapshot 或 voice。
...
当前 OpenAI Batch Speech Adapter 对 STT/TTS 请求显式发送 `Accept-Encoding: identity`，只接受无 `Content-Encoding` 或规范化后精确为 `identity` 的响应，并用 raw stream 按实际 identity bytes 限流；gzip、deflate、未知或损坏编码必须在读取 body 前以不可重试协议错误 fail closed。Adapter 向 `/audio/speech` 请求 `response_format=pcm`，只接受有界、非空、偶数字节的 raw 24 kHz mono signed PCM16 little-endian；它先写入固定 44-byte canonical RIFF/WAVE 头，再在浏览器实际 AIO-B playout rate 不同的情况下执行确定性的 server-owned 线性 PCM16 mono 重采样，并把 capability 精确声明为 `server_linear_pcm16_mono`。同采样率保持 PCM 样本但仍输出 Gateway canonical WAV。空响应、奇数字节、Provider/最终输出超限或未知 resampling capability 必须 fail closed，不能由浏览器、Provider 标签或隐藏 graph adaptation 伪装成目标采样率。该 OpenAI 合同不能自动套用到 Qwen 或其他 Provider；每个新增 Adapter 必须声明并验证自身实际格式。当前 24 kHz OpenAI TTS 输出可以显式转换到本机 Chrome 常见的 48 kHz AIO-B rate；最终 candidate 仍必须实听并记录实际 Provider、输入/输出 rate 和物理 playout 结果。
