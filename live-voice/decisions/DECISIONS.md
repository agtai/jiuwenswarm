...
- 日期：2026-08-23
- 状态：Accepted scoped 实现决定（基线 `c31e85ade1a69e934d05bfb9c277568a1238663c`，分支 `hx/0823_generation_interruption`）。本决定关闭"生成期免提语音无法中断或取代该回答"这一实现缺口，不授予物理、延迟、flag-on 上线、controlled-candidate、feature-complete、Production 或 `develop` credit。
...
- 证据与判读：新增后端 `test_generation_time_interruption.py` `13 passed`；`test_agent_conversation_runtime.py` `72 passed`；conversation runtime `50 passed`；P2 adapter `47 passed`；前端 `npm run test:live-voice-integrated-web` `480 passed`（`c31e85ade` 既有 472 + 8 新增）。后端 7 个变异体全部被杀死，前端 8 个变异体杀死 7 个，存活的一个是纵深防御守卫且已在证据中如实标注为未覆盖。完整 live_voice 后端套件中的 8 项失败已在基线 commit 的干净 worktree 中复现，与本改动无关。详见 [evidence](../evidence/GENERATION_TIME_INTERRUPTION_20260823.md)。
- 独立评审修复：首版实现被独立评审判为 FAIL（C0/I2/M0）。其一，即使 CR 围栏未应用也发出了 `round.cancel`——terminal 目标多发一次、stale 目标的取消甚至被接受；现已改为取消只能作为本次真正应用的围栏的伴随动作，已结算或已被取代的目标返回 `ALREADY_SETTLED` 且零取消，并补齐零取消断言与 stale 专用用例。其二，在途插话的成功/失败回调只检查挂载状态，切换 Session 后会污染后继路由；现要求精确 activation owner、Session 与 voice-loop generation，且未决插话已纳入采集授权屏障、关闭恢复屏障、cleanup 前置条件、轮次/文本准入与 `settleRetainedP2Operations` 的精确旧 owner 结算。
- 已知限制与 flag-on 阻断：Task 终态播报若落在用户插话说到一半时，无法在不抢话的前提下播出。现已改为拒绝播放而非尝试播放（不再让整条 P1 路由失败），但既有播报恢复仍会重建 P1 并因此丢弃正在说的那句话。生成期监听是让该竞态可达的唯一原因，修复它属于 cross-load arbitration 范围。前端用例刻意断言该限制的当前形态，使其被修复时立即失败而不会被遗忘。**在此修复前不得开启 flag。**
- 明确非声明：未做真机麦克风/TTS 旅程；未测量生成期监听窗口的延迟；生成期开麦下的回声与 double-talk 行为未评估；无 flag-on 上线、A/B 或回退证据；未做 ChatPanel ownership hook 与生成期窗口的端到端双标签 takeover 用例，面板层的 ownership 让出表现为 close，由 Exit 用例覆盖。
