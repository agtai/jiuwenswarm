...
- 2026-08-24 最终 follow-up 结论：对不可变候选 `6559c38e` 的独立单 finding 复核返回 **PASS — C0/I0/M0**。精确 object ownership 阻止已成功 `close(A)` 的前驱被 B continuation 复活；在途撤销共享同一 promise，失败撤销保留 A 做有界重试，B 仍独立发布并只撤销一次。确定性 oracle 构造 B activation 悬挂 → Exit → `close(A)` 成功 → B settlement，并精确断言 A/B close 次数；使用 `dab64023` 代码与新 oracle 的红组合为 `594/595`，唯一失败是 A `2 !== 1`，候选 Formal Web 连续两次 `496/496`，`git diff --check` PASS。该结果关闭 D-096 源码审核 Gate；下一步仅进入 D-095 §5 的 flag-on 真机麦克风/TTS 验收，不改变默认关闭、物理、延迟、上线或产品就绪结论。

## D-097 接受最小核心生成期打断真机验收并关闭功能包

- 日期：2026-08-25
- 状态：用户明确选择一次“生成中打断并取代”作为本功能的最小真机覆盖，并在精确部署源 `2c04da22` 上报告 PASS。该结果关闭 D-095/D-096 的核心功能包，不等于完整产品、固定语料延迟、回声/双讲、后台 Task 并发、默认开启或 Production 验收。
