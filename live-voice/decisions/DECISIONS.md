...
- 日期：2026-08-23
- 状态：实现完成，等待独立 Tier-3 follow-up（基线 `5415a3d3fb2c3c50b0b59fb5f5ad3a0aec423465`，分支 `hx/0823_generation_interruption`）。本决定不把任何 module Gate 判为已关闭。本决定关闭"生成期免提语音无法中断或取代该回答"这一实现缺口，不授予物理、延迟、flag-on 上线、controlled-candidate、feature-complete、Production 或 `develop` credit。
...
- 开关与回退决定：`VITE_FEATURE_LIVE_VOICE_GENERATION_INTERRUPTION` 默认关闭。开启会在提交与首音之间打开麦克风，改变每一轮免提的麦克风占用、回声暴露与采集成本。关闭时行为与既有实现完全一致，由未改变的 474 例基线累积套件证明。需注意这是浏览器行为一致，不是协议面一致：新 RPC 不受该 Vite flag 控制，在服务端与 Gateway 白名单中始终注册，已认证的 P2 客户端显式调用仍会执行。
- 证据与判读：新增后端 `test_generation_time_interruption.py` `15 passed`；`test_agent_conversation_runtime.py` `73 passed`；conversation runtime `50 passed`；P2 adapter `48 passed`；registry `176 passed / 6` 既有失败；前端 `npm run test:live-voice-integrated-web` `486 passed`（既有 474 + 12 新增）。后端 11 个变异体全部被杀死；前端 16 个杀死 13 个。合计 27 个中 24 杀死、3 个存活（playing 期 speech-start 守卫、ownership 让出时的窗口清理、可重试打断句柄的保留），均已在证据中如实标注。未列入变异表的不变式不声称已经变异检验。完整 live_voice 后端套件中的 8 项失败已在基线 commit 的干净 worktree 中复现，与本改动无关。详见 [evidence](../evidence/GENERATION_TIME_INTERRUPTION_20260823.md)。
