...
- 日期：2026-08-23
- 状态：实现完成，等待独立 Tier-3 follow-up（基线 `24e7e6106`，分支 `hx/0823_generation_interruption`；该 named ref 已在本工作期间 amend 四次，集成前必须用 `git merge-base` 重新确认并重新冻结）。本决定不把任何 module Gate 判为已关闭。本决定关闭"生成期免提语音无法中断或取代该回答"这一实现缺口，不授予物理、延迟、flag-on 上线、controlled-candidate、feature-complete、Production 或 `develop` credit。
...
- 证据与判读：新增后端 `test_generation_time_interruption.py` `17 passed`；`test_agent_conversation_runtime.py` `73 passed`；conversation runtime `50 passed`；P2 adapter `48 passed`；registry `176 passed / 6` 既有失败；前端 `npm run test:live-voice-integrated-web` `492 passed`（既有 478 + 14 新增）。合计 31 个变异体中 26 杀死、5 个存活。存活分两类：一类是修复已应用但触发路径在我的挖载用例中不可达（Exit 时对让位播报的两项），应视为“已修复但未证”而非已覆盖；另一类是外部效果已由独立守卫保证的状态卫生（playing 期 speech-start 守卫、ownership 让出窗口清理、可重试打断句柄）。另有两条必查不变式无 mutation-sensitive oracle（普通采集不轮询、跨 Session 精确重放），均已在证据 §3.2 列明。未列入变异表的不变式不声称已经变异检验。完整后端套件中的 11 项失败（live_voice 8 + gateway 3）已在基线 commit 的干净 worktree 中复现，与本改动无关。详见 [evidence](../evidence/GENERATION_TIME_INTERRUPTION_20260823.md)。
