...
- Internal request surface：只新增 `live_voice.internal.native.propose`、`live_voice.internal.native.presentation_ack`、`live_voice.internal.native.close` 三个 Gateway→AgentServer E2A method。它们属于 `live-voice.native-interaction.v1`，要求 exact capability、activation binding、closed payload、request ID/fingerprint replay 和 open lease；明确不进入 Web registered/forwarded/allowlisted methods。`propose` 统一承载 action/turn/response/done/delegate，Runtime admission 或 canonical Agent/Task delegate result 是 response，不新增 Gateway 可伪造的 delegate-result method；ACK/played cursor 与 close 只能作用于 exact Native route。
