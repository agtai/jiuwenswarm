...
## D-099 接受一个前驱段的精确跨 capture Batch 连续识别

- 日期：2026-08-24
- 状态：用户在获知 Tier-3 扩界及后续任务后明确要求执行；本决定仅激活 `hx/0823_generation_interruption` 上的窄连续识别包，不关闭 D-098 Gate，不授权远端更新。
...
- Gate：正向必须证明前驱话头与后继尾部只形成一个完整 Provider 输入和一个 final；负向必须覆盖重排、伪造音频、stale/replay、跨 Session/interaction/activation/connection/track，且所有拒绝路径的 Agent/Tool/Task/history 副作用为零。受影响自动化、静态检查、生产构建及一次独立 Tier-3 审核关闭后，才能进入 D-098 §5 真机验收。
- 实施状态：源候选 `35537a9a` 已实现上述边界；受影响 backend `257/257`、Formal Web `496/496`、Browser Gateway Media `38/38`、Browser Dedicated Media `27/27`、Gateway Batch Speech `30/30`、Ruff、diff check 与 `build:live-voice` 均 PASS。独立 Tier-3 审核和真机验收仍未发生，因此 D-098 Gate 与物理结论保持 BLOCKED。
- 2026-08-24 定向审核修复：`35537a9a` 的独立审核返回 **FAIL — C0/I5/M0**，分别命中 continuation 单段 Batch 准入、前驱可被多个后继复用、相同 generation、最小 tombstone 窗口丢失前驱以及 Exit 未取消在途组合识别。`4c1c7d68` 在原 D-099 范围内完成 fix-only 修复；当前受影响 backend `261/261`、Formal Web `496/496`、Browser Gateway Media `38/38`、Browser Dedicated Media `27/27`、Gateway Batch Speech `31/31`、Ruff、diff check 与生产构建均 PASS。下一步只对 `35537a9a..4c1c7d68` 做五项接缝及累计交互复核，不重启历史广审；follow-up 与真机验收未完成前，D-098 Gate、物理和上线结论继续 BLOCKED。
- 2026-08-24 Exit/cancel follow-up 修复：`4c1c7d68` 的 fix-only follow-up 接受其余四项，返回 **FAIL — C0/I1/M0**：Exit 在浏览器 audio close 悬挂时尚未发送识别 cancel/媒体撤销，且显式 fence 后迟到的 `REQUEST_ABORTED` 会再次 cancel 同一 operation。`dab64023` 不改变 D-099 协议或产品策略，只把精确识别 fence、两段本地 frame 释放和两项 media revoke 前移到 fallible browser cleanup 之前，并让 active recognition token 成为 cancel 的唯一所有者；Formal Web `496/496`、Browser Audio `103/103`、Browser Gateway Media `38/38`、Browser Dedicated Media `27/27`、Gateway Batch Speech `32/32`、diff check 与生产构建 PASS。下一步仅复核 `4c1c7d68..dab64023` 这一项及其累计交互；该 follow-up 与真机验收完成前，D-098 Gate、物理和上线结论继续 BLOCKED。
...
- 2026-08-24 最终 follow-up 结论：对不可变候选 `6559c38e` 的独立单 finding 复核返回 **PASS — C0/I0/M0**。精确 object ownership 阻止已成功 `close(A)` 的前驱被 B continuation 复活；在途撤销共享同一 promise，失败撤销保留 A 做有界重试，B 仍独立发布并只撤销一次。确定性 oracle 构造 B activation 悬挂 → Exit → `close(A)` 成功 → B settlement，并精确断言 A/B close 次数；使用 `dab64023` 代码与新 oracle 的红组合为 `594/595`，唯一失败是 A `2 !== 1`，候选 Formal Web 连续两次 `496/496`，`git diff --check` PASS。该结果关闭 D-099 源码审核 Gate；下一步仅进入 D-098 §5 的 flag-on 真机麦克风/TTS 验收，不改变默认关闭、物理、延迟、上线或产品就绪结论。

## D-100 接受最小核心生成期打断真机验收并关闭功能包

- 日期：2026-08-25
- 状态：用户明确选择一次“生成中打断并取代”作为本功能的最小真机覆盖，并在精确部署源 `2c04da22` 上报告 PASS。该结果关闭 D-098/D-099 的核心功能包，不等于完整产品、固定语料延迟、回声/双讲、后台 Task 并发、默认开启或 Production 验收。
