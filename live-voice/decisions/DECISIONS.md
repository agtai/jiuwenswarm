...
- Browser projection：唯一 Browser-visible 扩张是既有 `live_voice.composition.p2.notification.next` 的 closed `kind="native.audio"` variant，绑定 exact activation、`ResponseRef`、audio `PresentationUnit` 和既有 response-bound dedicated-media downlink descriptor。Gateway 从有界 Native downlink queue 投影；队列为空继续读取 AgentServer notification。该 variant 不新增 Browser RPC、media frame/control、WebSocket subprotocol、Task/Agent/history authority或第二 durable consumer。
