...
> Integration mapping (2026-09-02): W3 D-098/D-099/D-100 above retain their
> Task-control meanings. Generation-interruption source `c400188d` used those
> same three IDs; its imported decisions below are D-104/D-105/D-106.
> D-101/D-102/D-103 are reserved for the paired Native adaptation `4b247ddb`.
> Historical source evidence keeps its original branch-local IDs and exact
> acceptance scope; this mapping changes no product contract or past result.

## D-104 接受 Agent 生成期插话的精确围栏与取代

- 日期：2026-08-23
- 状态：实现完成，等待独立 Tier-3 follow-up（基线 `9a3a65fd0`，分支 `hx/0823_generation_interruption`；该 named ref 已在本工作期间 amend 四次并另有两次前进，集成前必须用 `git merge-base` 重新确认并重新冻结）。本决定不把任何 module Gate 判为已关闭。本决定关闭"生成期免提语音无法中断或取代该回答"这一实现缺口，不授予物理、延迟、flag-on 上线、controlled-candidate、feature-complete、Production 或 `develop` credit。
...
- 开关与回退决定：`VITE_FEATURE_LIVE_VOICE_GENERATION_INTERRUPTION` 默认关闭。开启会在提交与首音之间打开麦克风，改变每一轮免提的麦克风占用、回声暴露与采集成本。关闭时行为与既有实现完全一致，由未改变的 478 例基线累积套件证明。需注意这是浏览器行为一致，不是协议面一致：新 RPC 不受该 Vite flag 控制，在服务端与 Gateway 白名单中始终注册，已认证的 P2 客户端显式调用仍会执行。
- 证据与判读：新增后端 `test_generation_time_interruption.py` `19 passed`；`test_agent_conversation_runtime.py` `73 passed`；conversation runtime `50 passed`；P2 adapter `48 passed`；registry `176 passed / 6` 既有失败；前端 `npm run test:live-voice-integrated-web` `496 passed`（既有 478 + 18 新增）。合计 41 个变异体中 34 杀死、7 个存活。存活分两类：一类是一处修复的两半互为冗余（Exit 时对让位播报的两项）——第五轮复审用组合变异证明触发路径可达且被用例杀死，此前“已修复但未证/不可达”的说法有误，已撤回；另一类是外部效果已由独立守卫保证的状态卫生（playing 期 speech-start 守卫、ownership 让出窗口清理、可重试打断句柄）。另有五条必查不变式无 mutation-sensitive oracle（普通采集不轮询、跨 Session 精确重放、迟到打断结果的精确 voice-loop 换代、替换目标的 correlation 精确匹配、Exit 让位修复的单独一半），均已在证据 §3.2 列明。未列入变异表的不变式不声称已经变异检验。完整后端套件 `3961 passed / 11 failed`，那 11 项（live_voice 8 + gateway 3）已在基线 commit 的干净 worktree 中复现，与本改动无关。详见 [evidence](../evidence/GENERATION_TIME_INTERRUPTION_20260823.md)。
- 首轮独立评审修复：首版实现被独立评审判为 FAIL（C0/I2/M0）。其一，即使 CR 围栏未应用也发出了 `round.cancel`——terminal 目标多发一次、stale 目标的取消甚至被接受；现已改为取消只能发生在仍存在可围栏活跃目标时，已结算或已被取代的目标返回 `ALREADY_SETTLED` 且零取消，并补齐零取消断言与 stale 专用用例。其二，在途插话的成功/失败回调只检查挂载状态，切换 Session 后会污染后继路由；现要求精确 activation owner、Session 与 voice-loop generation，且未决插话已纳入采集授权屏障、关闭恢复屏障、cleanup 前置条件、轮次/文本准入与 `settleRetainedP2Operations` 的精确旧 owner 结算。
- 第二轮独立评审修复（C0/I6/M2）：Exit 未清理生成期监听窗口，导致任何一次 Exit 之后本会话再也无法开窗口（功能静默失效）；可重试的打断失败被无条件丢弃句柄，使旧 owner 无法精确结算；台账淘汰遇未决项即停扫，使上限与保留未决 future 两个义务无法同时成立；生产 Registry->lease->Runtime 接线无任何 Gate。四项均已修复并配变异敏感的 oracle（仅可重试句柄的重放路径仍无 oracle，已如实标注）。第五项“barge-in 后的插话仍发 round.cancel”保留行为：barge-in 不碰 Harness，那个 round 仍在生成，正是说话者要停的；已改用显式用例固定该契约，并把表述改为“取消绑定到仍可围栏的活跃目标”而非“本次产生了围栏效果”。
- 第三轮独立评审修复（C0/I4/M3）：让位的 Task 播报在判定前就结算了 playout，使清理能 ACK 一条从未播出的播报；浏览器在 RPC 发出前就乐观标记 response 为已打断且忽略返回值，使服务端返回 `ALREADY_SETTLED`（什么都没围栏）时那个仍然有效的回答被静默丢弃；Session 切换无条件清掉精确重放句柄。三项均已修复，其中 `ALREADY_SETTLED` 与 scope 负向 oracle 的变异均被杀死；Exit 让位两项的触发路径未能在挖载用例中复现，已如实标注为“已修复但未证”。第四项命名基线再次漂移，已 rebase 并在证据中改为“用 merge-base 实测，ref 名不是基线标识”。该 ref 在这轮修复提交之后又 amend 了一次（`7c589dcda` -> `24e7e6106`，补入 L0 latency baseline，触及本改动也改过的 `productP1VoiceRoute.ts`），已再次 rebase，最终基线为 `24e7e6106`，全部计数在其之上重测。
- 第四轮独立评审修复（C0/I3/M3）：可重试失败的打断句柄被有意保留以便原 owner 重放，但三处屏障按“存在任何未决打断”而非按 owner 匹配，且跨 Session 清理直接关闭前任而从不重放——关闭后 `interruptGeneration` 因失去 binding 而不可能执行，句柄遂成永久物，此后每个后继 Session 都被拒绝进入 generation 监听。已改为逐处按 owner 匹配，并在 activation 关闭时退休该句柄（重试幂等由服务端 `action_id` 台账承担）。第二项：`abandonCapture` 在物理停止采集之前就推进 operation generation，使 `stopCapture` 期间到达的权威 provider speech-start 因换代检查被丢弃，释放照常完成，用户已经开口的话被当作静音窗口丢弃而旧回答继续播放；现改为仅在释放不可逆时才推进换代，并在停止后重新检查 speech-start，观察到就保留采集。两项各自的变异均被杀死。另补两个 oracle：替换轮次必须在前驱围栏成功之后才提交（变异 KILLED）、客户端 scope 负向（已有）。M3 指出的两条无 oracle 边界中，voice-loop 换代校验经尝试仍不可达，已如实计入证据 §3.2。
...
## D-105 接受一个前驱段的精确跨 capture Batch 连续识别

- 日期：2026-08-24
- 状态：用户在获知 Tier-3 扩界及后续任务后明确要求执行；本决定仅激活 `hx/0823_generation_interruption` 上的窄连续识别包，不关闭 D-104 Gate，不授权远端更新。
...
- Gate：正向必须证明前驱话头与后继尾部只形成一个完整 Provider 输入和一个 final；负向必须覆盖重排、伪造音频、stale/replay、跨 Session/interaction/activation/connection/track，且所有拒绝路径的 Agent/Tool/Task/history 副作用为零。受影响自动化、静态检查、生产构建及一次独立 Tier-3 审核关闭后，才能进入 D-104 §5 真机验收。
- 实施状态：源候选 `35537a9a` 已实现上述边界；受影响 backend `257/257`、Formal Web `496/496`、Browser Gateway Media `38/38`、Browser Dedicated Media `27/27`、Gateway Batch Speech `30/30`、Ruff、diff check 与 `build:live-voice` 均 PASS。独立 Tier-3 审核和真机验收仍未发生，因此 D-104 Gate 与物理结论保持 BLOCKED。
- 2026-08-24 定向审核修复：`35537a9a` 的独立审核返回 **FAIL — C0/I5/M0**，分别命中 continuation 单段 Batch 准入、前驱可被多个后继复用、相同 generation、最小 tombstone 窗口丢失前驱以及 Exit 未取消在途组合识别。`4c1c7d68` 在原 D-105 范围内完成 fix-only 修复；当前受影响 backend `261/261`、Formal Web `496/496`、Browser Gateway Media `38/38`、Browser Dedicated Media `27/27`、Gateway Batch Speech `31/31`、Ruff、diff check 与生产构建均 PASS。下一步只对 `35537a9a..4c1c7d68` 做五项接缝及累计交互复核，不重启历史广审；follow-up 与真机验收未完成前，D-104 Gate、物理和上线结论继续 BLOCKED。
- 2026-08-24 Exit/cancel follow-up 修复：`4c1c7d68` 的 fix-only follow-up 接受其余四项，返回 **FAIL — C0/I1/M0**：Exit 在浏览器 audio close 悬挂时尚未发送识别 cancel/媒体撤销，且显式 fence 后迟到的 `REQUEST_ABORTED` 会再次 cancel 同一 operation。`dab64023` 不改变 D-105 协议或产品策略，只把精确识别 fence、两段本地 frame 释放和两项 media revoke 前移到 fallible browser cleanup 之前，并让 active recognition token 成为 cancel 的唯一所有者；Formal Web `496/496`、Browser Audio `103/103`、Browser Gateway Media `38/38`、Browser Dedicated Media `27/27`、Gateway Batch Speech `32/32`、diff check 与生产构建 PASS。下一步仅复核 `4c1c7d68..dab64023` 这一项及其累计交互；该 follow-up 与真机验收完成前，D-104 Gate、物理和上线结论继续 BLOCKED。
- 2026-08-24 pending-successor follow-up 修复：`dab64023` 的 follow-up 接受同步 cancel/frame 释放，但返回 **FAIL — C0/I1/M0**：B activation 悬挂期间 Exit 已成功 `close(A)` 后，B continuation 会把 A 重新加入 retained map，最终产生 `activate(B), close(A), close(A), close(B)`。`6559c38e` 仅在 owner 仍持有同一精确 A binding object 时转移 predecessor；成功撤销的 A 不再复活，未决或失败撤销仍可重试。未新增 tombstone、集合、协议或策略。该精确 oracle 在旧源上以 A close `2 !== 1` 失败，修复后 A/B 各一次；Formal Web `496/496`、Gateway Batch Speech `32/32`、diff check 与生产构建 PASS。下一步仅复核 `dab64023..6559c38e`；follow-up 与真机验收前 Gate 继续 BLOCKED。
- 2026-08-24 最终 follow-up 结论：对不可变候选 `6559c38e` 的独立单 finding 复核返回 **PASS — C0/I0/M0**。精确 object ownership 阻止已成功 `close(A)` 的前驱被 B continuation 复活；在途撤销共享同一 promise，失败撤销保留 A 做有界重试，B 仍独立发布并只撤销一次。确定性 oracle 构造 B activation 悬挂 → Exit → `close(A)` 成功 → B settlement，并精确断言 A/B close 次数；使用 `dab64023` 代码与新 oracle 的红组合为 `594/595`，唯一失败是 A `2 !== 1`，候选 Formal Web 连续两次 `496/496`，`git diff --check` PASS。该结果关闭 D-105 源码审核 Gate；下一步仅进入 D-104 §5 的 flag-on 真机麦克风/TTS 验收，不改变默认关闭、物理、延迟、上线或产品就绪结论。

## D-106 接受最小核心生成期打断真机验收并关闭功能包

- 日期：2026-08-25
- 状态：用户明确选择一次“生成中打断并取代”作为本功能的最小真机覆盖，并在精确部署源 `2c04da22` 上报告 PASS。该结果关闭 D-104/D-105 的核心功能包，不等于完整产品、固定语料延迟、回声/双讲、后台 Task 并发、默认开启或 Production 验收。
- 最小验收决定：先提交“请详细介绍太阳系八大行星”，在 Agent 思考/生成期间说“算了，换个问题，天空是什么颜色？”。验收要求替换问题的文字和语音均正常出现、旧回答停止且页面不再短暂显示 Live Voice 错误。原 handoff 的静默窗口和后台 Task 并发场景未在本次重跑，因而不获得新的真机信用；`round.cancel` 不得升级为 `task.cancel` 仍由源 Gate 和自动化约束。
- 验收期修复：`e8f55ded` 把已退休 P2 owner 的预期 `STREAMING_SPEECH_ROUTE_ABORTED` 归一为清理状态，同时保留当前 owner 的同名错误；`2c04da22` 把遗漏的 `live_voice.composition.p2.interrupt_generation` 接入 AgentServer 顶层产品路由，并仅在已经收到但不满足 durable schema/binding 的确定性响应上释放前端 retained operation。网络结果不确定时仍按同一 request ID 精确重放。
- 证据：Formal Integrated Web `499/499`，受影响 AgentServer/Registry/Gateway 集 `66/66`，`build:live-voice` 与 `git diff --check` PASS。受控 `formal-web-validation` 启动器记录 source dirty count `0`、flag-on bundle、P2/P3 route 与真实 Speech/security probe PASS。最终真机日志只有一个 interrupt normalize，约 0.15 秒内写回同 request ID 的 `e2a.complete` 并取消旧 round；没有同 ID recovery replay 或用户可见产品错误。随后旧 owner 关闭时记录的一次预期 `STREAMING_SPEECH_ROUTE_ABORTED` 已由 `e8f55ded` 归一为清理状态。
- 开关与集成：`VITE_FEATURE_LIVE_VOICE_GENERATION_INTERRUPTION` 继续默认关闭；本决定不授权 remote ref 更新、`develop` 集成、A/B rollout 或完整产品就绪声明。
- 重新评估条件：核心打断再次出现可复现错误或重复 replay；需要默认开启；或需要为静默窗口、后台 Task、回声/双讲、固定语料延迟或跨负载取得新信用。
## D-101 以独立 Tier-3 合同激活 OpenAI Realtime Native Interaction Engine

- 日期：2026-08-25。
- 状态：Accepted design and implementation activation（用户批准 Native 显式 opt-in、Cascade 保持默认的方案，并要求使用 Goal 推进完整 Tier-3 开发；本决定只接受设计和激活实施，不授予尚未产生的源码、自动化、真实 Provider/device、feature-complete、Production 或 remote credit）。
- 路径与基线：实现从 `hx/0812_live_voice_w3@1742c1b4e5fa5e7a25a7b41dad9c8eef8453e3cc` 建立独立 `codex/openai-realtime-native-interaction-engine` worktree/branch；旧 `codex/openai-realtime-native-voice@42f448aff7f8af9b0759c59a841f6a57a5792449` 保留为低 authority Speech Adapter 参考且不得改写。`LIVE_VOICE_INTERACTION_ENGINE=cascade` 保持默认，只有显式 `openai-realtime-native` 才激活 Native；默认模型为 `gpt-realtime-2.1-mini` 且允许服务端显式 override。同一 interaction 不 silent fallback，失败后重新选择 Cascade 必须创建新 interaction。
- 架构决定：抽取一个只拥有 WebSocket、event ledger、bounded queue/deadline、cleanup、unique finalization 和 failure precedence 的 `OpenAIRealtimeSession` kernel，供现有 Speech Adapter 与新的独立 `OpenAIRealtimeNativeInteractionEngine` 消费。Native Engine 维护连续 session 和 Provider state，只把 GA Realtime events 投影为闭集 `LISTEN/SILENCE/TURN_COMMIT/SPEAK/STOP/REVISE/DELEGATE` proposal；Conversation Runtime 继续唯一拥有 turn/response/generation/fence/history admission，Audio I/O 拥有实际播放与 ACK，Gateway 拥有 key/transport，Agent/Task Bridge 拥有业务副作用。
- 合同扩张：新增独立 closed `live-voice.native-interaction.v1`，其 `NativeTurnCommit` 绑定 scope/interaction/turn、Provider session/item/event、committed audio timing/cursor 和 Runtime activation fence，transcript 仅为可选 audit projection。现有非空文本 `TurnCommit` v2、serialization 和 SQLite schema 不变；只有 Runtime 验证通过的 bounded `jiuwen_delegate.request_text` proposal 才转换为带完整 native provenance 的标准 `TurnCommit`，再由现有 committed-input resolver 进入 Agent Bridge 或 Voice–Task Bridge/P3。
- Native audio、打断与历史：Provider direct foreground audio 只有经过 Runtime `ResponseRef + generation` admission 才可进入 dedicated media。播放期 speech-start 先停止浏览器本地播放，再由 Runtime exact cancel 推进 fence，随后 Engine 发送 `response.cancel`，并使用 Audio I/O 实际 played cursor 发送 `conversation.item.truncate`；旧 generation 的 audio/transcript/done/function/ACK 全部失效。Provider context 不是 canonical history；input transcript 可缺失，direct assistant transcript 只有在 exact provenance、successful done、完整 PCM presentation ACK 和 current generation 同时成立时才可写入；被打断部分不写 assistant text。
- 委托防火墙：Provider 只看见 closed application function `jiuwen_delegate({request_text})`，不看见 Jiuwen Tool、Task command、Store、filesystem 或 MCP。Engine 只提出 proposal；Runtime 验证并调用现有 Bridge。canonical Agent/Task result 作为 `function_call_output` 回送 Provider 前必须获得新的 Runtime native response generation；后续音频继续受 fence/ACK 约束。
- 关闭与退化：unknown config/model/capability、malformed/replayed/cross-scope event、无 authority 的 mutation 和 stale generation 全部 fail closed。authority/fence、explicit cancel、deadline、Provider、local protocol/codec、cleanup 依序冻结 primary failure；cleanup 不覆盖主失败。activation 失败不得注册半激活 media authority，session 中途失败不得自动创建 Cascade interaction。
- 交付门：按 red-green-refactor 依次关闭 v1 contract、共享 kernel characterization、连续 session/action、Runtime fence、barge-in、delegate/Bridge、history/config/close 和 Cascade cumulative regression。所有 mutation-capable negative/race 路径必须断言 zero forbidden effects；fake race 使用 barrier/manual clock/event injection，不使用任意 sleep。源码与自动化完成后冻结 exact candidate 并进行独立 Tier-3 cold review 和 fix-only follow-up；只有 `C0/I0` 后才可运行真实 Provider probe，真实设备/人工仍为独立 Gate。
- 非声明与重新评估：详细 owner、state machine、event mapping、A–E reuse boundary、测试矩阵和 exclusions 见 [Native Interaction Engine design](../architecture/OPENAI_REALTIME_NATIVE_INTERACTION_ENGINE_2026-08-25.md)。若实现要求改变现有 TurnCommit/DB schema、第二 classifier/Runtime/history/Task/session authority、Provider 直连 Jiuwen Tool/MCP/Task、缺失可验证 played cursor、部分 transcript canonicalization、自动跨 Engine fallback、新公开协议/部署/account/billing 或 remote ref update，必须先重新定 scope、合同和 risk tier。

## D-102 Native v1 使用有界 internal carrier 与既有 P2 notification 的单一音频变体

- 日期：2026-08-25。
- 状态：Accepted D-101 implementation re-scope（在编写生产代码前的源码勘察证明，Provider session/原始音频由 Gateway 持有而 Conversation Runtime/Agent/Task Bridge 由 AgentServer 持有；若不冻结一个跨进程 authority carrier 就只能复制 Runtime 或让 Browser 中继 authority。用户已要求在设计后连续完成真正 Native Engine 并明确要求遇到新协议先 re-scope/re-tier；本决定保持 Tier-3，接受下面最小共享扩张，不授予实现或验收信用）。
- 激活与 capability：Native 继续复用 authenticated P2 activation，不增加 Browser activation method。AgentServer 为 exact authenticated scope/session/interaction/activation generation mint 一个随机 256-bit、进程内、不可持久化的 capability；Gateway response observer 只在私有 response seam 取得它，对 Browser 发送前必须删除。capability 不得进入 payload log、telemetry、history、Store 或 evidence，activation close/replacement/expiry 立即使其失效。
- Internal request surface：只新增 `live_voice.internal.native.propose`、`live_voice.internal.native.presentation_ack`、`live_voice.internal.native.close` 三个 Gateway→AgentServer E2A method。它们属于 `live-voice.native-interaction.v1`，要求 exact capability、activation binding、closed payload、request ID/fingerprint replay 和 open lease；明确不进入 Web registered/forwarded/allowlisted methods。`propose` 统一承载 action/turn/response/done/delegate，以及 metadata-only audio observation；后者只允许 `provider_event_id`、`provider_response_id`、`provider_item_id`、`content_index`、`sequence`、`sample_count`、`content_sha256` 和已 admission 的 `ResponseRef`，严禁 PCM、base64、bytes 或可还原音频内容。AgentServer 先返回 Runtime-authored audio `PresentationUnit`，Gateway 才可将本地对应 PCM 放入 dedicated-media downlink。Runtime admission 或 canonical Agent/Task delegate result 是 response，不新增 Gateway 可伪造的 delegate-result method；ACK/played cursor 与 close 只能作用于 exact Native route。
- Browser projection：Browser-visible 扩张只含两个闭合集合且不新增 RPC。Native P2 activation 经 Gateway 删除私有 capability 后返回 exact `native_interaction={contract_version:"live-voice.native-interaction.v1",engine:"openai-realtime-native",model}` descriptor；Cascade 不含该字段，Engine/model 只来自服务端配置与私有 activation seam，不接受 Browser 覆盖。既有 `live_voice.composition.p2.notification.next` 增加 closed `kind="native.audio"` variant，绑定 exact activation、`ResponseRef`、audio `PresentationUnit` 和既有 response-bound dedicated-media downlink descriptor。Gateway 从有界 Native downlink queue 投影；队列为空继续读取 AgentServer notification。两者都不新增 Browser RPC、media frame/control、WebSocket subprotocol、Task/Agent/history authority或第二 durable consumer。
- 失败与 zero effects：wrong/missing/stale capability、Browser direct call、changed replay、cross-scope/generation、closed lease、Gateway/AgentServer timeout、malformed notification 或 queue overflow 全部 fail closed；不得创建 Provider/Runtime response、Agent/Tool/Task effect、media ticket、history、presentation ACK 或 retained registry entry。Native failure不 silent fallback 为 Cascade。
- 非声明与重新评估：本决定不改变 `TurnCommit` v2、SQLite/P3 command、media v1、canonical history、Production、public deployment 或 remote ref。任何更多 internal method、Browser RPC/notification kind、持久 capability、第二 queue/Runtime/classifier、通用 execution API 或把 Browser 当 authority relay，都必须重新定 scope、schema owner 和 Tier-3 acceptance。详细边界见 [Native design §5.4](../architecture/OPENAI_REALTIME_NATIVE_INTERACTION_ENGINE_2026-08-25.md#54-gatewayagentserver-carrier-与-browser-downlink-投影)。

## D-103 Native response 使用单一有界流式载体并补齐围栏与生命周期闭环

- 日期：2026-08-25。
- 状态：Accepted D-101/D-102 Tier-3 correction re-scope（exact candidate `0a1a5d36e851998e5fe384c48302f7b446f52d12` 的独立冷复核为 `C2/I5/M0 — FAIL`；用户在该失败证据后明确选择方案 A。本决定只批准修复设计和实施，不把失败候选、修复中工作、真实 Provider/device/human 或 Production 记为通过）。
- 响应级流式载体：一个 Provider Native response 只创建一个 `native.audio` P2 notification、一个 dedicated-media ticket、一个 WebSocket 和一个有界 async source。每个已规范化的 20 ms Provider audio frame 仍逐一经过 Runtime admission 并获得独立 `PresentationUnit`，但按 sequence 追加到同一 response source；Browser descriptor 使用既有字段的 `streaming=true`、`frame_count=null`，不新增 RPC、notification kind、media frame/control 或 subprotocol。source 只保留有界未消费 frame、sequence→Runtime unit 映射和增量 digest，不保留整段原始音频。
- ACK 与 backpressure：首个 Runtime unit 作为既有 route/ticket anchor；每个 browser media frame 与一个 Runtime unit 一一对应。Browser 完成播放后以既有 receipt 回报最终 rendered-through sequence；Gateway 将它精确映射到最后已播放 Runtime unit，并以一个现有 `native.presentation_ack` 确认同一 response 的连续 presentation prefix。错序、越界、重复、stale generation、队列饱和或映射缺失 fail closed，且不得产生 history、Agent/Tool/Task、额外 ticket 或跨 response effect。
- Barge-in：speech-start 在首帧前、帧间和播放中都必须保留，直到 exact response 获得围栏。存在真实 Audio I/O played cursor 时，Runtime admission 先推进 generation fence，Engine 再发送 `response.cancel`，随后按该 cursor 精确 truncate。缺失 cursor 时，Browser 立即建立本地播放 fence，Runtime fence/cancel 当前 generation，Gateway 关闭 response source，Engine 丢弃该 generation 的迟到 audio/transcript/done/function；不得发送无事实依据的 Provider cancel/truncate。`native.presentation_ack` 只在既有 internal method 内增加 closed fence-only variant，不增加第四个 method。
- 历史、deadline 与资源关闭：history done-first/ACK-second 和 ACK-first/done-second 都调用同一 Runtime reconciliation；eligible canonical writer task 由 AgentServer 持有，使用有界自动重试并在 close 时 drain，不依赖 Browser 重放 ACK。delegate carrier 使用 method-specific 30 s deadline，仍保持 request replay、timeout-after-effect 与 cancel fence。Provider close 未完成时由一个有界 retained close owner 持有 Engine/capacity，直到 final close 或 shutdown 的 truthful failure；registry 不得通过提前 pop 假装资源释放。
