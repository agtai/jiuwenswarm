...
- 开关与回退决定：`VITE_FEATURE_LIVE_VOICE_GENERATION_INTERRUPTION` 默认关闭。开启会在提交与首音之间打开麦克风，改变每一轮免提的麦克风占用、回声暴露与采集成本。关闭时行为与既有实现完全一致，由未改变的 472 例基线累积套件证明。
- 证据与判读：新增后端 `test_generation_time_interruption.py` `13 passed`；`test_agent_conversation_runtime.py` `72 passed`；conversation runtime `50 passed`；P2 adapter `47 passed`；前端 `npm run test:live-voice-integrated-web` `482 passed`（既有 472 + 10 新增）。后端 8 个变异体全部被杀死；前端 14 个变异体杀死 12 个，2 个存活（playing 期 speech-start 守卫、ownership 让出时的窗口清理），均已在证据中如实标注为纵深防御而非已覆盖。完整 live_voice 后端套件中的 8 项失败已在基线 commit 的干净 worktree 中复现，与本改动无关。详见 [evidence](../evidence/GENERATION_TIME_INTERRUPTION_20260823.md)。
