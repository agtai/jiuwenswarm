...
- 日期：2026-08-23
- 状态：实现完成，等待独立 Tier-3 follow-up（基线 `a44843b62`，分支 `hx/0823_generation_interruption`；该 named ref 已在本工作期间 amend 四次并另有一个 docs-only 前进，集成前必须用 `git merge-base` 重新确认并重新冻结）。本决定不把任何 module Gate 判为已关闭。本决定关闭"生成期免提语音无法中断或取代该回答"这一实现缺口，不授予物理、延迟、flag-on 上线、controlled-candidate、feature-complete、Production 或 `develop` credit。
...
- 证据与判读：新增后端 `test_generation_time_interruption.py` `18 passed`；`test_agent_conversation_runtime.py` `73 passed`；conversation runtime `50 passed`；P2 adapter `48 passed`；registry `176 passed / 6` 既有失败；前端 `npm run test:live-voice-integrated-web` `495 passed`（既有 478 + 17 新增）。合计 37 个变异体中 30 杀死、7 个存活。存活分两类：一类是修复已应用但触发路径在我的挖载用例中不可达（Exit 时对让位播报的两项），应视为“已修复但未证”而非已覆盖；另一类是外部效果已由独立守卫保证的状态卫生（playing 期 speech-start 守卫、ownership 让出窗口清理、可重试打断句柄）。另有三条必查不变式无 mutation-sensitive oracle（普通采集不轮询、跨 Session 精确重放、迟到打断结果的精确 voice-loop 换代），均已在证据 §3.2 列明。未列入变异表的不变式不声称已经变异检验。完整后端套件 `3958 passed / 11 failed`，那 11 项（live_voice 8 + gateway 3）已在基线 commit 的干净 worktree 中复现，与本改动无关。详见 [evidence](../evidence/GENERATION_TIME_INTERRUPTION_20260823.md)。
...
- 播报让位决定：Task 终态播报若落在用户插话说到一半时，不得抢话，也不得拆毁正在承载这句话的 P1 路由。生成期监听是让该竞态可达的唯一原因——在此之前回答未落地时从不开麦。播放改为拒绝而非尝试，且该拒绝被归类为「让位」而非「失败」：精确的已投递播报被原样保留，说话者结束后由既有仲裁重播，不重新拉取、不重建 P1、不做未播即 ACK。真实播放失败仍走原有恢复路径。
