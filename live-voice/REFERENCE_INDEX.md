...
| Foreground latency optimization (2026-09-06) | [Source verification of the optimization directions](reviews/LATENCY_OPTIMIZATION_CODE_VERIFICATION_2026-09-06.md): which levers the code supports (prompt size, per-call model construction, per-attempt git rev-parse, per-operation SQLite connections, per-response TTS client), which were refuted (serial tools, git status on the dialogue path) and what remains unmeasured |
| Target architecture for a rewrite (2026-09-07) | [LiveVoice target architecture](LIVEVOICE_TARGET_ARCHITECTURE_2026-09-07.md): capability-derived design (sequence identity, session event log, one owner per process, three small databases), why the current code is 170K, invariant→component→test mapping, bottom-up estimate 26–29K ex-Native, and the route A (refactor to 60–65K) versus route B (rewrite behind the two seams) decision |
| Design-simplification slimming path (2026-09-07) | [LiveVoice design-simplification plan](LIVEVOICE_DESIGN_SIMPLIFICATION_PLAN_2026-09-07.md): the semantic-change path (rewrite contracts, rewrite tests) to 60–65K lines; per-module anatomy (value types, guards, owners), per-module target contracts, semantic-change catalogue SC-1..10, packages S0–S8 and the decisions it needs. Reproduce with `scripts/live_voice/slimming/anatomy_modules.py` |
| Official Hermes voice comparison (2026-09-06) | [Module-by-module comparison with NousResearch/hermes-agent@9a84bee26](reviews/OPENJIUWEN_LIVEVOICE_OFFICIAL_HERMES_VOICE_COMPARISON_2026-09-06.md): Hermes voice inventory (20,265 LOC, comparable set 17,887), the 18-module comparison, LiveVoice explained in Hermes-voice terms with reasonableness judgments, LOC and slimming analysis, and the AgentCore F1–F6 calibration against Hermes's core background-work substrate (≈3.3K). Reproduce with `scripts/live_voice/slimming/hermes_voice_inventory.py`. Supersedes the personal-repository comparison for planning |
| LiveVoice slimming / AgentCore downshift | Current entry is the [slimming master plan](roadmap/LIVEVOICE_SLIMMING_MASTER_PLAN.md); the preparation-branch audits are imported verbatim under `reviews/OPENJIUWEN_*` with provenance notes. Dated supporting records: [2026-09-05 activation precheck](reviews/OPENJIUWEN_LIVEVOICE_SLIMMING_ACTIVATION_PRECHECK_2026-09-05.md): feature-complete verdict, blocking conditions and the `59998e2c..ebd2b4575` stable-symbol delta, then the [2026-09-05 plan refit](reviews/OPENJIUWEN_LIVEVOICE_SLIMMING_PLAN_REFIT_2026-09-05.md) for the demo-freeze premise and the [2026-09-05 current-branch analysis and plan](reviews/OPENJIUWEN_LIVEVOICE_CURRENT_BRANCH_ANALYSIS_AND_PLAN_2026-09-05.md) (new-code analysis by responsibility group, updated AgentCore F1–F6 contracts, target layout and packages A–D, decisions still owed) and the [2026-09-05 removability verification](reviews/OPENJIUWEN_LIVEVOICE_REMOVABILITY_VERIFICATION_2026-09-05.md) (why the 52 `CONSOLIDATE_RETIRE` rows are removable, per-row verification on the current tip, what is removable now versus after each gate) and the [2026-09-05 slimming-thesis verification](reviews/OPENJIUWEN_LIVEVOICE_SLIMMING_THESIS_VERIFICATION_2026-09-05.md) (module-by-module measured size, Hermes and target comparison, which reduction mechanisms are confirmed and which remain hypotheses); the preparation audits stay on `codex/livevoice-agentcore-hermes-prep@b9dc8a5c` (remote `agtai`) and are not merged |
