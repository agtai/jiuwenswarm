...
## 7c. 缺陷 11（Alpha 引入，**已定位未修**）：合成超时是整条流的固定 15 秒预算

- **来源**：缺陷 10 修复后，用户在真实设备上重听，症状依旧：文字回答完整正确
  （三小节），但朗读在「采集、降噪」处终止，另有**撕裂感与点流感**。
- **取证**：失败原因已经换层，说明缺陷 10 的修复生效：

| 版本 | 降级原因 | 触发时刻 |
|---|---|---:|
| 缺陷 10 修复前 | `STREAMING_SPEECH_EVENT_QUEUE_EXHAUSTED` | 5,859 ms / 4,766 ms |
| 缺陷 10 修复后 | `STREAMING_SPEECH_PROVIDER_TIMEOUT` | **恰好 15,000 ms** |

  两次都是 `operation=synthesis.stream`、`first_audio_emitted=true`，
  随后 `live_voice_streaming_synthesis_fallback ... action=text_or_retry`。
  15,000 ms 无抖动，说明是**客户端声明的预算**而非内部竞态。
- **根因**：`gatewayBatchSpeechClient.ts` 的 `DEFAULT_TIMEOUT_MS = 15_000` 作为
  `timeout_ms` 发给 `live_voice.speech.synthesize_batch`，而流式合成把它当作
  **整条流**的预算。一段三小节的真实回答需要约 40 秒朗读，因此**任何超过 15 秒
  语音时长的回答都会被设计性地截断**。流式合成的预算必须是**逐事件**的
  （下一块音频的到达时限），而不是整条流的总时长。
- **归属**：`gatewayBatchSpeechClient.ts` 与流式合成路由都不在 `3f3cdbb7f`
  与 `2a69c2b87` 中，由本批 S6 引入。**Alpha 引入**。
- **状态**：**已定位，本批次未修**。它跨越前端契约（客户端声明的 `timeout_ms`）
  与 Adapter 对该字段的解释，属于契约级改动；在没有余量完成「改动 + 回归 +
  真实路径回退验证」的情况下不动它，避免留下未验证的半成品。
- **下一步（精确）**：把流式合成的时间预算从「整条流总时长」改为「到下一块音频的
  时限」，并让批量路径保留原语义；改完必须用 `s6_02_realtime_playout.py` 的实时
  节奏做一次回退验证，再请用户重听。

### 7c-2 未解释的独立症状：撕裂感 / 点流感

用户第三次观察把这两件事彻底分开了：该次 Agent 回答很短，**音频完整推进没有截断**，
但**撕裂感与电流感依旧存在**。因此：

- 截断与**回答长度相关** → 由缺陷 11 的 15 秒整流预算解释；
- 撕裂/电流感与**长度无关**，在短回答上稳定复现 → 是一个**独立缺陷候选（12）**。

这也排除了一个嫌疑：`_StreamingLinearResampler` 是有状态的，其 docstring 与实现
（全局 `_total_input` / `_next_output` 游标 + 残留缓冲）保证**分块不改变输出**，
所以块边界不会因重采样产生不连续。撕裂不来自重采样。

原始记录：用户在两次观察中都报告了播放的**撕裂与点流**，这与「被截断」是**两个不同**的症状：
截断是流被杀，撕裂是播放连续性问题。最可能的嫌疑是在途音频过浅——
网关合成路由的 `_DEFAULT_MAX_PENDING_FRAMES = 8`，即只有 **160 ms** 的在途缓冲，
任何调度或 RTT 抖动都会让 AudioWorklet 欠载并发出咔哒声；线性插值上采样
（24 kHz → 48 kHz）也可能贡献主观音质。**本批次没有测量它**，不作判定。
它需要自己的证据：至少一次带欠载计数的浏览器侧测量（AudioWorklet 欠载次数、
帧到达间隔分布、在途帧数水位），不能用服务端帧数替代——服务端在实时节奏下
已实测 930 帧无降级，却仍听得到电流感，正说明服务端计数不能证明听感。
