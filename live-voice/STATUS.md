...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from production Web. LVL-09 now has same-workload target materiality: schedule-to-start changed from 718.549/721.306 ms controls to 48.432 ms at 250 ms, exposing **670–673 ms** headroom with 2.757 ms control drift. A2 cancelled after the target mark, so physical/default acceptance remains open. LVL-10 remains `INCONCLUSIVE`; LVL-10L retains directional long-form completion gain but stopped before formal evidence | Preserve the 1000 ms WebAudio default until completed playout/reliability evidence closes LVL-09. Run the already-implemented LVL-08 Provider pilot next. In parallel, implement only the missing Agent first-delta producers and prepare a distinct long-form pre-final stable-prefix screen. See the [LVL-09 result](evidence/LVL09_ADAPTIVE_PLAYOUT_LEAD_PILOT_RESULT_2026-08-25.md), [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md), [LVL-10L result](evidence/LVL10L_LONG_FORM_CHUNKED_TTS_RESULT_2026-08-24.md) and [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
1. **Run implemented experiments.** The first LVL-09 same-workload pilot
   ...
   and a default change remain open. LVL-08 Semantic VAD
