...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from production Web. LVL-09 exposes **670–673 ms** same-workload schedule-to-start headroom but retains incomplete physical/default acceptance. LVL-08's protocol repair now completes the full real 12-slot AUTO pilot. AUTO saves **684–734 ms** of EOT on no-pause/300 ms cases but produces `EARLY_EOT` at 600/1000 ms, so it is integrity-rejected; HIGH remains intentionally unrun. The isolated Agent first-visible-delta producer packet passes Tier-2 source/test review but has no timed population or integration credit. LVL-10 remains `INCONCLUSIVE`; LVL-10L remains directional and stopped | Preserve the 1000 ms WebAudio and 1200 ms Server-VAD defaults. Keep Semantic AUTO rejected and do not run HIGH; any future VAD work requires a new continuation-safe arbitration hypothesis. Measure the isolated Agent first-delta packet before specifying connection/warm-up/prompt changes, and keep long-form pre-final stable-prefix work separate. See the [LVL-09 result](evidence/LVL09_ADAPTIVE_PLAYOUT_LEAD_PILOT_RESULT_2026-08-25.md), [LVL-08 retry result](evidence/LVL08_SEMANTIC_VAD_AUTO_RETRY_RESULT_2026-08-25.md), [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md) and [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
1. **Run implemented experiments.** The first LVL-09 same-workload pilot
   ...
   was not run.
2. **Prepare missing measurement packets.** Isolated Agent lane commits
   `3b85425ac` and `4efd2b6c0` connect the already-catalogued
   `agent.agent_started` and `agent.agent_first_delta` producers and pass
   independent Tier-2 re-review `C0/I0/M0`, without changing request or model
   behaviour. They remain unintegrated and unmeasured. Queue, connection,
