...
- **Current product status:** **P3 EXPANSION ACTIVE — P3-G0 passed its scoped
  ...
  latency, generation-time interruption, broader generalization and cumulative
  product acceptance remain open. The later mounted Exit/immediate-re-enable
...
| Speech Recognition | **PARTIAL.** Real microphone finals now sustain repeated automatic turns on `e1df8b452`. Ordinary and barge-in capture both retain the 1.2-second server-VAD silence contract; one EOT-to-submit interval was 0.669 seconds. The optional `openai-realtime` Adapter fences its data plane until strict negotiation. Current product/test source `750d5529620bc11b4fc3f8d5eac5bb1fe91c2b0c` retains the bounded settlement semantics accepted on `87b57a69` and makes `gpt-realtime-2.1-mini` the default while preserving explicit family-member override; the settlement boundary's final targeted independent review passes `C0/I0/M0` on exact docs HEAD `688ae114942ecd26bcfff8b37effe7ebb59998d9` | Shortest real OpenAI Realtime server-to-server recognition probe against `gpt-realtime-2.1-mini`, then fixed-corpus ordinary/barge-in p50/p95 and device validation remain open | Audio I/O, media route and benchmark owner; [native Adapter review](reviews/OPENAI_REALTIME_NATIVE_SPEECH_ADAPTER_REVIEW_2026-08-23.md); source/automation/review PASS grants no physical credit |
| Speech Synthesis | **PARTIAL.** Streaming/Batch TTS, browser playout, response ownership and ACK paths exist; repeated short/long TTS was physically audible on `e1df8b452` without the repaired ACK/receipt failures. The optional `openai-realtime` Adapter strictly requires complete initial and terminal response resources from one shared ten-field contract, buffers transcript-gated PCM, and retains the sole receive owner through successful close-handshake EOF before audio publication. Current product/test source `750d5529` retains the accepted finalization owner from `87b57a69` and selects `gpt-realtime-2.1-mini` when no explicit model override is supplied; the final targeted shared-helper regression passes within the exact `688ae114` independent review | Shortest real Realtime synthesis probe against `gpt-realtime-2.1-mini`, then audible/device evidence and complete physical stale/cancel recovery remain open | Conversation Runtime ownership and Audio I/O stop confirmation; [native Adapter review](reviews/OPENAI_REALTIME_NATIVE_SPEECH_ADAPTER_REVIEW_2026-08-23.md); source/automation/review PASS grants no streaming first-audio or physical credit |
| Realtime Media | **PARTIAL.** Dedicated transport, capture rotation, media registration and presentation ACK sustain the accepted physical loop. The bounded P2 notification pull removes the diagnosed one-notification-per-RPC tail from production Web: Web requests `16`, the server accepts explicit `2..16`, and omitted input remains legacy single-pull compatibility. Gateway-only OpenAI Realtime WebSocket negotiation now has bounded source/automation coverage | Backpressure/load targets; drop/reorder/corruption/reconnect matrix; stable diagnostics across repeated recovery; native Adapter real network/device probe and fixed-corpus p50/p95 proof | Audio I/O plus Conversation Runtime; [D-094](decisions/DECISIONS.md), [D-095](decisions/DECISIONS.md), [D-096](decisions/DECISIONS.md) and cumulative real network/device verification |
...
| Automated verification and product acceptance | **PARTIAL overall; four lifecycle module Gates, the older playout/capture decoupling seam and the bounded P2 notification integration follow-up PASS independent Tier-3 review.** The Native Realtime settlement source/automation/review Gate passes on `87b57a69` / `688ae114` at **`C0/I0/M0`**. The later Tier-1 default-model source `750d5529` retains that product/test tree except for the model constant and direct default/override oracles; its Provider file passes `160/160`, its production-factory focus passes, and static checks pass | Begin the separately scoped real `gpt-realtime-2.1-mini` Provider/network probe; real device, audibility, Provider degradation and fixed-corpus latency retain no physical credit | [D-096](decisions/DECISIONS.md), [P2 evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and [native Adapter review](reviews/OPENAI_REALTIME_NATIVE_SPEECH_ADAPTER_REVIEW_2026-08-23.md); scoped evidence grants no P3-9, physical or product-readiness credit |
...
### OpenAI Realtime native Speech Adapter (SOURCE/AUTOMATION/INDEPENDENT REVIEW PASS; REAL PROVIDER PROBE NEXT, 2026-08-24)

- **Capability/module:** Speech Recognition, Speech Synthesis, Realtime Media
  and Conversation Runtime Provider edge. This packet implements the optional
  Native Audio Engine direction accepted by D-039 without replacing the
  JiuwenSwarm Agent/Tool control plane.
- **Intended behaviour:** with the existing formal streaming flag enabled and
  `LIVE_VOICE_SPEECH_PROVIDER=openai-realtime`, Gateway alone opens official
  OpenAI Realtime server WebSockets. A configurable `gpt-realtime` family
  model owns native audio sessions; the user-selected default is
  `gpt-realtime-2.1-mini`, while an explicit
  `LIVE_VOICE_SPEECH_REALTIME_MODEL` may select another compatible member.
  Its optional input transcription remains the only text eligible for the
  existing committed-input path. Authoritative
  JiuwenSwarm Agent text is then rendered by an out-of-band Realtime audio
  response bound to the exact response/generation/unit identity. Because model
  instructions are not an authority guarantee, Provider audio remains buffered
  until its completed output transcript exactly equals the authoritative Agent
  `spoken_text`; a mismatch releases zero audio. Automatic Provider responses
  and Provider tools stay disabled.
- **Owned product/test surfaces:**
  `server/live_voice/openai_streaming_speech.py`, its focused Provider tests,
  existing Gateway streaming recognition/synthesis integration regressions,
  environment selection/configuration diagnostics, and this synchronized
  decision/status boundary. Existing Media, Runtime, Agent Bridge and Task
  authorities are consumers, not new implementation owners.
- **Risk:** Tier 3 under root `TESTING.md`. Provider events cross audio,
  recognition-final, response/generation, cancel/fence and degradation
  boundaries. Acceptance therefore covers the complete applicable
  `P/N/B/S/T/C/R/I/F/K/X` matrix, including zero Agent/Tool/Task/history effects
  for partial, stale, malformed, mismatched and failed Provider paths.
- **Acceptance:** focused Provider protocol/conformance tests, affected Gateway
  recognition/synthesis and Web-factory regressions, static/diff checks and an
  independent module-boundary review must pass. A real API/network/device run
  is required before any physical Provider or user-perceived quality credit;
  absence of credentials leaves that evidence explicitly unclaimed rather
  than weakening automated closure.
- **Current result:** exact source `774f6ae7025990c7418a69e44b9f2cd38347ed4b`
  passed its recorded automation, but the subsequent independent Tier-3 review
  returned **`C0/I3/M1`** and revoked its earlier PASS. The bounded repair now
  adds required server `event_id` replay fencing, an explicit native response
  lifecycle and no-progress deadline, strict native `create_response=false` /
  `interrupt_response=false` effective-session authority, a shared
  voice-purpose model predicate, production Web-factory composition, and exact
  per-delta/aggregate audio-limit oracles. Focused Provider tests pass
  `107/107` with `88%` target-module coverage; affected speech/Gateway tests
  pass `171/171` with one independently reproduced inherited baseline failure
  deselected; full Web-factory tests pass `80/80`; Ruff, format, targeted mypy,
  compile and diff checks pass. Agent/Tool/Task/history and the existing
  `openai` route semantics remain outside the repair. This is
  **repair source/automation evidence only**. The subsequent detached review of
  exact repair source `f83338649415b567964d8e1328fe3dc6a32ac8b5`
  returned **`C0/I3/M0`**: terminal response events can remain uninspected after
  buffered PCM release; recognition data-plane calls can race ahead of strict
  `session.updated` validation; and `response.created` does not yet bind the
  full initial response resource. That independent result revokes the repair
  PASS and activates a second bounded repair covering terminal quarantine,
  negotiated-recognition publication/fencing and initial response validation,
  plus their exact positive/negative/concurrency oracles. A further independent
  review is required, and no real credential/network/device validation may
  start before it passes. The second repair now fences recognition audio,
  commit and output until strict effective-session validation; rejects Provider
  output before negotiation; binds the complete initial in-progress audio
  response; and holds exact PCM through a bounded terminal quarantine and
  successful socket close. Queued late response events release zero audio,
  while post-terminal `rate_limits.updated` remains legal without renewing a
  deadline. Focused Provider tests pass `116/116` at `89%` target-module
  coverage; affected speech/Gateway tests pass `171/171` with the same inherited
  baseline failure deselected; Web-factory tests pass `80/80`; Ruff, format,
  isolated targeted mypy, compile and diff checks pass. These are implementer
  results only. The third detached review of exact source
  `21d3de2a4d05c5885f6c482856ec0f03bd835370` returned **`C0/I4/M0`**:
  a response-scoped frame received during the close handshake can escape the
  fixed terminal quarantine; `cancel_recognition` does not settle an opening
  session's `ready` future; terminal `response.done` accepts contradictory
  object/status-details/usage facts; and native recognition has no bounded
  server-event-ID replay ledger. That result revokes the second repair's
  source/automation disposition and activates a third bounded repair limited to
  receive/close settlement, opening-cancel settlement, complete terminal
  response validation and recognition event identity, with exact concurrency,
  boundary and zero-effect oracles. A fourth independent review is required
  before any real Provider/device validation.
  The third repair now keeps a sole terminal receive owner active through the
  close handshake and requires transport EOF before PCM publication; settles
  opening-recognition `ready` after cancel cleanup without a second timeout
  fact; validates terminal response object, status-details and internally
  consistent usage; and requires unique bounded native recognition event IDs.
  Focused Provider tests pass `128/128` at `88%` target-module coverage
  (`1986` statements, `232` missed); affected speech/Gateway tests pass
  `171/171` with the inherited baseline failure deselected; production Web
  composition tests pass `80/80`; Ruff, format, isolated targeted mypy,
  compile and diff checks pass. Those were implementer-run source/automation
  results only; at that point the latest independent disposition remained the
  third review's `C0/I4/M0` and a fourth detached review was required before any
  real Provider/device validation.
  The fourth detached review of exact source
  `e3664e798fe9901c3bbdd69a140f51bca6542e4a` returned **`C0/I2/M0`**. It
  closed the prior close-handshake, terminal-resource and recognition-event-ID
  findings, but proved that close-induced recognition EOF can still settle an
  opening session's `ready` before the cancel owner and that the initial
  response validator accepts omitted `status_details` or `usage` as explicit
  null. The fourth repair now checks `closing` before any receive-worker ready
  mutation; serializes concurrent cancel; finalizes transport, worker,
  conformance and registry exactly once before releasing the opening waiter;
  preserves process-control only after finalization; and requires both initial
  nullable keys to be present. Focused Provider tests pass `133/133` at `87%`
  target-module coverage (`2041` statements, `257` missed); affected speech/
  Gateway tests pass `171/171` with the inherited baseline failure deselected;
  production Web composition tests pass `80/80`; Ruff, format, isolated
  targeted mypy, compile and diff checks pass. These are implementer-run
  source/automation results only. At that point the latest independent
  disposition remained `C0/I2/M0`.
  Consolidated source `1d4f067cf697c4773b3ec7f0cfba307a9238594e` then
  replaces the serial repair structure with one per-session finalization task.
  Cancel, Provider failure, failed-open rollback and Provider close now share
  the same close/worker/conformance/registry/ready settlement; concurrent
  callers await that task through `shield`, so cancelling one waiter cannot
  cancel the lifecycle owner. Receive-owned failures preserve transport close
  as the post-observability completion barrier, while external close owners
  close first to wake receive. Initial and terminal native response validators
  also share one ten-field presence contract, and both run every required-field
  deletion as a table-driven negative oracle. The consolidation reduces the
  target module from `2041` to `2029` statements, reduces direct recognition
  conformance-close call sites from six to two, and reduces
  `cancel_recognition` from about 95 lines to a small delegating entry point.
  Focused Provider tests pass `150/150` at `88%` target-module coverage (`2029`
  statements, `247` missed); affected speech/Gateway tests pass `171/171` with
  the inherited baseline failure deselected; production Web composition tests
  pass `80/80`; Ruff, format, isolated targeted mypy, compile and diff checks
  pass. These remained implementer-run source/automation results and advanced
  to one detached complete consolidated-module review before any real
  Provider/device validation.
  That detached review inspected exact clean source
  `cdb88eb0ae81a32aadb58c0aaf92e838e21206b1` and returned
  **`C0/I3/M1`**. It confirmed all historical findings closed, but reproduced
  four lifecycle interleavings: a valid negotiation frame queued before close
  could settle opening after cancel; normal recognition final bypassed the
  terminal owner and could lose an accepted FINAL to concurrent cancel;
  synthesis terminal cleanup could be interrupted by a cancelled waiter or
  observability process-control; and an opening receive-owned process-control
  escaped through both the public open call and its private worker.
  Lifecycle-convergence source
  `2698bd9b811f3fe6a710cbbb8c132dd4a9ed2861` fixes the two shared root causes.
  Normal recognition final and normal synthesis completion now install the
  same shielded first-owner task used by cancel, Provider failure, rollback and
  service close. Recognition fences a frame returned after closing and keeps
  `open_recognition` as the sole opening process-control surface. Synthesis
  finalization settles transport, worker, conformance, registry, buffered PCM
  and degradation truth before returning stored failure. Exact coordinated
  race tests cover all four findings and retain zero Agent/Tool/Task/history
  effects. Focused Provider tests pass `155/155` at `87%` target-module
  coverage (`2098` statements, `273` missed); affected speech/Gateway tests
  pass `171/171` with the inherited baseline failure deselected; production Web
  composition tests pass `80/80`; Ruff, format, targeted mypy, compile and diff
  checks pass. These are implementer source/automation results only.
  The required detached complete-module review then inspected exact clean HEAD
  `6224f8e27fa1ba4508f08e4820c4871ba162c8a2` and returned
  **`C0/I1/M0`**. It closed every prior interleaving but proved one remaining
  shared settlement defect: `_close_socket()` / `_close_stream()` returning
  `False` was treated as success, so normal recognition could publish FINAL
  with an open transport and close-side process-control could escape cancel
  before retry and the required degradation fact. This is one root cause, not
  a new module-review cycle.
  Transport-settlement source
  `6aed58f5bce5fdfed3bc2920937af377ebafddc3` preserves the typed boolean
  cleanup result, retries close within the installed session finalizer, records
  `SPEECH_PROVIDER_CLEANUP_INCOMPLETE` when bounded settlement cannot finish,
  and emits required cancellation/failure truth before re-exposing stored
  process-control. Exact deterministic oracles cover blocked normal-final close
  plus recognition and synthesis close-side `GeneratorExit`; the latter close
  on their second attempt, retain one fact and then rethrow. Focused Provider
  tests pass `158/158` at `87%` target-module coverage (`2111` statements,
  `275` missed); affected speech/Gateway tests pass `171/171` with the inherited
  baseline failure deselected; production Web composition tests pass `80/80`;
  Ruff, format, targeted mypy, compile and diff checks pass. These remain
  implementer source/automation results. Per root `TESTING.md`, only one
  targeted independent confirmation of the I1 repair and its materially
  affected paths remained before real Provider/device validation; it was not a
  request to repeat the already completed full module review.
  That detached targeted review inspected exact clean HEAD
  `e6663dfa8c6e0fcac88b91ee3fcd1be2f6d45aef` and returned
  **`C0/I1/M0`**. It confirmed every session-backed settlement path, but found
  one omitted entry: recognition could allocate a socket and then fail before
  constructing its session, where rollback still called `_close_socket()` only
  once. A first `False` therefore hid incomplete cleanup, while a first
  `GeneratorExit` escaped before retry, conformance reaping and the open-failure
  fact.
  Product/test source `87b57a69cdb0ffd496468092463ebcf926fb6a10`
  passes the same `_FinalizationFailures` owner into failed-open rollback and
  uses the existing two-attempt socket settlement before conformance/fact and
  failure-priority release. It creates neither a session nor another
  finalization task. Two exact event-driven oracles cover incomplete close and
  one-shot close process-control. The focused Provider file passes `160/160` at
  `87%` target-module coverage (`2124` statements, `276` missed); affected
  speech/Gateway tests pass `171/171` with the unchanged inherited failure
  deselected; Ruff, format, targeted mypy, compile and diff checks pass. Web
  factory composition was not rerun because this recognition-only pre-session
  rollback neither changes configuration nor reaches that factory seam.
  The final detached targeted review then inspected exact clean docs HEAD
  `688ae114942ecd26bcfff8b37effe7ebb59998d9` and returned
  **PASS — `C0/I0/M0`**. Its six exact cases confirm incomplete-close truth,
  two-attempt process-control ordering, zero active/retained identity, one
  open-failure fact, session-helper regression and zero forbidden business
  effects. This closes the source/automation/independent-review Gate; no
  seventh complete source review is required. A separately scoped shortest
  real Provider/network probe is now the next trigger and grants no physical or
  quality credit until it actually runs.
  User-directed Tier-1 configuration source
  `750d5529620bc11b4fc3f8d5eac5bb1fe91c2b0c` subsequently changes only the
  Native Adapter default to `gpt-realtime-2.1-mini` plus its direct default,
  effective-URL and explicit-override oracles. The Provider file passes
  `160/160`; the production Web-factory focus, Ruff, format, isolated mypy,
  compile and diff checks pass. D-096 records the choice. This bounded default
  change neither reopens the accepted settlement review nor grants real
  Provider credit; the next probe now targets `gpt-realtime-2.1-mini`.
- **Excluded:** browser-held API keys or direct browser-to-OpenAI transport;
  provider-native tool registration/calls; OpenAI-generated product answers;
  bypass of committed final, confirmation, Runtime identity, Agent Bridge,
  Voice–Task Bridge or presentation ACK; migration of the current `openai`
  streaming cascade; default-on rollout; continuous single-session model-level
  duplex/barge-in during Agent generation; streaming first-audio latency credit
  for the transcript-gated output buffer; billing/account/key changes, physical
  acceptance, public deployment and remote-ref updates.
...
- **Latency:** the code-fact diagnosis and implementation approach are now
  ...
  and default-on under D-094. Next evaluate VAD finalization, first-audio startup
