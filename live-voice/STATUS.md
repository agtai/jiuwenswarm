...
| Observability, benchmark and latency | **PARTIAL.** Trace/correlation and historical verification foundations exist. The minimal probe and controlled-WAV Browser runner remain available for later physical validation. The accepted no-Browser P2 bounded-pull component result reduced p50 by 90.0–92.9%. A separate independently reviewed no-Browser real-Provider VAD A1/E1/E2/A2 screen at `e2773bec2` completed 80 attempts with clean pacing/cleanup and zero forbidden effects; 900/800 ms each failed turn integrity 5/20, so 1200 ms remains the fixed default. Neither result grants physical Browser or end-to-end product credit | Follow the [reconciled non-Agent priority](roadmap/NON_AGENT_P1_P2_P3_LATENCY_OPTIMIZATION_BRAINSTORM_2026-08-21.md): first compare/port Hongxing's divergent TTS/capture contracts and establish no-Chrome TTS first-audio A1; run EOT overlap only behind its materiality gate; physical Browser confirmation remains required before product-path claims | Accepted P2 notification transport candidate; rejected lower fixed VAD thresholds; next TTS/capture reconciliation and causal benchmark owner |
