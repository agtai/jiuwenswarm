...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from production Web. LVL-09 exposes **670–673 ms** same-workload schedule-to-start headroom but retains incomplete physical/default acceptance. LVL-08 AUTO is integrity-rejected after early EOT at 600/1000 ms. LVL-11 measures real-Agent first-delta timing. LVL-12 passes exact-prefix materiality, and LVL-13 then accepts its no-Browser real-Agent/real-TTS component candidate: Agent→first-PCM p50 improves by **1.723 s / 52.51% medium** and **3.422 s / 67.38% long**, with valid A1/A2 drift, 30/30 completion and zero forbidden effects. This grants no pre-final product speech, Browser, physical first-audible or `PresentationUnit` authority. LVL-10 remains `INCONCLUSIVE`; LVL-10L remains directional and stopped | Preserve the 1000 ms WebAudio and 1200 ms Server-VAD defaults. Keep Semantic AUTO rejected. Decompose Agent queue/request/connection/network/model first-token timing independently. LVL-13 now requires a separate product-authority design before any Runtime/P2/Browser wiring, followed by a deployed physical Gate-C A/B/A if that design is accepted. Keep LVL-13 distinct from post-final LVL-10/LVL-10L. See the [LVL-13 result](evidence/LVL13_PRE_FINAL_STABLE_AGENT_TTS_RESULT_2026-08-25.md), [LVL-12 result](evidence/LVL12_PRE_FINAL_STABLE_SEGMENTATION_RESULT_2026-08-25.md), [Agent first-delta baseline](evidence/AGENT_FIRST_DELTA_BASELINE_2026-08-25.md), [LVL-09 result](evidence/LVL09_ADAPTIVE_PLAYOUT_LEAD_PILOT_RESULT_2026-08-25.md), [LVL-08 retry](evidence/LVL08_SEMANTIC_VAD_AUTO_RETRY_RESULT_2026-08-25.md), [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md) and [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
1. **Run implemented experiments.** The first LVL-09 same-workload pilot
   measured schedule-to-start at 718.549/48.432/721.306 ms and exposed
   approximately 670–673 ms of target headroom with only 2.757 ms control drift.
   A1/B completed, while A2 cancelled after its target mark; physical acceptance
   and a default change remain open. LVL-08 protocol commits `3817aaaea` and
   `222582618` pass their Tier-3 re-reviews and a clean real retry completed all
   12 A1/B/A2 slots. AUTO saves 684–734 ms of EOT where it completes, but the
   600/1000 ms pause cases fail `EARLY_EOT`; AUTO is integrity-rejected and HIGH
   was not run.
2. **Use the completed Agent diagnostic to prepare the next packets.** Isolated
   Agent lane commit `f71a5d830` connects the catalogued
   `agent.agent_started` and `agent.agent_first_delta` producers to a private
   real-Agent runner. Its 15/15 population measures first-delta p50 at
   523–592 ms and delta→final p50 at approximately 9 ms / 1.68 s / 3.52 s for
   short/medium/long output, without changing request or model behaviour.
   Queue, request preparation, connection/network and model first-token splits
   remain unknown. LVL-12 now passes a separate 10/10 medium/long exact-prefix
   screen with candidate→final p50 1.587/3.573 s. Short remains stopped. The
   LVL-13 completes that no-Browser real Agent→TTS screen 30/30. Against
   interpolated A1/A2 controls, its digital Agent→first-PCM p50 improves by
   1723.166 ms / 52.510% medium and 3422.345 ms / 67.377% long, with valid
   control drift and zero forbidden effects. The next gate is a product-
   authority design; no provisional product `PresentationUnit`, Browser or
   physical first-audible credit is authorized. This pre-final question
   remains separate from LVL-10/LVL-10L post-final chunked synthesis.
...
accepted feature-on human validation (audible TTS; the earlier
`SPEECH_OPERATION_NOT_AUTHORIZED` failure did not recur), D-094 batch-16
default-on with deployment-switch retirement, and the Successor-ACK/TTS
Tier-3 review. The prior latency-lane P2 bounded-pull candidate stack is
superseded by that accepted implementation: this composition resolves every
conflicted P2 product/test file in favour of `c31e85ade`. The latency catalog,
...
The Provider-only LVL-10 materiality screen is complete at its declared phase-1
boundary. Two formal real-Provider populations completed 45/45 with zero
Provider errors, but separate A1/A2 reserve drift violations made both runs
`INCONCLUSIVE`. Long completion repeatedly improved 20–24% and medium
completion regressed, which motivated the separately stopped LVL-10L screen.
No product wiring or Browser Lane C is authorized; the current
full-final SSE route remains unchanged. LVL-08 returns as the next existing
no-Browser screen. See the [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md).
This composition grants no new physical, Gate C or product-readiness credit
beyond the recorded D-094 acceptance.

LVL-10L separately froze completion as primary and compared one, two and four
post-final TTS requests. Its v2 600/1200/2100 corpus passed a 12/12 real-
Provider pilot with zero errors; B2/B4 improved 2100-character completion by
derived 6.93/7.75 seconds. This reproduces the earlier long-form direction but
is one round, not a formal population. Hongxing requested that long-duration
testing stop while the approved five-round formal was running; only its run and
manifest survive, so it receives zero timing credit. Product source remains
unchanged. See the [LVL-10L result](evidence/LVL10L_LONG_FORM_CHUNKED_TTS_RESULT_2026-08-24.md).
