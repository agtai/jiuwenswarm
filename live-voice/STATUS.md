...
| Observability, benchmark and latency | **PARTIAL.** Trace/correlation and historical verification foundations exist. The minimal probe and controlled-WAV Browser runner remain available for later physical validation. Accepted no-Browser component results cover P2 bounded pull (90.0–92.9% p50 reduction) and reviewed TTS successor-ACK decoupling: at 250/750 ms injected ACK delay, first-source p50 changed from 255.597/756.542 ms to 0.486/0.476 ms. The real-Provider VAD screen rejected fixed 900/800 ms, and the exact-runner TTS connection experiment rejected application-client reuse after 0/3 B warm connections reused and p50 regressed 7.0%. None grants physical Browser or end-to-end product credit | Re-rank the [reconciled non-Agent priority](roadmap/NON_AGENT_P1_P2_P3_LATENCY_OPTIMIZATION_BRAINSTORM_2026-08-21.md). Post-`audio.done` EOF draining requires a new bounded spec if pursued; EOT overlap remains conditional behind materiality; physical first-audible confirmation remains open. TTS receipt settlement still waits for successor readiness and is not credited as improved | Accepted P2 notification and TTS first-audio component candidates; rejected lower fixed VAD thresholds and application-level TTS client reuse; [connection result](roadmap/TTS_PROVIDER_CONNECTION_REUSE_RESULT_2026-08-21.md) |
