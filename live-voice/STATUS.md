...
| Speech Synthesis | **PARTIAL.** Streaming/Batch TTS, browser playout, response ownership and ACK paths exist. The reviewed no-Chrome TTS first-audio candidate at `cd4d1b7d3` opens downlink independently of successor-capture ACK and preserves truthful non-duplex receipts | Provider-neutral configuration, Provider connection prewarm/reuse, physical first-audible/underrun/pronunciation targets and complete stale/cancel recovery; authoritative settlement still waits for bounded successor readiness | Conversation Runtime ownership and Audio I/O stop confirmation; [TTS first-audio causal result](evidence/TTS_FIRST_AUDIO_CAUSAL_RESULT_2026-08-21.md) |
...
| Observability, benchmark and latency | **PARTIAL.** Trace/correlation and historical verification foundations exist. The minimal probe and controlled-WAV Browser runner remain available for later physical validation. Accepted no-Browser component results now cover P2 bounded pull (90.0–92.9% p50 reduction) and reviewed TTS successor-ACK decoupling: at 250/750 ms injected ACK delay, first-source p50 changed from 255.597/756.542 ms to 0.486/0.476 ms. The real-Provider VAD screen rejected fixed 900/800 ms after each preserved only 15/20 turns, so 1200 ms remains default. None grants physical Browser or end-to-end product credit | Follow the [reconciled non-Agent priority](roadmap/NON_AGENT_P1_P2_P3_LATENCY_OPTIMIZATION_BRAINSTORM_2026-08-21.md): evaluate TTS Provider connection prewarm/reuse next; EOT overlap remains conditional behind its materiality gate; physical first-audible confirmation remains open. TTS receipt settlement still waits for successor readiness and is not credited as improved | Accepted P2 notification and TTS first-audio component candidates; rejected lower fixed VAD thresholds; next TTS prewarm causal owner |
