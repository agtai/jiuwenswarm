...
- **Failed line-ending deployment retained:** validation label `1bc2535d` uses project
  ...
  clean Git status and zero remotes. Controlled preflight, Live Voice build
  and launch passed on clean source, but its first two real Tasks both became
  `terminal/failed` with `PROJECT_WORKTREE_BASELINE_MISMATCH`; neither result
  nor requested file exists. The machine's inherited `core.autocrlf=true`
  retained LF bytes in the just-created main checkout but would materialize
...
- **Fresh isolated redeployment:** validation label `e303b28f`
  ...
  and data root
  `C:\Users\admin\AppData\Local\Temp\jiuwenswarm-realtime-p3-data-e303b28fb4364d4ab3169321754eb9e3`.
  The project pins repository-local `core.autocrlf=false`, contains only
