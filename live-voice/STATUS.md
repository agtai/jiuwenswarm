...
- **Current streaming-final request-budget repair:** **SOURCE/AUTOMATED PASS;
  PHYSICAL RERUN AND INDEPENDENT TIER-3 REVIEW OPEN.** Initial and successor
  Formal P1 Speech adapters plus the panel now preserve the Speech client's
  existing 38-second timeout/abort options through WebClient. This prevents the
  unrelated 15-second default from terminating a valid Gateway streaming-final/
...
### P1 streaming-final request-budget forwarding

- **Packet/status:** **SOURCE/AUTOMATED PASS; PHYSICAL RERUN AND INDEPENDENT
  TIER-3 REVIEW OPEN.** The implementation is the commit containing this
  STATUS, based on the preceding same-tab Task re-entry repair.
- **Owner/risk:** Formal P1 initial/successor Speech request adapters and the
  panel request boundary; Tier 3 because premature local failure can discard a
  valid committed utterance/fallback result.
- **Root cause:** the Speech client declared 38 seconds for Gateway's bounded
  36-second streaming-result wait, but three adapters discarded the request
  options and WebClient applied its 15-second default.
- **Implemented invariant:** initial and successor P1 adapters forward the exact
  timeout/signal object, and the panel passes the exact request function. P1
  does not own or redefine the timeout value.
- **Automated result:** focused forwarding passes `4/4`; complete Formal Web
  passes `453/453`.
- **Excluded/remaining:** no timeout policy, Provider fallback, P2 authority,
