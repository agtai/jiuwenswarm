...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from the production Web path; validation-branch manual samples were `10.65s`, `7.05s`, and `2.78s / 3.14s / 3.14s` with audible TTS. A current-source muted pilot completed two dialogue rounds and cancelled four Task batches; its partial cross-profile medians were `587.6ms` EOT→STT final, `1518.3ms` TTS request→first downlink and `728.0ms` schedule→first-start. LVL-10 later completed two real-Provider 45/45 populations with zero errors but remains `INCONCLUSIVE` because separate 426.8 ms / 321.7 ms A1/A2 drift violations invalidated both decisions. Long completion improved 20–24% while medium regressed; no Browser/product credit follows | Preserve LVL-10 as inconclusive and the one-request product path unchanged. LVL-08 Semantic VAD is the next already-specified no-Browser screen; a conditional long-form completion candidate requires a new prospective LVL-10L contract. Repair the manual-driver Gate before LVL-09 or another credited Browser population. See the [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md), [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) and [catalog](evidence/LATENCY_EXPERIMENT_CATALOG_2026-08-22.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
LVL-09 playout screen. Completed LVL-10 receives no product/Browser credit; a
conditional long-form successor requires a new prospective packet. Native
...
completion regressed, which supports only a new conditional long-form
hypothesis. No product wiring or Browser Lane C is authorized; the current
