...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from the production Web path; validation-branch manual samples were `10.65s`, `7.05s`, and `2.78s / 3.14s / 3.14s` with audible TTS. A current-source muted pilot completed two dialogue rounds and cancelled four Task batches; its partial cross-profile medians were `587.6ms` EOT→STT final, `1518.3ms` TTS request→first downlink and `728.0ms` schedule→first-start. These small/incompatible samples localize headroom but are not a frozen-corpus baseline, audibility proof or p50/p95 evidence. The 250 ms playout lead and post-final segmented TTS remain unaccepted screens, not product changes | Execute the activated no-Browser LVL-10 Provider materiality screen first. Repair the manual-driver terminal/profile/round Gate before another credited Browser population; LVL-08 Semantic VAD and compatible LVL-09 playout A1/B/A2 remain queued under the [LVL-10 spec](roadmap/AUTHORITATIVE_FINAL_SEGMENTED_TTS_MATERIALITY_SPEC_2026-08-24.md), [muted-pilot evidence](evidence/MANUAL_MUTED_LATENCY_PILOT_20260824_37da36e68.md), [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md), [catalog](evidence/LATENCY_EXPERIMENT_CATALOG_2026-08-22.md) and [SOTA review](reviews/REALTIME_VOICE_SOTA_LATENCY_REVIEW_2026-08-24.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
The user has activated the Provider-only LVL-10 materiality screen ahead of
LVL-08/LVL-09 implementation. Phase 1 remains validation tooling: compare the
unchanged full-final SSE A1/A2 route with manifest-bounded post-final B under
the prospective [LVL-10 specification](roadmap/AUTHORITATIVE_FINAL_SEGMENTED_TTS_MATERIALITY_SPEC_2026-08-24.md).
It excludes product wiring and Browser credit; only a real-Provider
materiality/integrity PASS may open that later packet. The two writers must use
isolated worktrees and non-overlapping file ownership, the two reviewers remain
read-only, and Main remains the sole Integration Owner.
