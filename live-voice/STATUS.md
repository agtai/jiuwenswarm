...
| Observability, benchmark and latency | **PARTIAL.** Trace/correlation and historical verification foundations exist. The default-off minimal latency probe records the Browser timeline plus Gateway/Agent same-clock drill-downs and passed its final independent Tier-3 code review. A default-off controlled-WAV Browser composition, supervised loopback runner and clean-source A/B/A comparison implement post-capture run preparation, exact result/shard gating, v1 reports and baseline-drift checks. Hongxing's source-bound physical evidence additionally identifies a P2 one-notification-per-RPC tail; no current-source causal A1, physical baseline or optimization credit exists yet | Establish the deterministic no-Browser P2 10/50/100 A1 on reviewed current source; implement one named bounded P2 transport candidate B in a separate worktree; run unchanged-source A2 and accept/revise/discard through A1/B/A2. Retain the controlled-WAV Browser route as later physical confirmation before any product-path claim, then continue the remaining [latency optimization plan](roadmap/LATENCY_OPTIMIZATION_PLAN_2026-08-18.md). Compare Hongxing's divergent P1/P2 fixes in a clean worktree rather than rebasing the active dirty branch | P2 notification transport owner first; fixed physical Browser corpus/environment and then Conversation Runtime/P1 media owners |
