...
- Current priority: prepare the real dependencies listed in the [Wave C integration review](ALPHA_WAVE_C_INTEGRATION_REVIEW_2026-08-07.md), then run the real Provider/media, P2 browser Agent/Tool, P3alpha Executor/voice, X-OBS/deployment and joint journeys on one immutable candidate. Missing external conditions remain fail-closed and do not block unrelated dependency-independent work.
- Wave A+B tested implementation is `dd637e604939cef7200d4ff5d30acf55a89d62f2`; its documentation closure is pushed as `107104bb22b9cfc705b02634a4eaf86d1d64f3bf`. Wave C tested local code is `4d1672eabce1edba205fd069369c3ffe64392605` on `codex/lv-alpha-wave-c-integration`, two commits ahead of unchanged local/upstream `hx/0803_live_voice` before documentation closure. Its only implementation delta is the reviewed bounded atomic TaskEvent authority handoff; P1, P2 and X-OBS returned `NO_CHANGE`, `BLOCKED` or `UNAVAILABLE` where real dependencies were absent. No Wave C branch or remote ref has been pushed or updated, no real acceptance Gate has run, and Replacement Ledger credit remains zero.
...
- `PARTIAL / STILL OPEN`: tested Wave C code `4d1672ea` retains default-off stock-Web P2 committed-text submission, retained output/presentation acknowledgement, exact Agent history, P3 trusted confirmation/create/cancel mutation, route recovery and bounded replay state. It adds a bounded atomic SQLite TaskEvent snapshot/cursor source with exact authority and cleanup fencing, but product registration, CR/Media/no-projection-arbiter composition, production identity, registered real Media/Provider, X-OBS backend and real browser/device/service evidence remain open.
...
- Development branch `hx/0803_live_voice` tracks `origin/hx/0803_live_voice`; both remain at pushed Wave A+B documentation commit `107104bb22b9cfc705b02634a4eaf86d1d64f3bf`. Local Wave C tested code is `4d1672eabce1edba205fd069369c3ffe64392605` on `codex/lv-alpha-wave-c-integration`, consisting of governance `f80946fb` and reviewed T3 integration `4d1672ea` above the baseline. The tested-code SHA deliberately excludes the documentation-only tip; resolve exact final HEAD and dirty state dynamically at resume. No Wave C branch or remote ref was pushed or otherwise updated.
...
- P3alpha query, text progress and bounded confirmation-backed create/cancel are centrally wired under explicit default-off Alpha flags. Package code now has an exact bounded atomic TaskEvent snapshot/cursor source, but production authorization and formal product voice remain closed because composition does not register it and CR/Media plus no-projection arbiter sequencing are absent.
...
3. Decide whether Code Agent support paths `.gitignore`, `coding_memory/`, `prompt_attachment/` and `.agent_history/` are relocated or explicitly governed; review CR/Media plus no-projection arbiter composition before registering the atomic P3 voice source.
