...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from production Web. LVL-09 now has same-workload target materiality: schedule-to-start changed from 718.549/721.306 ms controls to 48.432 ms at 250 ms, exposing **670–673 ms** headroom with 2.757 ms control drift. A2 cancelled after the target mark, so physical/default acceptance remains open. LVL-08 is blocked at its first Server-VAD control by Provider protocol/unknown cleanup; Semantic AUTO/HIGH remain untested. LVL-10 remains `INCONCLUSIVE`; LVL-10L retains directional long-form completion gain but stopped before formal evidence | Preserve the 1000 ms WebAudio and 1200 ms Server-VAD defaults. Repair LVL-08 failure retention/control compatibility before another Provider run. Re-scope the missing Agent first-delta propagation before implementation, and keep the long-form pre-final stable-prefix screen separate. See the [LVL-09 result](evidence/LVL09_ADAPTIVE_PLAYOUT_LEAD_PILOT_RESULT_2026-08-25.md), [LVL-08 blocker](evidence/LVL08_SEMANTIC_VAD_PILOT_BLOCKER_2026-08-25.md), [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md) and [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
1. **Run implemented experiments.** The first LVL-09 same-workload pilot
   ...
   and a default change remain open. The first LVL-08 real attempt then failed
   its initial Server-VAD control with Provider-protocol plus unknown cleanup;
   Semantic AUTO/HIGH remain untested and receive no timing credit.
