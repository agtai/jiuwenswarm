...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from production Web. LVL-09 exposes **670–673 ms** same-workload schedule-to-start headroom but retains incomplete physical/default acceptance. LVL-08's protocol repair now completes the full real 12-slot AUTO pilot. AUTO saves **684–734 ms** of EOT on no-pause/300 ms cases but produces `EARLY_EOT` at 600/1000 ms, so it is integrity-rejected; HIGH remains intentionally unrun. The isolated Agent first-visible-delta packet now has a 15/15 real-Agent population: first-delta p50 is **523–592 ms**, while delta→final p50 is **9 ms / 1.68 s / 3.52 s** for short/medium/long output. This is diagnostic materiality, not immutable-prefix or integration credit. LVL-10 remains `INCONCLUSIVE`; LVL-10L remains directional and stopped | Preserve the 1000 ms WebAudio and 1200 ms Server-VAD defaults. Keep Semantic AUTO rejected and do not run HIGH; any future VAD work requires a new continuation-safe arbitration hypothesis. Decompose Agent queue/request/connection/network/model first-token timing and run an exact-prefix stable-segmentation screen for medium/long output before product wiring. Keep this pre-final work separate from post-final LVL-10/LVL-10L chunked TTS. See the [Agent first-delta baseline](evidence/AGENT_FIRST_DELTA_BASELINE_2026-08-25.md), [LVL-09 result](evidence/LVL09_ADAPTIVE_PLAYOUT_LEAD_PILOT_RESULT_2026-08-25.md), [LVL-08 retry result](evidence/LVL08_SEMANTIC_VAD_AUTO_RETRY_RESULT_2026-08-25.md), [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md) and [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
2. **Use the completed Agent diagnostic to prepare the next packets.** Isolated
   ...
   remain unknown. Short pre-final segmentation remains stopped; medium/long
   now justify a validation-only exact-prefix screen before any provisional
   `PresentationUnit` or product TTS wiring. This pre-final question remains
   separate from LVL-10/LVL-10L post-final chunked synthesis.
