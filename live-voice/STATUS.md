...
- **Current highest priority:** the consolidated P1/P2 lifecycle repair packet
  ...
  acceptance until the independent Tier-3 review of the cross-capture repair
  candidate `35537a9a` closes — and P3-9 cumulative product acceptance. The
  interruption-ledger bound is repaired on `30300f32`; `35537a9a` now retains
  and server-validates one exact predecessor plus successor, while preventing
  either segment from minting a competing Streaming commit receipt. The L0
...
- **Implemented source:** `35537a9a` adds one content-free predecessor close
  ...
  atomic replay tombstones. Both segment Streaming result/receipt paths are
  fenced; Streaming remains available only to observe the successor EOT.
- **Current evidence:** affected backend `257/257`; Formal Web `496/496`;
  Browser Gateway Media `38/38`; Browser Dedicated Media `27/27`; Gateway Batch
  Speech `30/30`; Ruff, diff check and `build:live-voice` PASS. This is source
  candidate credit only. Independent Tier-3 review and physical §5 acceptance
  remain open.
...
| Integrated Web product experience | **PARTIAL overall; the four P1/P2 lifecycle repair module Gates and the bounded decoupling review PASS `C0/I0/M0`; generation-time interruption remains blocked from physical acceptance pending review.** Same-tab recovery, exact Speech timeout/signal forwarding, browser-global capture ownership, retained predecessor ACK settlement and playout/capture decoupling are closed at their scoped source/automation boundaries. The exact consolidated product/test tree passed same-tab A -> B -> A, Exit/immediate re-enable, ordinary forward/reverse two-tab takeover and the cumulative microphone/Agent/playout journey. Production P2 notification pulls now use batch size `16`, with the validation-branch feature-on prompts audibly accepted. Generation-time listening, its exact-response interruption and its replacement turn are implemented behind a default-off flag. The targeted review at `b476873b` returned `C0/I2/M0`; `30300f32` repairs the ledger bound and `35537a9a` repairs the release-race transcript with an exact predecessor-close/successor-activation chain and one combined Batch final. Current Formal Web is `496/496`, but no independent review or real-device journey has accepted this repair ([handoff](GENERATION_INTERRUPTION_HANDOFF.md)) | Close the independent Tier-3 review, then run the flag-on real-device journey; real Provider degradation longer than 15 seconds, fixed-corpus latency/generalization, P3-9 and complete product acceptance remain open | [lifecycle closure](#p1p2-lifecycle-repair-closure), [generation interruption evidence](evidence/GENERATION_TIME_INTERRUPTION_20260823.md), scoped decoupling review and remaining P1/P2/P3-9 completion; no controlled-candidate credit |
...
| Automated verification and product acceptance | **PARTIAL overall; existing lifecycle/decoupling/P2 review Gates remain PASS, while L0 final review and generation-time interruption closure are open.** On `35537a9a`, the cross-capture affected backend set is `257/257`; Formal Web is `496/496`; Browser Gateway Media is `38/38`; Browser Dedicated Media is `27/27`; Gateway Batch Speech is `30/30`; Ruff, diff check and the Live Voice production build pass. Positive evidence proves predecessor-prefix then successor-tail ordering and one Batch final; negative evidence covers stale/replay, reorder, forged digest/track, cross Session/interaction/activation/connection, missing close/activation links and zero competing Streaming receipt. These checks grant source-candidate credit only; independent Tier-3 review and physical acceptance are still absent. Earlier L0 and broad-sweep results retain only their exact-source historical credit | Independent Tier-3 review of `35537a9a`; final L0 independent review, physical cold/warm fixed-corpus runs, P3-9 cumulative verification and controlled product-candidate journey remain open; Provider component/digital-loopback data has no physical credit | [generation-time interruption evidence](evidence/GENERATION_TIME_INTERRUPTION_20260823.md), [L0 evidence](evidence/L0_MEASUREMENT_BASELINE_EVIDENCE_20260823.md) and [P2 evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md); scoped evidence grants no P3-9 or product-readiness credit |
