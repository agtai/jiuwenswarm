...
- Routes: P1 `fallback + reviewed AIO/batch-Speech + bounded Media/browser-transport activation owners, still unavailable at the product registry`; P2 `default-off stock-Web committed-text submission into the real AgentManager/Harness/CR path + exact final-presentation acknowledgement/history + a pure dependency-readiness evaluator + one bounded immutable real Chrome/Agent/Tool journey`; P3alpha `default-off authenticated query/progress + bounded trusted confirmation/create/cancel + reviewed package-level atomic TaskEvent sequence coverage, still unregistered for product voice`; X-OBS `correlation + retained lifecycle + deterministic fault harness, not registered`; Integrated `default-off Web→Gateway→AgentServer P2/P3alpha text path + a pure deployment-configuration evaluator, without a real P1/Provider, X-OBS backend or acceptance journey`.
- Current priority: select and configure the real Speech/Media, P3alpha Executor, X-OBS and deployment dependencies; then run their verticals and the joint journey on one immutable candidate. Expand the bounded P2 evidence to refresh/reconnect, cancel/fault, multi-turn and rollover scenarios in parallel where dependencies permit. Missing external conditions remain fail-closed and do not block unrelated dependency-independent work.
- Current integrated branch is `hx/0803_live_voice` at its current documentation-only evidence tip; its tested P2 code remains `fb40f12b46fd3c407314c66055bf99911bc35267`. Above integration tip `124e70b6`, it adds formal P2 lifecycle hardening in `0ac21ce4`, the reviewed passive-Agent-tracer filter in `fb40f12b`, and the accompanying immutable evidence documentation; the exact clean tested candidate completed one stock-Web real DeepSeek Agent/Bash-Tool journey with presentation ACK, history, terminal and close evidence. The bounded result is recorded in [P2_BROWSER_AGENT_TOOL_20260807_fb40f12b.md](evidence/P2_BROWSER_AGENT_TOOL_20260807_fb40f12b.md). No acceptance Gate ran and Replacement Ledger credit remains zero.
...
- `PARTIAL / STILL OPEN`: tested P2 code `fb40f12b` retains the default-off committed-text/acknowledgement/history path and closes the passive internal tracer leak/rejection found by the first immutable real run. One clean stock-Web/Chrome → real DeepSeek Agent → Bash/Git Tool → final/ACK/history/terminal/close journey passed. Product-wide CR/Media composition, production identity, registered real Media/Provider, X-OBS backend, P3 Executor and the remaining browser/device/resilience matrix remain open.
- `CROSS-CUTTING PARTIAL`: P2/P3alpha form a real source-level Web→Gateway→AgentServer→Agent/Task text path with fail-closed limits and cleanup; P2 now also has one bounded browser-observed single-turn real Agent/Tool sample. It is not a cumulative real P1/P2/P3alpha journey, and the single P2 runtime intentionally admits at most 256 committed turns with no stock-Web automatic rollover.

## Git and release identity

- The checked-out local branch is `hx/0803_live_voice` and tracks matching `origin/hx/0803_live_voice` at its current documentation-only evidence tip. The immutable tested P2 code is `fb40f12b46fd3c407314c66055bf99911bc35267`; resolve exact HEAD, upstream and dirty state dynamically at resume.
...
| P2 Realtime | stock Web default-off activates an Alpha-authorized runtime, submits committed text once to the real AgentManager/Harness/CR path, retains exact notification/final state and ACKs exact presentation/history; immutable `fb40f12b` passed one real Chrome/DeepSeek/Bash-Tool single-turn journey | cover refresh/reconnect, cancel/fault, multi-turn/concurrency and a reviewed rollover design if sessions must exceed the bounded 256-turn runtime | no production identity, speech/media or joint journey; the bounded sample grants no P2/Web credit |
| P3alpha Task | stock Web authenticates query/progress, owns exact UI delivery ACK, issues bounded trusted create/cancel mutation, and package code owns exact SQLite TaskEvent prefix/live-tail plus atomic no-projection sequence coverage through terminal | register and compose the source only after CR/Media and product Composition review, then validate against a disposable registered Code project/model/configuration | product voice remains unavailable; oversized history fails closed, bounded Alpha authority is not production authorization, and UI ACK is not human observation |
| Integration / X-OBS | default-off Web→Gateway→AgentServer P2/P3alpha text routes, exact error propagation, reconnect fencing and cleanup are source-integrated; a pure HTTPS/WSS/origin/CSP/CORS/proxy/TLS evaluator is available; X-OBS has a bounded deterministic fault harness | select and configure the real deployment and X-OBS owners/backends, then run real P1/P2/P3alpha vertical and joint journeys | no deployment was observed; X-OBS is not registered; no exporter/retention/SLO backend, secure Chrome/device/service journey or runnable Integrated Gate |
...
- Integrated cumulative mode is still not runnable. P2 candidate `fb40f12b` has one bounded real browser/service Agent/Tool text journey, but the product still has no registered Media/Provider, X-OBS backend, production identity, P3 Executor/voice journey or cumulative P1/P2/P3alpha path. Current V0, stable-sentence and task modes remain separate compatibility/Demo routes.
...
1. Select/configure the real Speech Provider and Media/deployment owner, including the batch-versus-streaming decision and registered route-to-disk zero-persistence proof.
2. Retain the proven P2 model/Agent/Tool fixture for refresh/reconnect, cancel/fault, multi-turn/concurrency and rollover evidence; supply a disposable registered Code project, model/configuration and runnable Executor for P3.
3. Decide whether Code Agent support paths `.gitignore`, `coding_memory/`, `prompt_attachment/` and `.agent_history/` are relocated or explicitly governed; complete CR/Media and product Composition review before registering the atomic P3 voice source.
4. Select/configure the X-OBS backend, transport, retention/redaction and SLO policy, and provide the HTTPS/WSS proxy, CSP/CORS, WebSocket-upgrade and allowed-origin deployment.
5. Perform the real P1, P3alpha and X-OBS verticals plus one consolidated desktop-Chrome physical and joint service journey on the same immutable candidate; the existing P2 sample is bounded single-turn text evidence only.
6. Run the Immutable Alpha Gate only after all required real evidence exists; otherwise keep the `0/100` ledger and Gate state unchanged.
