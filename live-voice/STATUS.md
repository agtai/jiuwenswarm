...
| Observability, benchmark and latency | **PARTIAL.** Trace/correlation and historical verification foundations exist. The default-off minimal latency probe records the Browser timeline plus Gateway/Agent same-clock drill-downs and passed its final independent Tier-3 code review. The controlled-WAV Browser runner remains available for later physical validation. The candidate-neutral no-Browser P2 A1/A2 at `31f9209d6` reproduced 50/250/500 RPCs for 10/50/100 notifications. Bounded-pull candidate `c1b4a47f5` passed independent Tier-3 review, reduced those totals to 5/20/35 and reduced p50 by 90.0–92.9% against both baselines, with 15/15 successful attempts per run and zero forbidden effects. This is accepted causal P2 component evidence only; no physical Browser or end-to-end product-path improvement is claimed | Physically confirm the accepted bounded-pull candidate through the controlled Browser lane before product-path credit, then continue the remaining [latency optimization plan](roadmap/LATENCY_OPTIMIZATION_PLAN_2026-08-18.md) one owner at a time. Compare Hongxing's divergent P1/P2 fixes in a clean worktree rather than rebasing the active dirty branch | Accepted P2 notification transport candidate; next fixed physical Browser corpus/environment, then Conversation Runtime/P1 media owners |
