...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from production Web. LVL-09 exposes **670–673 ms** same-workload schedule-to-start headroom but retains incomplete physical/default acceptance. LVL-08's repaired runner retained a clean real AUTO pilot: all four Server-VAD controls completed, then the first Semantic AUTO case failed `UNKNOWN / PROVIDER_PROTOCOL` with clean cleanup and no timing; HIGH remains untested. The isolated Agent first-visible-delta producer packet passes Tier-2 source/test review but has no timed population or integration credit. LVL-10 remains `INCONCLUSIVE`; LVL-10L remains directional and stopped | Preserve the 1000 ms WebAudio and 1200 ms Server-VAD defaults. Diagnose the LVL-08 Semantic AUTO Provider protocol path before one bounded retry; do not run HIGH. Measure the isolated Agent first-delta packet before specifying connection/warm-up/prompt changes, and keep long-form pre-final stable-prefix work separate. See the [LVL-09 result](evidence/LVL09_ADAPTIVE_PLAYOUT_LEAD_PILOT_RESULT_2026-08-25.md), [LVL-08 AUTO result](evidence/LVL08_SEMANTIC_VAD_AUTO_PILOT_RESULT_2026-08-25.md), [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md) and [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
1. **Run implemented experiments.** The first LVL-09 same-workload pilot
   ...
   and a default change remain open. LVL-08 runner repair `5811aeb7f` then
   passed its Tier-2 Gate and a real AUTO pilot completed all four Server-VAD
   controls before the first Semantic AUTO case failed `UNKNOWN /
   PROVIDER_PROTOCOL` with clean cleanup. AUTO receives no timing and HIGH was
   not run.
