...
- Current priority: execute the dependency-backed [Wave C packet](roadmap/ALPHA_WAVE_C_EXECUTION_2026-08-07.md) from the pushed Wave A+B documentation baseline. Advance real Provider/media, P2 voice, P3alpha Executor/voice handoff and X-OBS/deployment work only where repository and machine facts provide the required owner; otherwise finish dependency-independent hardening and return an exact fail-closed dependency result.
- Wave A+B tested implementation is `dd637e604939cef7200d4ff5d30acf55a89d62f2`; its documentation closure is pushed as `107104bb22b9cfc705b02634a4eaf86d1d64f3bf`. At Wave C bootstrap, local `hx/0803_live_voice`, its `origin/hx/0803_live_voice` upstream and local `codex/lv-alpha-wave-c-integration` all resolved to `107104bb` with development divergence `0/0` and a clean worktree. Resolve current candidate HEAD and dirty state dynamically after work begins. No Wave C commit has been pushed, no real cumulative acceptance Gate has run, and Replacement Ledger credit remains zero.
...
- Development branch `hx/0803_live_voice` tracks `origin/hx/0803_live_voice`; both resolved to pushed Wave A+B documentation commit `107104bb22b9cfc705b02634a4eaf86d1d64f3bf` with `0/0` divergence at Wave C bootstrap. Local Wave C integration uses `codex/lv-alpha-wave-c-integration` from that exact baseline and must not push. Resolve current HEAD, upstream and dirty state dynamically at resume.
...
1. Run the Wave C read-only environment preflight and four owned lane oracles; continue every dependency-independent implementation while keeping absent Provider, device, Executor, deployment and exporter paths unavailable.
2. Integrate only reviewed dependency-backed lane commits through the single-writer lease, then run the D-061 cumulative smoke and production build on one local candidate.
3. When real conditions exist, prove the registered Media route-to-disk zero-persistence boundary, real P1/P2/P3alpha verticals, X-OBS backend behavior and the joint desktop-Chrome/service journey.
4. Run the Immutable Alpha Gate only after all required real evidence exists; otherwise return exact blockers and keep the `0/100` ledger and Gate state unchanged.
