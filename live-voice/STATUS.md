...
- Routes: P1 `fallback + reviewed AIO/batch-Speech + bounded Media/browser-transport activation owners, still unavailable at the product registry`; P2 `default-off stock-Web committed-text submission into the real AgentManager/Harness/CR path + exact final-presentation acknowledgement/history + a pure dependency-readiness evaluator`; P3alpha `default-off authenticated query/progress + bounded trusted confirmation/create/cancel + reviewed package-level atomic TaskEvent sequence coverage, still unregistered for product voice`; X-OBS `correlation + retained lifecycle + deterministic fault harness, not registered`; Integrated `default-off Web→Gateway→AgentServer P2/P3alpha text path + a pure deployment-configuration evaluator, without a real P1/Provider, X-OBS backend or acceptance journey`.
- Current priority: prepare the real dependencies listed in the [post-Wave-C integration review](ALPHA_POST_WAVE_C_INTEGRATION_REVIEW_2026-08-07.md), then run the real Provider/media, P2 browser Agent/Tool, P3alpha Executor/voice, X-OBS/deployment and joint journeys on one immutable candidate. Missing external conditions remain fail-closed and do not block unrelated dependency-independent work.
- Current tested local code is `83aace72462cd15bd6e13e6eb0221204b5b4c623`, now integrated into local `hx/0803_live_voice`. It adds pure P2/deployment readiness leaves and closes package-level P3 no-projection/terminal sequence coverage above the Wave C documentation baseline `41a05d79a482c4e2660ff18854c58975d9eb0479`; reviewed documentation closes at `b42ec99ca1ef0d60ebea73e8559ce9c92c204b8a`. No product registration, real E2E or remote ref changed; no acceptance Gate ran and Replacement Ledger credit remains zero.
...
- `PARTIAL / STILL OPEN`: tested code `83aace72` retains the default-off P2 committed-text/acknowledgement/history path and P3 trusted confirmation/create/cancel route. It adds closed, non-discovering P2/deployment readiness evaluators and package-level atomic SQLite TaskEvent no-projection/terminal sequence closure. Product registration, CR/Media composition, production identity, registered real Media/Provider, X-OBS backend and real browser/device/service evidence remain open.
...
- Local `hx/0803_live_voice` now contains the reviewed post-Wave-C candidate through documentation closure `b42ec99ca1ef0d60ebea73e8559ce9c92c204b8a` and tracks unchanged `origin/hx/0803_live_voice` at `107104bb22b9cfc705b02634a4eaf86d1d64f3bf`; including this mechanical STATUS synchronization, it is eight commits ahead. The preserved integration branch `codex/lv-alpha-next-integration` remains at `b42ec99c`, and tested code remains `83aace72`. The user authorized the local fast-forward; no remote ref changed. The final STATUS-only tip is intentionally outside the tested-code SHA; resolve exact HEAD and dirty state dynamically at resume.
...
| P2 Realtime | stock Web default-off activates an Alpha-authorized runtime, submits committed text once to the real AgentManager/Harness/CR path, retains exact notification/final state and ACKs exact presentation/history; a pure evaluator reports explicit dependency blockers without discovery or Gate claims | run a real registered Agent/Tool browser journey and add a reviewed rollover design if sessions must exceed the bounded 256-turn runtime | no production identity, speech/media journey, real Agent/Tool E2E or P2/Web credit |
...
- Integrated cumulative mode is still not runnable. Tested implementation `83aace72` adds only source/package readiness and P3 sequence closure above the existing default-off P2/P3alpha route; it still has no product-registered Media/Provider, X-OBS backend, production identity or real browser/service Agent/Tool/task journey. Current V0, stable-sentence and task modes remain separate compatibility/Demo routes.
...
2. Supply a registered/configured real Agent/Tool and matching model runtime for P2, plus a disposable registered Code project, model/configuration and runnable Executor for P3.
...
5. Perform one consolidated desktop-Chrome physical journey and the real P1/P2/P3alpha/X-OBS vertical and joint service journeys on the same immutable candidate.
