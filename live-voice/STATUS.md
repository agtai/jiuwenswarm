...
> Updated: 2026-09-07 (Shenzhen rehearsal exposed further playback and delegation defects).
...
That showcase is BLOCKED on these defects and grounded current-information lookup.
The diagnosis and script do not claim a product repair or supersede retained
...
|---|---|
| Answer hardcode retirement | Formal/Cascade Task explanations use one tool-disabled Agent and current authenticated facts; complete bounded result pages and lossless sequential TTS replace content deletion. Unreachable Task phrase/naming special cases are removed. Scoped tests/build/review and pre-existing suite failures: [retirement evidence](reviews/ANSWER_HARDCODE_RETIREMENT_20260905.md). Native-specific handling remains deferred. |
...
| OpenAI AgentModel | Official GPT-5.6 uses API-key Responses through the common single-Agent builder and Web validation. A pinned SDK fix preserves reasoning metadata and original tool calls across execution; real foreground read/background file delivery have scoped evidence in [OpenAI integration](reviews/OPENAI_AGENTMODEL_INTEGRATION_20260905.md). Model/key setup remains machine-private; full voice rehearsal and performance comparison remain open. |
