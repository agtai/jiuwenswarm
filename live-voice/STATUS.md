...
- **Last product-code baseline assessed:**
  `ddd845e561145cdb1aa2eb37d6ea9c57633494f6`. Documentation-only commits after
...
| Speech Recognition | **PARTIAL.** Controlled OpenAI Streaming/Batch STT and browser fallback work. Independently reviewed no-Browser real-Provider VAD screening at `e2773bec2` rejected fixed 900/800 ms and retained 1200 ms after A1/A2 passed 40/40 while each lower candidate passed 15/20 | Provider-neutral configuration, fixed accuracy/latency corpus, robust fallback/cancel, semantic/adaptive VAD evaluation and broader device/network validation | Audio I/O, media route and benchmark owner; [VAD/EOT causal result](evidence/VAD_EOT_CAUSAL_RESULT_2026-08-21.md) |
| Speech Synthesis | **PARTIAL.** Streaming/Batch TTS, browser playout, response ownership and ACK paths exist. The reviewed no-Chrome TTS first-audio candidate at `cd4d1b7d3` opens downlink independently of successor-capture ACK and preserves truthful non-duplex receipts. A real-Provider A1-v2/B test rejected application-level HTTPX client reuse: B warm reuse was 0/3 and warm first-PCM p50 regressed from 832.0 to 889.9 ms, so request-scoped lifecycle was restored at `ddd845e56` | Provider-neutral configuration, separately specified bounded post-`audio.done` EOF/reuse experiment if pursued, physical first-audible/underrun/pronunciation targets and complete stale/cancel recovery; authoritative settlement still waits for bounded successor readiness | Conversation Runtime ownership and Audio I/O stop confirmation; [TTS first-audio causal result](evidence/TTS_FIRST_AUDIO_CAUSAL_RESULT_2026-08-21.md) and [Provider connection result](roadmap/TTS_PROVIDER_CONNECTION_REUSE_RESULT_2026-08-21.md) |
...
| Interaction Intelligence | **PARTIAL.** VAD/EOT and bounded dialogue/background routing exist for the controlled journey. Exact 1000 ms pause evidence confirms that a lower global fixed threshold is unsafe | General natural-language routing, semantic/adaptive endpointing, false endpoint/interruption and echo/double-talk evaluation, language/config generalization; Native model-level duplex remains optional | Streaming Speech plus Runtime; measured golden corpus |
...
| Observability, benchmark and latency | **PARTIAL.** Trace/correlation and historical verification foundations exist. The minimal probe and controlled-WAV Browser runner remain available for later physical validation. Accepted no-Browser component results cover P2 bounded pull (90.0–92.9% p50 reduction) and reviewed TTS successor-ACK decoupling: at 250/750 ms injected ACK delay, first-source p50 changed from 255.597/756.542 ms to 0.486/0.476 ms. The complete EOT/STT A1 at `8e5dab8b8` closed `NO_MATERIAL_SERIAL_GAP`: all four removable-gap p50/fraction pairs were below 80 ms/0.10, so no early-wait candidate exists. The real-Provider VAD screen rejected fixed 900/800 ms, and the exact-runner TTS connection experiment rejected application-client reuse after 0/3 B warm connections reused and p50 regressed 7.0%. None grants physical Browser or end-to-end product credit | Route the latency workstream next to the specified Provider-native Semantic VAD screen with the 1200 ms fallback retained. Post-`audio.done` EOF draining requires a new bounded spec if pursued; physical first-audible confirmation remains open. TTS receipt settlement still waits for successor readiness and is not credited as improved | Accepted P2 notification and TTS first-audio component candidates; closed [EOT/STT materiality](evidence/EOT_STT_SETTLEMENT_MATERIALITY_RESULT_2026-08-21.md) with no candidate; rejected lower fixed VAD thresholds and application-level TTS client reuse; [connection result](roadmap/TTS_PROVIDER_CONNECTION_REUSE_RESULT_2026-08-21.md) |
