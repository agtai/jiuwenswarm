...
| Observability, benchmark and latency | **PARTIAL overall; scheduled P3 observability producer boundary closed.** Validated configuration, trusted public correlation, bounded exporter/backend and exact producers remain integrated. The diagnosed one-RPC-per-notification tail is removed from production Web. LVL-09 exposes **670–673 ms** same-workload schedule-to-start headroom but retains incomplete physical/default acceptance. LVL-08 AUTO is integrity-rejected after early EOT at 600/1000 ms. LVL-11 measures real-Agent first-delta timing. LVL-12 then passes its validation-only exact-prefix materiality screen 10/10: candidate→final p50 is **1.587 s medium / 3.573 s long**, with Agent→candidate p50 **658 / 708 ms** and zero forbidden effects. This grants no pre-final speech or product authority. LVL-10 remains `INCONCLUSIVE`; LVL-10L remains directional and stopped | Preserve the 1000 ms WebAudio and 1200 ms Server-VAD defaults. Keep Semantic AUTO rejected. Decompose Agent queue/request/connection/network/model first-token timing independently. LVL-12 now authorizes only a separate no-Browser real Agent→TTS component screen; keep it distinct from post-final LVL-10/LVL-10L and do not wire Runtime/P2/Browser before a product-authority design. See the [LVL-12 result](evidence/LVL12_PRE_FINAL_STABLE_SEGMENTATION_RESULT_2026-08-25.md), [Agent first-delta baseline](evidence/AGENT_FIRST_DELTA_BASELINE_2026-08-25.md), [LVL-09 result](evidence/LVL09_ADAPTIVE_PLAYOUT_LEAD_PILOT_RESULT_2026-08-25.md), [LVL-08 retry](evidence/LVL08_SEMANTIC_VAD_AUTO_RETRY_RESULT_2026-08-25.md), [LVL-10 result](evidence/LVL10_AUTHORITATIVE_FINAL_CHUNKED_TTS_RESULT_2026-08-24.md) and [inventory](roadmap/LATENCY_OPTIMIZATION_INVENTORY_2026-08-21.md) | D-093 observability evidence, [P2 default-on evidence](evidence/P2_NOTIFICATION_BATCH_DEFAULT_ON_20260821.md) and D-094 |
...
2. **Use the completed Agent diagnostic to prepare the next packets.** Isolated
   ...
   next gate is a validation-only no-Browser Agent→TTS screen; no provisional
   product `PresentationUnit` is authorized. This pre-final question remains
