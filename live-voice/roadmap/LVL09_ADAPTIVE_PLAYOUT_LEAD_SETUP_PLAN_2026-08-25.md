...
2. Repair private `lv-driver.sh`: validate a LVL-09 manifest before launch; derive expected profile/case/round; only beep/advance on matching completed export; snapshot all rows using row identity; restart only the Vite process group at each eligible arm transition with the next declared build-time lead; start services with `setsid`; drain/terminate/kill whole groups.
