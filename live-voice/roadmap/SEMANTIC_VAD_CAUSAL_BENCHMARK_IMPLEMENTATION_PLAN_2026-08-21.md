...
## 2026-08-24 execution clarification

- This plan is now local to the current latency lane rather than readable only
  through `latency_checkpoint_accepted_optimizations`.
- Local forensics confirmed the current tree has only manual/Server-VAD typed
  modes; Semantic VAD is not already implemented here.
- Current SOTA research recommends Provider-native `semantic_vad` first.
  LiveKit/Pipecat/local hybrid detectors are excluded from this packet.
- Execute `auto` and `high` as independent blocks. Do not pool them or use one
  block's controls to rescue the other.
- Each pilot is 12 attempts. Each fast formal is 60 attempts. The expanded
  corpus is conditional on fast-screen eligibility and receives a separate
  cost/request review before execution.
- The fixed 1200 ms Server-VAD product route remains unchanged throughout.
- The complete rationale, clocks, staged procedure, stop rules and non-claims
  are in spec section 16; executors must read that section before Task 1.
