...
| Historical LVL-07 stable-sentence Agent→TTS overlap | **ORIGINAL FIXTURES `STOP`; SUPERSEDED MATERIALITY QUESTION CONTINUES AS LVL-12** | Pure response policy, real formal Agent and benchmark-only real TTS; no Runtime/P2/Browser wiring | Historical projected gain p50 **177.2 ms**; later LVL-12 exact-prefix candidate→final p50 **1587.308 / 3573.381 ms** medium/long | Historical 3/3 result remains immutable; separate LVL-12 completes 10/10 at `ea6475120` | Do not rewrite LVL-07. LVL-12 now passes exact-prefix materiality for medium/long and moves only to a no-Browser TTS component screen. |
...
| 6 | Pre-final stable-sentence Agent→TTS overlap | **EXACT-PREFIX SCREEN PASS; NO-BROWSER TTS SCREEN NEXT; SHORT `STOP`** | Candidate→final p50 medium/long **1587.308 / 3573.381 ms**; TTS overlap gain `UNKNOWN` | LVL-12 real-Agent 10/10 at `ea6475120`; no TTS/Runtime/P2/Browser product wiring | Exact-prefix materiality passes for medium/long. Next measure real TTS first PCM without product wiring; short remains stopped. |
...
5. Preserve the completed LVL-11 Agent first-delta baseline before attempting
   ...
   observed.
6. Keep stable-sentence stopped for short. Preserve the LVL-12 medium/long
   exact-prefix PASS and run a validation-only no-Browser Agent→TTS component
   screen. Keep LVL-10/LVL-10L independent because they start after
   authoritative `chat.final`.
