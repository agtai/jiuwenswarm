...
The LVL-13 pre-final Agent-to-TTS component screen is bound separately to
branch `latency/pre-final-stable-agent-tts-screen`, JiuwenSwarm source
`9600adbcf214a6bbc9dd6e08db4a8c59697854d3` and the same installed Agent-Core
`94e10cb6102c36fe78a64547957c0def97299273`. It measures real Agent to real
Speech Provider first PCM without Runtime, P2, Browser, WebAudio or product
wiring.
...
| LVL-12 pre-final stable segmentation | **EXACT-PREFIX MATERIALITY PASS — NO PRODUCT/TTS AUTHORITY** | Validation-only real formal Agent policy/runner; no Browser, STT, TTS, Tool, Task or product wiring | Candidate→final p50 medium/long **1587.308 / 3573.381 ms**; Agent→candidate p50 **657.947 / 707.935 ms** | Accepted v2 10/10 at `ea6475120`, exact-prefix/terminal-complete throughout, zero forbidden effects; prior long 0/5 population retained as rejected diagnostic | Its authorized no-Browser Agent→TTS follow-up completed as LVL-13; LVL-12 itself grants no TTS/product credit. See the [LVL-12 result](../evidence/LVL12_PRE_FINAL_STABLE_SEGMENTATION_RESULT_2026-08-25.md). |
| LVL-13 pre-final exact-prefix Agent→TTS overlap | **ACCEPTED — REAL-AGENT/REAL-TTS COMPONENT; NO PRODUCT/BROWSER CREDIT** | Validation-only no-Browser A1/B/A2 runner; real formal Agent and Speech Provider; product source unchanged | Agent→first-PCM p50 gain **1723.166 ms / 52.510% medium** and **3422.345 ms / 67.377% long** | Formal 30/30 at `9600adbcf`, exact-prefix completion, valid A1/A2 drift, zero forbidden effects; independent artifact audit `C0/I0/M0` | Requires a product-authority design before Runtime/P2/Browser wiring and physical Gate C. See the [LVL-13 result](../evidence/LVL13_PRE_FINAL_STABLE_AGENT_TTS_RESULT_2026-08-25.md). |
...
### 5.8 LVL-13 pre-final exact-prefix Agent→TTS component screen
...
5. Preserve the 15/15 real-Agent first-delta baseline. Decompose queue,
   ...
   exists.
6. Keep stable-sentence stopped for short responses. Preserve the LVL-12
   exact-prefix PASS and the LVL-13 real-Agent/real-TTS component acceptance
   for medium/long. Product pre-final speech remains excluded: first define and
   review immutable presentation authority, reconciliation and cancellation,
   then run deployed physical Gate C. Keep LVL-10 independent because it starts
...
immutable prefix or carry product latency credit. LVL-12 subsequently proved
exact-prefix materiality 10/10 for medium/long, and LVL-13 accepted the next
real-Agent/real-TTS component screen 30/30: digital Agent→first-PCM p50 gain is
1.723 s / 52.510% medium and 3.422 s / 67.377% long. The gain combines overlap
and shorter first-prefix input; remaining-response continuity, Runtime/P2,
Browser and physical first-audible remain unmeasured.
