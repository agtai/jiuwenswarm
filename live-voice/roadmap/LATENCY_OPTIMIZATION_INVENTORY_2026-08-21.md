...
| 6 | Pre-final stable-sentence Agent→TTS overlap | **EXACT-PREFIX SCREEN PASS; NO-BROWSER TTS SCREEN NEXT; SHORT `STOP`** | Measured candidate→final p50 medium **1587.308 ms**, long **3573.381 ms**; TTS overlap gain remains `UNKNOWN` | LVL-12 real-Agent 10/10 exact-prefix screen at `ea6475120`; no TTS/Runtime/P2/Browser wiring | The prefix materiality gate passes for medium/long. Next measure real TTS first-PCM overlap without product wiring. Short remains non-material; post-final LVL-10/LVL-10L answer a different question. |
...
6. Keep stable-sentence stopped for short responses. Preserve the LVL-12
   medium/long exact-prefix PASS and run a validation-only no-Browser
   Agent→TTS component screen. Product pre-final speech remains excluded; keep
