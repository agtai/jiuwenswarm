...
Status: approved design promoted to the current latency lane; local/SOTA
research reconciled on 2026-08-24; implementation and real-Provider evidence
pending
...
This packet follows, and does not combine with, the existing EOT/STT
settlement-overlap packet recorded on
`latency_checkpoint_accepted_optimizations` at
...
The credited fixed-threshold experiment recorded on
`latency/vad-eot-causal-benchmark` at
`live-voice/evidence/VAD_EOT_CAUSAL_RESULT_2026-08-21.md` showed:
...
## 16. 2026-08-24 rationale and operator procedure

### 16.1 Why this is the next experiment

The fixed-threshold result proved both the opportunity and the failure mode:
900/800 ms recovered approximately 285--412 ms on surviving turns, but each
configuration cut all five utterances containing a 1000 ms sentence-internal
pause. Silence duration alone cannot distinguish hesitation from completion.

Current primary-source research and local code forensics support the narrowest
next move: use OpenAI's Provider-native `semantic_vad` inside the existing
Realtime transcription session. It adds no second semantic RPC, socket or
commit owner. Local LiveKit/Pipecat-style or hybrid arbitration remains a later
shadow/re-scope candidate because it would introduce another clock, resource
budget, fallback and authority decision.

### 16.2 Frozen implementation sequence

Implementation proceeds in this order:

1. bind one clean JiuwenSwarm source, installed Agent-Core commit, current
   Provider/model labels and corpus hashes;
2. add the closed typed mode/config/capability domain and prove manual plus
   Server-VAD compatibility first;
3. add exact OpenAI wire/echo handling for `auto` and `high`, keeping both
   authority flags false;
4. generalize only the existing Provider-owned VAD commit/fence path in the
   Gateway; do not add another EOT owner;
5. extend the existing VAD runner/configuration dimension and reuse its pacing,
   transcript, event-order, privacy, cleanup and zero-effect oracles;
6. freeze/verify the expanded private semantic corpus;
7. close deterministic Tier-3 tests and independent module review before any
   real Provider attempt;
8. execute the staged Provider procedure below and stop at the first failed
   gate.

### 16.3 Staged Provider procedure

The procedure is deliberately incremental:

1. validate the current `vad-en-v1` manifest/WAV hashes and acquire the shared
   benchmark lock;
2. run `AUTO` pilot: four cases × A1/B/A2 × one attempt = 12 attempts;
3. stop on any wire/echo, pacing, identity, transcript, early-EOT, timeout or
   cleanup failure;
4. independently run the same 12-attempt `HIGH` pilot;
5. only a clean pilot may run its own fast formal block: four cases × three
   arms × five attempts = 60 attempts;
6. require 20/20 candidate integrity before considering generalization;
7. apply every latency/integrity/drift gate separately to `auto` and `high`;
8. run the expanded semantic corpus only for a fast-screen-eligible setting,
   after separately reviewing its request count and cost;
9. retain every failed/invalid run under a new immutable run ID; never retry or
   overwrite it to improve the population.

The long-duration TTS STOP does not change this packet: the VAD fixtures are
short, fixed audio cases, but their real-Provider cost remains explicitly
staged and bounded by the pilot/formal gates above.

### 16.4 What the measurements mean

The primary `final_voiced_frame_to_eot_ms` tests whether endpointing actually
became earlier. `eot_to_final_ms` detects shifted wait, and
`final_voiced_frame_to_final_ms` tests the complete Speech component. A
candidate is not faster if it merely moves time from EOT into STT finalization.

Integrity always dominates timing. One early EOT, missing continuation token,
duplicate/mismatched boundary, client commit, post-fence frame, unclean
cleanup, invalid pacing or forbidden downstream effect rejects the attempt and
withholds a latency sample. The 1000 ms pause case is the minimum direct oracle
against repeating the fixed-threshold failure.

### 16.5 Selection and non-claims

`auto` and `high` retain independent controls. `auto` wins ties; `high` is
selected only under the frozen 80 ms every-completed-case advantage and no-
regression rule in section 12. A passing no-Browser screen grants only
component eligibility. Product activation, Browser/device behaviour, barge-in,
Chinese/Portuguese/generalization and Production remain separate packets.

The supporting research is retained privately at:

```text
/home/renan/openJiuwen-ai/live-voice-latency-runs/private-review-archive/2026-08-24/lvl08-sota/
```

Those reports inform this design but do not substitute for current-source
Provider evidence.
