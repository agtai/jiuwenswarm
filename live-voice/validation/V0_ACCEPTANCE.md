...
- 不可变 Released / Frozen baseline：`ee2896a4afb186e693c720476b6de10797e66f72`
...
git fetch agtai
git merge-base --is-ancestor $expectedV0Sha agtai/hx/0803_live_voice
$LASTEXITCODE
```
...
新机器必须从 `uv.lock` 和 `package-lock.json` 重建依赖，禁止复制其他机器的 `.venv` 或 `node_modules`。详细命令和私有配置边界见 [E2E_RUNBOOK.md](../runbooks/E2E_RUNBOOK.md)。
...
随后按 [E2E_RUNBOOK.md](../runbooks/E2E_RUNBOOK.md) 启动服务。浏览器必须真实收到 `connection.ack`，并先用文字请求确认：
...
严格执行 [DEMO_SHOWCASE.md](../demo/DEMO_SHOWCASE.md) 的三轮主演示。使用同一个 candidate SHA 和固定环境，完整脚本连续成功 3 次。任一完整 run 失败，连续计数清零；保存失败证据并从新的完整 Run 1 开始，不能只补跑失败的一轮。
...
- 共享累计分支保存 `2c700934` 与 `d4c3e32a` 两次失败历史、D-037 hotfix、Released / Frozen baseline `ee2896a4` 和已推送 Post-V0 Foundation；focused hotfix tests 只证明修复本身，V0 放行由 D-038 的完整 Gate 0–6 证据决定；
- `d4c3e32a` 的 Gate 0/1 与真实语音 Gate 2 已通过，但 Gate 3 Attempt 1 因 Windows Git 非 ASCII 日期格式 OOM 和重复失败保护不足而 FAIL；该失败历史不得改写为通过。47/47 和 22/22 是 V0 baseline 自动化，最新 Foundation 的自动化必须在 `STATUS.md` 中与 V0 证据分开报告；
- `ee2896a4` 已按 D-038 完成 Gate 0–6，包括连续任务、分阶段打断、21m58s soak、主演示连续 3 次和等效冷恢复；Gate 3 采用每个固定只读目标最多两次语音重试的任务级成功口径，Gate 6 未宣称第二台独立机器实测；
...
建议每次正式验收新建 `live-voice/evidence/V0_<日期>_<candidate短SHA>.md`，只提交脱敏文本摘要；原始录屏和日志留在受控位置。
