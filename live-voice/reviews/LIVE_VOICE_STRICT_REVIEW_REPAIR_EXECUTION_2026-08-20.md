...
- **Closure boundary — now lifted.** A1's acceptance held at closure, but product
  TTS end to end remained capped at 256 per channel owner by
  `StreamingSynthesisRouteOwner._retained_bindings`, the second limit the review
  numbers separately as A17. **SRR-25 closed A17**, so both ceilings are now
  bounded-and-fenced rather than terminal. The route's own retained ledgers stay
  capped at `_MAX_ROUTE_IDENTITIES`; what changed is that reaching the cap retires
  a settled identity behind a fence instead of ending streaming for the owner.
- Exclusions: no new product policy or classifier, no protocol/schema change,
...
## 5.11 Wave 11 ownership record — CLOSED
...
- **Four unguarded claims in the progress bridge's pre-existing close path
  ...
  are comment-stated invariants with no killing test: `close()`'s claim that
  the detach intent is recorded before any cancellable await (a mutant moving
  `_close_requested` into `_close_impl`, across a task scheduling point,
  survives all 54), and its claim that a cancelled waiter releases the retry
  slot (removing that release survives). A third is a silent barrier with no
  claim and no guard — deleting `_close_impl`'s `async with self._delivery_lock:
  pass` survives. The fourth is this module's second name-promises-more-than-it-
  guards case: `test_cancelled_close_waiter_does_not_cancel_detach_cleanup`
  passes with its `shield` removed, because asserting only the final
  `close_calls == 1` cannot separate "cleanup was preserved" from "cleanup was
  cancelled and redone". The review measured the minimal fix — one
  `assert subscription.close_calls == 1` inside the `pytest.raises` block, green
  on the real implementation and fatal to the unshielded mutant. Unlike the
  round-two blocker, the shield mechanism itself does have a killer elsewhere,
  so only the name is unearned.
- **A worker containment claim that is a port contract, not an input invariant
  (from SRR-30 review).** The bridge accepts an arbiter with no runtime type
  gate and calls its acknowledge port bare, so containment must survive any
  failure that port raises. The packet pins this by patching the real
  `ProgressNotificationArbiter` instance rather than substituting a double, but
  the shipped implementation swallows every input error internally, so no
  shipped input can reach the raise. Two reviews independently judged it
  adequate as a port contract and inadequate as a shipped-path invariant, and
  recorded it rather than blocking on it. The distinction is worth keeping: a
  claim about what an injected collaborator may do is not testable from shipped
  inputs alone.
...
- **Cancel-lane symmetry gaps in the SRR-27 control ledger (from SRR-27
  ...
  mirrors — dropping the reservation in `_request_response_cancel`, reordering
  fence and exact in `post_response_cancel` — still survive mutation. These are
  symmetry gaps rather than new property classes, so they did not block the
  packet, but the cancel body is the less forgiving of the two: it mutates cancel
  state and emits an effect, while the barge path's `_emit_playback_stop_once` is
  idempotent by design. Cheap to close by parametrising the two existing tests
  across both lanes.
- **A caller future hangs when `BaseException` escapes caller-supplied control
  code (from SRR-27 review, pre-existing).** `_apply_operation` catches only
  `Exception`, so a `BaseException` propagates out of `_run`; `_fail_pending`
  drains the queued lanes but the in-flight operation's future was already
  popped, leaving an awaited `barge_in` unresolved forever. Confirmed identical
  at baseline `7e4f410e6` with zero touched lines in that path, so SRR-27
  neither introduced nor widened it; the packet pins it as an explicit
  characterization rather than a claim.
- **`_require_generation` could be aligned with `MAX_SAFE_INTEGER` (from SRR-26
  review).** The adapter accepts any positive int while its sole production
  caller already validates `<= MAX_SAFE_INTEGER` at both entry points. Aligning
  them would make the fence saturation question moot entirely. It newly rejects
  inputs the adapter accepts today, so it is a validation-policy change needing
  its own scope checkpoint.
- **The Python 3.11 flake residual (from SRR-26).** The rollback tests are
  settled with `asyncio.eager_task_factory`, which is 3.12+. `pyproject.toml`
  declares `>=3.11,<3.14` and there is no in-repo CI matrix, so on 3.11 those two
  tests behave exactly as before and the wall-clock flake persists there. The
  alternative, a frozen-clock fallback, would have turned a genuine regression
  into a hang, so the explicit guarded no-op was chosen deliberately.
- **Fence degradation is asymmetric across the two structures (from SRR-25
  review).** The reviewer measured what nobody had quantified: the 1 MiB
  admission bitmap holds 0% false refusal at 100k retired identities and 2.0% at
  1M, while the 256 KiB generation sketch over-estimates 0.55% at 4k, 19% at 16k
  and 72% at 64k — roughly 250x earlier. Under-estimation was 0 everywhere, so
  the direction is always fail closed with the existing reason, and the sketch
  only matters for an interaction that already survived 256+ interactions without
  a stream. The memory split is nevertheless inverted relative to where the
  pressure falls; worth revisiting if long-lived channels grow.
