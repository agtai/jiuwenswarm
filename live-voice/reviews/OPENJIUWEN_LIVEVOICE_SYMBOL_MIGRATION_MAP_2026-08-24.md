...
> 文中的 `D-096`/`D-097` 指该准备分支自己的两条决定（见
> [准备分支决定摘录](PREP_BRANCH_DECISIONS_D096_D097_2026-09-01.md)），与本分支 `decisions/DECISIONS.md`
> 里同编号的 L0 普通 Chrome 决定无关；移植时未接受为产品决定。文中的产品事实绑定 `59998e2c`，
> 当前差异见 [激活预检](OPENJIUWEN_LIVEVOICE_SLIMMING_ACTIVATION_PRECHECK_2026-09-05.md)。
