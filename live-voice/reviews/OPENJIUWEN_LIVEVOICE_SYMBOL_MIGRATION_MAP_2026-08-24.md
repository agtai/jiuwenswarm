...
| Confidence/open issue | Medium; choose same-store or safe two-phase linkage without making Checkpointer a Task authority or claiming unavailable cross-store atomicity. |
...
| Dependencies | `EVT-01`. |
| Retirement Gate | AgentCore cursor is sole durable consumption truth and text/voice classes remain independently acknowledged. |
| Confidence/open issue | High; decide whether a closed presentation creates a tombstone or only advances/retains the cursor. |
...
| Classification | `ADAPTER_DOWNSTREAM` |
| Target owner/API | AgentCore `EffectJournal` from `ADD-04`; its continuation token gates adapter invocation and its observation API records the returned evidence. |
| JiuwenSwarm Adapter | Input: execution/effect continuation token plus expected project state. Output: typed `not_observed/observed/ambiguous` evidence and optional compensation result. |
...
7. Within the fixed AgentTeams database boundary, do EffectJournal methods live
   on TaskDao or a subordinate EffectDao sharing its transaction/identity?
   Business-neutral Workflow `engine/` may receive an injected protocol but may
   not import TaskDao/A2. Each Tool must declare probe/compensation availability.
