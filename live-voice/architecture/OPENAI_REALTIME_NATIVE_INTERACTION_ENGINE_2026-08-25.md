...
- 决策记录：[D-101](../decisions/DECISIONS.md#d-101-以独立-tier-3-合同激活-openai-realtime-native-interaction-engine)
...
源码勘察确认 Provider session/原始音频必须留在 Gateway，而 Conversation Runtime、Agent Bridge 和 Voice–Task Bridge/P3 留在 AgentServer。D-102 因此把跨进程 carrier 明确纳入同一个 `live-voice.native-interaction.v1`，风险仍为 Tier-3：
...
### 5.5 D-103 response-scoped streaming correction overlay
...
闭环。D-101/D-102 保留为当时的设计历史，本节和 D-103 是后续实现的控制性修正。
...
[D-103 correction plan](../../docs/superpowers/plans/2026-08-25-openai-realtime-native-response-stream-correction.md)。
...
- 范围扩展到 D-102 已冻结的单一 `native.audio` 变体之外的新客户端协议、public deployment、account/billing、Production 或 remote ref update。
