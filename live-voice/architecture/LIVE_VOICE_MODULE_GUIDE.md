...
> 2026-09-15 Task 控制与初始化读取替代后：Voice **109867** / Host **48139** / SDK **34212**，合计净增 **192218**；相对本轮实际 HEAD 基线 **194530** 净减 **2312**。累计生产新增 **781** / 删除 **3093**，不含测试和文档。本组新增329行、删除1337行，净减1008行：Native 共用受理、Host 接管确认至执行、Task 面板与 Voice 共用初始化读取，并删除隐藏手工 mutation owner。减量主要来自隐藏控制链，不能视为全部活跃管理状态已统一；此前1003行休眠前端删除仍计入累计值。进度副本与剩余 Host 受理归属仍未闭合。[逐项决策](../reviews/MANAGEMENT_REPLACEMENT_20260915.md)、[当前逐文件差量与合并模块汇总](../evidence/MANAGEMENT_TASK_CONTROL_20260915.json)、[前一批队列判定差量](../evidence/MANAGEMENT_QUEUE_CONTROL_20260915.json)。后文数字保留各自历史阶段。
