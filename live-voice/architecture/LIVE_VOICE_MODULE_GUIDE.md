...
> 2026-09-15 正式 Task 应用管理收敛：Voice **109326** / Host **48517** / SDK **34212**，合计净增 **192055**；相对本轮实际 HEAD 基线 **194530** 净减 **2475**。累计生产新增 **1322** / 删除 **3797**，不含测试和文档。本组新增205行、删除221行，净减16行：Host 统一受理准备、澄清选择和执行前决策，Registry 删除对应业务判断并保留传输适配。累计成果包含活动 Task 事实/恢复所有者和重复受理链替代，也包含此前1003行休眠前端及隐藏控制链删除；不将全部删行归为活动实现消重。正式 Task 阶段完成；整体深度融合、Work 后续统一与物理产品验收仍未完成。[逐项决策与验收](../reviews/MANAGEMENT_REPLACEMENT_20260915.md#formal-task-phase-completion-audit)、[当前逐文件差量与合并模块汇总](../evidence/MANAGEMENT_TASK_ADMISSION_20260915.json)。后文数字保留各自历史阶段。
...
```mermaid
flowchart LR
  A[语音 / 文本 / Task 面板] --> B[入口适配与请求身份]
  B --> C[Host 受理 / 澄清 / 确认 / 控制]
  C --> D[AgentCore Core / Store 原子事实]
  D --> E[原生 Runner / TaskManager]
  E --> F[Host Agent / Harness 与项目副作用校验]
  D --> G[Host 查询与结果服务]
  G --> H[共享前端 Task 事实与恢复]
  H --> I[面板展示 / Voice 播放与 ACK]
```

AgentServer 装配上述应用服务和执行依赖。面板 retry 保留其精确 Attempt
确认适配，最终进入相同 Host/Core。请求重放、来源身份与播放 ACK 各自保留
必要证明；它们不持有第二份 Task 执行状态。此图描述已交付的 Task 边界，
不表示 Work、音频生命周期或全产品已统一。
...
> 后续归属收敛：Task／Work 调度 → 原生 TaskManager.create_root_task → 同一 create_task；Task 来源恢复 → 已接受 Task.native_source 与已有调用回执，旧来源表只读；浏览器自然输入 → ProductUnifiedCommittedInputOwner，Task 视图 → ChatPanel 共享 FormalTaskSessionProvider。休眠的 ProductFormalTaskIntentOwner 已删除。 Task 事件订阅统一从 Store 原子快照或持久消费水位开始；Task／Attempt／Task 三次读取的旧启动已删除，公共验证、分页与 ACK 归属保持。 排队控制：Host 读取 TaskAuthorityReadSnapshot.queue_control；Store 在更新／优先级修改事务中重查同一规则，旧页面能力不授予写入权限。
