...
helper does not reinterpret them as S7-03, S7-04 or Alpha PASS.
...
WSS `connection.ack`. Port occupancy alone is never health. Any mismatch returns
`BLOCKED`; it never changes the environment.
...
Exit Live Voice, stop the exact frontend/Gateway/AgentServer processes recorded
in the session, and update the S8-03 user observations. The helper never kills a
process. It only verifies that each recorded PID is gone, all exact ports are
released, Task/outbox/owner/lease rows are settled, fixture effects match the
declared paths/diff, and candidate HEAD/worktree identity is unchanged.
