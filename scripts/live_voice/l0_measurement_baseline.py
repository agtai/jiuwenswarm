...
PROVIDER_REPORT_VERSION = "live-voice.l0-provider-component-report.v1"
...
def _binding(profile_index: int, sample_index: int) -> L0RoundBinding:
...
def _emit_round(
    ...
    *,
    ...
    binding = _binding(profile_index, sample_index)
...
async def build_provider_component_baseline(
    *,
    corpus_path: Path,
    successful_rounds: int,
    max_attempts_per_temperature: int,
    source_head: str,
) -> tuple[dict[str, object], bool]:
    if successful_rounds < 20:
        raise ValueError("provider profiles require at least 20 successful rounds")
    if max_attempts_per_temperature < successful_rounds:
    ...
    all_complete = True
    for temperature in (L0RoundTemperature.COLD, L0RoundTemperature.WARM):
        warm_provider = (
            create_environment_batch_speech_provider()
            if temperature is L0RoundTemperature.WARM
            else None
        )
        successes: list[dict[str, float]] = []
        failures = 0
        attempts = 0
        while len(successes) < successful_rounds and attempts < max_attempts_per_temperature:
            attempt = attempts
            attempts += 1
            provider = warm_provider or create_environment_batch_speech_provider()
            try:
                ...
                synthesized = await provider.synthesize(
                    ProviderSynthesisRequest(
                        f"l0-provider-tts-{temperature.value}-{attempt}",
                        stimulus,
                        "zh-CN",
                        None,
                        24_000,
                ...
                recognized = await provider.recognize(
                    ProviderRecognitionRequest(
                        f"l0-provider-stt-{temperature.value}-{attempt}",
        ...
        groups.append(
            {
                "profile_id": f"real-provider-digital-loopback-{temperature.value}",
...
def _parser() -> argparse.ArgumentParser:
    ...
    provider.add_argument("--max-attempts-per-temperature", type=int, default=25)
    ...
    if args.command == "provider-baseline":
        report, complete = asyncio.run(
            build_provider_component_baseline(
                corpus_path=args.corpus,
                successful_rounds=args.successful_rounds,
                max_attempts_per_temperature=args.max_attempts_per_temperature,
                source_head=source_head,
            )
        )
        _write_json(args.output, report)
