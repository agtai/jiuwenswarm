...
- Long Slack-native reports can place
  ...
  carrying several of them arrives as the brief plus one threaded reply per
...
- `blockkit_allowed_block_types` restricts which block types such a fence may
  carry. Empty (the default) restricts nothing, because which types Slack draws
...
- `blockkit_allow_interactive` (default `false`) decides separately whether such
  a fence may carry buttons, selects and other elements that post back to the
