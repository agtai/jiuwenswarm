...
- 表情确认需要 `reactions:write` 权限；缺少该权限时只记录日志，不影响请求处理。未配置 `acknowledge_mode` 时仍兼容已废弃的 `acknowledge_requests`：`true` 映射为 `text`，`false` 映射为 `off`——该开关在模式出现之前控制的正是那一条文本确认的开与关。配置了 `acknowledge_mode` 即以其为准，两个键同时存在时不会读取该布尔值；冒号后不写任何值时视为未设置，沿用默认模式。
