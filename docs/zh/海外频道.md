...
4. 在 **OAuth & Permissions** 中添加以下 Bot Token Scopes：
   ...
   - `app_mentions:read`
   - `im:history`
   - 如果需要自动处理链接或总结公开频道历史，添加 `channels:history`
   - 推荐添加 `users:read`，使频道摘要显示成员姓名而不是 Slack 用户 ID
   - 只有在需要总结私有频道历史时才添加 `groups:history`
5. 在 **Event Subscriptions** 中订阅 Bot Events：
   - `app_mention`
   - `message.im`
   - 如果需要自动处理指定频道中的链接，再订阅 `message.channels`
   - 按需生成频道摘要不需要新增事件订阅，由 `app_mention` 触发即可
6. 将应用安装到 Workspace，保存生成的 `xoxb-...` Bot Token。新增或修改 OAuth scope 后，需要重新安装应用到 Workspace。
...
- 将当前会话 ID 加入 `history_digest_channel_ids`，或配置 `['*']` 显式允许机器人可访问的所有 Slack 会话后，可以让机器人整理当前频道及其所有消息列回复中的高价值信息，例如：
  ...
  机器人始终从请求上下文确定当前频道，并在该请求所在的消息列中返回摘要，不会接受提示词中指定的频道 ID。“全部”是指 Slack 仍保留且机器人有权访问的历史，同时受内置消息数、API 调用数、内容大小和 90 秒扫描时限约束。如果保留策略、权限、上限或 API 错误导致无法完整扫描，摘要会明确标记为“部分覆盖”。机器人必须已经加入目标频道；私有频道还需要 `groups:history`。
...
- 定时任务可以将 `targets` 设置为 `slack`。从 Slack 创建的任务会继承来源频道和线程；设置 `post_as_root: true`（统一 cron 接口使用 `delivery.post_as_root: true`）可将每次结果发布为频道顶层消息。
- 较长的 Slack 原生报告可以在摘要和详细证据之间放置 `<!-- jiuwenswarm:slack-thread-details -->`。JiuwenSwarm 会把摘要发布为频道顶层消息，并把详细内容发布到该摘要自己的线程中；没有该标记时，会尽量按段落边界切分，避免截断链接和列表项。
...
- 确认机器人已加入当前频道并具有 `channels:history`；私有频道需要 `groups:history`。
- 确认当前会话 ID 已加入 `history_digest_channel_ids`，或使用 `['*']` 允许机器人可访问的所有会话；空列表会关闭该能力。
- 添加推荐的 `users:read` scope 以解析成员姓名，然后重新安装应用。
