...
1. 打开 JiuwenSwarm Web 界面，在左侧导航栏点击 **「定时任务」** 菜单
2. 进入定时任务管理页面，点击 **「创建」** 按钮（红框标注）
3. 填写任务配置表单：

![定时任务创建界面](../assets/images/cron/create_schedule_task.png)
...
![通过对话创建定时任务](../assets/images/cron/chat_create_schedule_task.png)
...
![定时任务执行](../assets/images/cron/schedule_task_demo1.png)

上图展示了定时任务触发后的执行过程。

![定时任务执行2](../assets/images/cron/schedule_task_demo2.png)
...
![工作提醒定时任务执行1](../assets/images/cron/schedule_task_demo_daily_work_notice_1.png)
...
![工作提醒定时任务执行2](../assets/images/cron/schedule_task_demo_daily_work_notice_2.png)
...
| **执行中占位** | `{任务名} is running. Results will be posted when ready (scheduled_at=…).` |
