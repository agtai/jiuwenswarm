...
- The only browser-visible schema expansion is an exact `native.audio` variant carried by the existing P2 notification RPC; document its fields in D-102 before implementation. It reuses the existing response-bound dedicated-media downlink descriptor and adds no RPC method, media control/frame type, or browser authority.
