 ...
 * connection list, the add-connection control and the deployment-default model
 ...
 * left column holds only an "add connection" card and the right column carries the
 * guidance. A user sees the shape this page will take before configuring anything,
 * so where they learn things are does not change once they have.
 */
...
export function DocsPanel({ isConnected }: { isConnected: boolean }) {
  ...
  // Default: **only the documents actually being watched**. Adoption is cheap and
  // automatic (a share is enough), so the managed list grows to everything anyone
  // ever shared, while the watched few are what a person came here to see. The
  // ones that are off are one dropdown away, and the count is on the control.
  const [filterTier, setFilterTier] = useState('apply_scoped');
  ...
  const offCount = docs.filter(
    (d) => (!filterConn || d.connection_id === filterConn) && !watches[d.doc_id],
  ).length;
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      // returns the most informative verdict.
      const out = await webRequest<AddResult>('clouddoc.add_doc', { url });
      if (out?.result === 'ok') await syncShared(null);
      setAddResult(out);
      if (out.result === 'ok') {
        setAddUrl('');
        await reload();
    ...
    <div className="flex h-full w-full flex-col px-6 py-4">
      ...
      {conf?.ask_channel === false && (
        <div
          ...
          className="mt-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-2.5 text-[13px] text-amber-900"
        >
      ...
      <div className="mt-3.5 grid min-h-0 flex-1 gap-4" style={{ gridTemplateColumns: 'minmax(340px, 430px) 1fr' }}>
        ...
        >
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-auto px-4 py-3 text-[13px]">
                ...
                conns.map((c) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      {PROVIDER_LOGOS[c.provider] && (
                        <img src={PROVIDER_LOGOS[c.provider]} alt="" aria-hidden className="h-3.5 w-3.5 flex-none object-contain" />
                      )}
                      <span className="min-w-0 flex-1 truncate font-medium">
                        {providerLabel(c.provider, c.provider_name, t)}
                      </span>
                      <span className="flex-none text-[11px] text-text-muted">
                        {t('docs.connDocs', { count: c.docs_count })}
                      </span>
                    </div>
                    <div className="mt-0.5 truncate font-mono text-[11px] text-text-muted" title={c.agent_address}>
                      {c.agent_address}
                    </div>
                    <div className="mt-1.5 flex items-center gap-3">
                      ...
                      {/* Disconnecting is a grant-lifecycle act -- every watch issued
                          under this identity stops meaning anything -- so it goes
                          through the same confirmation the panel already had. */}
                      <button
            ...
            {/* ── Adding one: one field for a path or a pasted key, plus the file
                chooser and the keys already on disk. All visible together. ── */}
            <div>
              ...
              <textarea
                ...
                className="mt-2 h-20 w-full rounded-md border border-border bg-transparent p-2 font-mono text-[11px] placeholder:font-sans placeholder:text-text-muted focus:border-[#2563eb] focus:outline-none"
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <label
                  ...
                  className="max-w-[55%] cursor-pointer truncate rounded-md border border-border px-2.5 py-1 text-xs hover:border-[#2563eb]"
              ...
              {savedKeys.length > 0 && (
                <div className="mt-2.5">
                  <div className="mb-1 text-[11px] text-text-muted">{t('docs.conn.savedKeys')}</div>
                  {savedKeys.map((k) => (
                    <div key={k.filename} className="mb-1 flex items-center gap-2">
                      <button
                        data-testid="docs-conn-saved-key"
                        className="min-w-0 flex-1 truncate rounded-md border border-border px-2 py-1 text-left font-mono text-[11px] hover:border-[#2563eb] disabled:opacity-60"
                        title={k.client_email || k.filename}
                        disabled={connBusy || k.in_use}
                        onClick={() => void submitConnection({ credentials_path: k.path })}
                      >
                        {k.client_email || k.filename}
                      </button>
                      {k.in_use ? (
                        ...
                        <button
                          className={`flex-none rounded-md border px-2 py-0.5 text-[11px] ${
                            keyDeleteArm === k.filename
                              ? 'border-red-500 bg-red-50 text-red-600'
                              : 'border-border text-text-muted hover:border-red-400 hover:text-red-500'
                          }`}
                          onClick={() => {
              ...
              {connError && (
                <div
                  ...
                  className="mt-2 rounded-r-md border-l-[3px] border-red-500 bg-red-50 px-3 py-2 text-[11.5px]"
                >
                  {connError}
                </div>
              )}
            ...
            <div className="rounded-md bg-bg-hover p-3 text-[11.5px] leading-relaxed">
              <div className="font-medium">{t('docs.conn.guideTitle')}</div>
              <div>{t('docs.conn.guideApis')}</div>
              <div>{t('docs.conn.guideShare')}</div>
            </div>
          </div>
        </aside>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-2.5">
            ...
            >
              ...
              {conns.map((c) => (
                <option key={c.id} value={c.id}>
                  {providerLabel(c.provider, c.provider_name, t)} · {c.agent_display || (c.agent_address || c.id).split('@')[0]}
                </option>
              ))}
            </select>
          )}
          ...
          >
            ...
            <option value="off">{t('docs.table.offCount', { count: offCount })}</option>
            ...
            <>
              <button
                ...
                className={`rounded-md border px-2.5 py-1 text-xs disabled:opacity-40 ${
                  armBulk === 'on' ? 'border-[#2563eb] text-[#2563eb]' : 'border-border hover:bg-bg-hover'
                }`}
        ...
        {hasConn ? (
          <>
                  ...
                  <tr className="border-b border-border text-left text-[11.5px] uppercase tracking-wide text-text-muted">
                    <th className="w-9 px-3 py-2">
                      <input
                        ...
                        className="h-3.5 w-3.5 align-middle accent-[#2563eb]"
                      />
                    </th>
                          ...
                          <td className="whitespace-nowrap px-3 py-2" onClick={(e) => e.stopPropagation()}>
                            <button
                              ...
                              className={`rounded-md px-1.5 py-0.5 text-[11px] font-medium hover:ring-1 hover:ring-border-strong ${
                                armApply === d.doc_id ? 'bg-[#eff4fe] text-[#2563eb]' : tierCls
                              }`}
                            >
            ...
            {/* ── The foot of the table: a diagnostic, not the way in ──
                Sharing a document with the connection is what puts it under
                management; discovery adopts everything the connection can edit,
                on panel load, on Refresh, and on a timer. So this box does not
                add documents -- it answers "why isn't mine here?", which
                discovery cannot answer because it never sees that document.
                The connection selector stays for the same reason: which identity
                you are asking about *is* the diagnostic question. */}
              ...
              <div className="flex items-start gap-2">
                <div className="min-w-0 flex-1">
                  <input
                    ...
                    disabled={addBusy}
                    data-testid="docs-diagnose-input"
                    className="w-full rounded-lg border border-border bg-transparent px-3.5 py-2 font-mono text-xs placeholder:font-sans placeholder:text-text-muted focus:border-[#2563eb] focus:outline-none"
                  />
                </div>
              </div>
              {adopted > 0 && (
