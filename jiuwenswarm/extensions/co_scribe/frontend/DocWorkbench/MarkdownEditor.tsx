...
export function MarkdownEditor({ tab, nonce, highlightLines, onSaved }: {
  ...
  // Switching documents starts clean; a reload of the same document does not.
  ...
  const load = useCallback(async () => {
    ...
    setText(out.text ?? '');
    // Not cleared here: a save bumps the receipt nonce, which reloads, which used
    // to wipe the "saved, receipt xxxx" line a second after it appeared -- the one
    // thing telling the person their edit is in the ledger. The reload keeps it;
    // switching documents clears it.
  }, [tab.docId, t]);
