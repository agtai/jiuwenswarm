 ...
 * edit (a share or a trash changed no text), and the document has to be one the
 * workbench can actually aim at: a markdown file, whose text the editor searches
 * for the changed lines, or a receipt carrying the platform's own anchor
 * (``#gid=…&range=…``, ``#slide=id.…``), which the frame reloads onto.
 *
 ...
 */
export function canLocate(kind: string | undefined, r: ReceiptRow): boolean {
  if (r.status !== 'applied' && r.status !== 'applied_unverified') return false;
  if (r.op && r.op !== 'edit') return false;
  return kind === 'markdown' || !!receiptAnchor(r);
}
 ...
 *
 * Distinct from ``activeToolDocId``, which answers "most recently touched" and
 * is the right question for a receipt. This one backs a line that reads
 * "editing: X", so it has to be true in the present tense: a writing tool, still
 * in flight. It deliberately has no fallback to the focused tab -- naming the
 ...
 *
 * The workbench's markdown editor saves by replacing the file, so the ledger
 * honestly records (whole old text, whole new text) -- that is what happened.
 * Every surface that renders an edit as a fragment has to know the difference,
 * or it prints the entire document as a one-line summary and locates the whole
 * file as "the change". One predicate so the two surfaces cannot drift apart.
 */
 ...
 *
 * Line granularity rather than character: the two consumers are a markdown line
 * highlight and a one-line summary, and a fragment that starts mid-line reads
 * as noise in both. A pair that is not whole-document is returned untouched --
