...
export class RealtimeDuplexSession {
  ...
  cancelToolTask(jobId: string, callId: string): void {
    ...
    this.pendingNotices.push(`任务停止确认：${JSON.stringify({job_id: jobId, status: 'cancelled', stopped: true})}。该任务已取消，不要说任务完成；其他任务不受影响。`);
    this.dispatchQueuedToolResult();
  }
    ...
    if (question) {
      ...
      this.send({ type: 'conversation.item.create', item: {
        type: 'message', role: 'user', content: [{ type: 'input_text', text:
          `后台 Agent 交互通知（不是新任务）：${JSON.stringify(question)}。请向用户说明是哪项任务在提问并转述问题；等用户回答后，先查询问题是否仍有效，再调用 jiuwen_task_answer。不要自行回答或宣称完成。`,
        }],
      } });
      this.requestResponse();
      return;
