    ...
    return task_management_tools() + [
        {
            ...
            "function": {
                ...
                "description": (
                    ...
                    " 用户明确要求后台 Agent 工作时必须调用。若要求 Agent 先询问预算或其他信息，"
                    "将提问和等待回答的要求包含在 task 中立即委托，不能自行问问题代替创建任务。"
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
...
def parse_qwen_omni_tool_call(value: Any) -> QwenOmniToolCall:
    ...
    if type(arguments.get("independent", False)) is not bool:
    ...
    specs = [
        ...
        (
            ...
            ),
            {
                ...
                "answers": {
                    "type": "array",
                    "description": "Plain text answers in observed question order, e.g. [\"5000元\"]. Answer every question exactly once; never copy or paraphrase the question.",
                    "minItems": 1,
                    "maxItems": 32,
        ...
        (
            ...
            ),
            {
                ...
                "status": {"type": "string", "enum": ["queued", "running", "waiting_user", "cancelling", "unknown", "completed", "failed", "cancelled", "unfinished"]},
            },
            [],
        ),
        (
