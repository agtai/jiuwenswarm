...
def ground_user_instruction(instruction: str, tool_context: str = "") -> str:
    ...
    return (
        ...
        '独立新任务可使用 jiuwen_delegate，arguments 包含 task、independent:true、depends_on:前置任务ID数组、resources:冲突资源数组。'
        '只有不依赖其他工作的独立任务才声明 independent；无共享资源影响的天气查询等可用 resources:[]，文件使用规范绝对路径，同资源顺序执行。'
        '不确定资源时省略 resources；不确定独立性时保持普通委托。修改已有任务不能重新委托。'
