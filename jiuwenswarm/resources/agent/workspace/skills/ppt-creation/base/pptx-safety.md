...
- 官方模板原始画布为 当前模板自身记录的 EMU 尺寸（本版约 13.333 × 7.50 英寸）。默认合并流程会等比放大坐标与字号，并把内容页挂到官方 `slideLayout7.xml`；不要手工再缩放一次。
...
- 交付前运行 `audit_pptx.py`；模板合并模式丢失相对于实际模板的 Master / Layout、画布不一致、显式字号低于 absolute floor 或标准内容页缺总结条都属于硬错误。
