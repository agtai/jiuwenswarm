...
class TaskService:
    ...
    def kick(self):
        ...
        active = [
            t
            for t in tasks
            if t["status"] in {"running", "cancelling", "unknown", "waiting_user"}
            or t["id"] in self.workers
        ]
    ...
    def submit(self, owner, session, command_id, instruction, request=None):
        ...
        if type(request.get("independent", False)) is not bool:
    ...
    def reorder(
        self, owner, session, task_id, revision, before_id=None, *, command_id=None, command_revision=None
    ):
        self.start()
        with self.store.transaction() as db:
            task = self.store.get(db, task_id, owner, session)
            if command_id is not None:
                replay, fingerprint = self.store.replay(
                    ...
                    session,
                    command_id,
                    ["reorder", task_id, revision if command_revision is None else command_revision, before_id],
                )
                if replay:
                    return replay
            if task["status"] != "queued":
    ...
    async def _send_notifications(self, key):
        try:
            while key in self.pending_notifications:
                ...
                try:
                    await asyncio.wait_for(self.on_change(task), timeout=2)
                except Exception:
                    pass  # Saved records remain authoritative if transport fails.
    ...
    async def _drain(self, task_id):
        ...
        with self.store.transaction() as db:
            ...
            active = [
                ...
                for t in tasks
                if t["id"] != task_id
                and t["status"] in {"running", "cancelling", "unknown", "waiting_user"}
            ]
