...
_INSPECT_ONLY = frozenset({"which", "command", "type", "ls", "stat", "file", "echo", "printf"})
...
def _is_guarded_cli_command(command: Any) -> bool:
    ...
    if head not in _PLATFORM_CLIS:
        # The binary is named but not run here (an interpreter, xargs, a script):
        # nothing can be judged, so nothing is admitted.
        return True
    rest = argv[1:]
    if not rest:
