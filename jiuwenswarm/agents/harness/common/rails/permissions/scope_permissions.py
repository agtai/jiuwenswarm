...
the runtime, at the tool call -- which is the only place that knows the tool and
its arguments, and the only place a refusal can be turned into something the
model reads. Both processes import the same ``common/scopes`` package, so the
...
The conversation comes from two ContextVars the request handler sets, not from
the ``PermissionContext`` beside this module. That is deliberate:
``setup_permission_context`` returns ``None`` for an ordinary turn -- it builds a
context only for the digital-avatar scene, or when memory is off -- so an
ordinary Slack conversation has no ``PermissionContext`` at all and a rule hung
off one would never fire for the conversations it was written for.
``TOOL_PERMISSION_CHANNEL_ID`` is set unconditionally at every runtime entry
point, and ``TOOL_PERMISSION_CHAT_ID`` is set beside it.
"""
...
# recompiles once and says its piece once.
#
...
# ``scopes:`` list, and a key that watched only the list would serve the old
# answer until something else happened to change.
_compiled_key: "str | None" = None
_compiled: tuple[Scope, ...] = ()


#: What both entry points say when a scope cannot be settled, and why it is not
#: a refusal. Both are answers to a callback agent-core owns: the scene hook and
#: the permissions snapshot. Neither had any way to fail before this section
#: existed, and neither has a caller that would catch one, so an exception from
#: here does not fall back to the operator's permission block, it takes down the
#: turn. The permission config is evaluable without any scope, and it is what a
#: deployment that has written no scope is already running on.
_UNAVAILABLE = (
...
def runtime_scopes() -> tuple[Scope, ...]:
    ...
    ``compile_scopes`` warns about what it cannot honour rather than raising,
    but that is a property of the module next door and not something this one
    may assume, and the two callers below have no guard of their own.
    """
    ...
    try:
        ...
        return ()
    # Published together, so a reader never sees a key that names something the
    # compiled tuple beside it is not.
