...
class FeishuDocsProvider(DocProvider):
    ...
    async def unshare_document(self, doc_ref: DocRef, address: str) -> None:
        member_type = self._member_type_of(address)
        # High-risk in lark-cli's rating, like adding a member; the confirmation it
        # asks for was given at the tool's own gate (or is the revert of a share the
        # person approved), so ``--yes`` is passed here as it is in share_document.
        await self._drive_json(doc_ref, lambda ftype: [
            "drive", "+member-remove", "--token", doc_ref, "--type", ftype,
            "--member-id", address, "--member-type", member_type, "--yes",
        ])
    ...
    async def restore_document(self, doc_ref: DocRef) -> None:
        # No open-platform call brings a file back from the recycle bin (lark-cli 1.0.93
        # has no such verb). Said plainly rather than faked: the receipt stays
        # ``applied`` and the person restores it in Feishu's own recycle bin.
        raise ProviderError(
            "unsupported",
            "飞书没有从回收站恢复的开放接口，请在飞书「回收站」中手动恢复该文档。",
        )
