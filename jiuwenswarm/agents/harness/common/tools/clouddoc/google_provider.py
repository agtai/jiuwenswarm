...
class GoogleDocsProvider(DocProvider):
    ...
    async def restore_document(self, doc_ref: DocRef) -> None:
        _, drive = self._clients()
        await self._call(
            drive.files().update(fileId=doc_ref, body={"trashed": False}, fields="id").execute
        )

    async def unshare_document(self, doc_ref: DocRef, email: str) -> None:
        # Drive addresses a permission by its own id, not by the address it was granted
        # to, so the list is read first. A permission that is not there is left alone:
        # the state the caller asked for -- this address cannot open the file -- holds.
        _, drive = self._clients()
        listing = await self._call(
            drive.permissions()
            .list(fileId=doc_ref, fields="permissions(id,emailAddress,type)")
            .execute
        )
        want = email.strip().lower()
        for perm in (listing or {}).get("permissions") or []:
            if str(perm.get("emailAddress") or "").lower() == want:
                await self._call(
                    drive.permissions().delete(fileId=doc_ref, permissionId=perm["id"]).execute
                )
