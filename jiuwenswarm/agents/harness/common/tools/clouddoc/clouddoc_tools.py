...
#
# **Nine, not the seven the design says.** §3.3's as-built list was written when there
# were seven and PR2a added the workmode pair without updating the count -- which is the
# same drift this constant exists to stop, caught the first time the list was written
# down in one place and compared with what the toolkit actually builds.
...
class CloudDocToolkit:
    ...
    async def batch_edit(
        ...
        * **writing into a document that is not empty without an anchor.** An empty
        ...
        if self._mandate and getattr(self._provider, "receipt_sink", None) is None and not self._ask_channel():
            return _fail(
                "回执通道不可用，这次写入将不可回退（降为不可逆写），而当前会话是 "
    ...
    async def trash_document(self, doc_id: str | None = None) -> dict:
        ...
        On Google the receipt's revert restores it; on Feishu the platform has no
        restore call and the tool result says so before the act, so the person can
        ...
        if rid:
            sink.commit(rid, revision_after=None)
        owner = getattr(self._provider, "owner", None)
        prov = owner(str(doc_id)) if owner else self._provider
        restorable = getattr(prov, "kind", "") == "google"
        return _ok(
            doc_id=doc_id, receipt_id=rid,
            detail=("已移入回收站。" + ("可从 Docs 面板的履历回退（恢复）。" if restorable
                    else "该平台没有恢复接口，如需恢复请让用户在平台的回收站里手动恢复。")),
        )
    ...
    async def write_region(
        ...
        **Chat only.** On this path the person is here and their instruction is the
        ...
        if self._mandate and getattr(self._provider, "receipt_sink", None) is None and not self._ask_channel():
            return _fail(
                "回执通道不可用，这次区域写入将不可回退（降为不可逆写），而当前会话是 "
                "Full Access——没有确认通道，按底线拒绝。请修复工作区 config 目录的"
                "回执存储，或把会话权限切回逐项确认（default）。"
            )
    ...
    def get_tools(self) -> list[Tool]:
        ...
        return [
            ...
            make(
                ...
                "Google 上可从 Docs 面板履历回退恢复，飞书上没有恢复接口，需用户手动恢复——"
                "结果里会说明，原样转达。",
                {
                    "type": "object",
            ...
            make(
                ...
                "**回退某条回执只能由用户在 Docs 面板的修改履历里点「回退这批」**，"
                "你没有回退工具，用户要回退时告诉他去面板。"
            ...
            make(
                ...
                "自动留回执（可回退）。仅无人值守回合可用；doc_id/comment_id 由系统绑定，"
