...
class DocProvider(ABC):
    ...
    # The lifecycle inverses. Not abstract: a platform that lacks one refuses it with
    # ``unsupported`` and the receipt machinery reports the revert as impossible,
    # which is the honest outcome -- a provider forced to implement what its platform
    # cannot do would have to fake success.
    async def trash_document(self, doc_ref: DocRef) -> None:
        """Move the document to the platform's trash (the inverse of ``create``).
        The document keeps its id and can come back through ``restore_document``
        where the platform allows it."""
        raise ProviderError("unsupported", "该平台不支持移入回收站。")

    async def restore_document(self, doc_ref: DocRef) -> None:
        """Bring a trashed document back (the inverse of ``trash``)."""
        raise ProviderError("unsupported", "该平台不支持从回收站恢复。")

    async def unshare_document(self, doc_ref: DocRef, email: str) -> None:
        """Take one address's access away (the inverse of ``share``). Removing an
        address that has no permission is not an error: the state asked for holds."""
        raise ProviderError("unsupported", "该平台不支持取消共享。")
