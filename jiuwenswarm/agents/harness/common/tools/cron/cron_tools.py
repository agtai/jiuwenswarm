...
class CronTools:
    ...
    async def create_job(self, params: dict[str, Any]) -> Any:
        ...
        if isinstance(sid, str) and sid.strip():
            ...
            # it was read off the route, not off the params -- so provenance is
            # whatever the request itself can show. The check is still run
            # rather than assumed, so that this path and the gateway RPC path
            # answer the same question with the same rule.
    ...
    async def update_job(self, job_id: str, patch: dict[str, Any]) -> Any:
        ...
        if "session_id" in normalized_patch:
            # Re-established from the patching request, exactly as at creation.
            # The store clears the flag on any session change, so leaving this
            # out would quietly revoke a job's history access on an unrelated
            # edit -- and setting it without checking would grant access the
            # request cannot back.
