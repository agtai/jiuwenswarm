...
the ranking is Slack's. The pair is named by scope for that reason: one reads *a
conversation*, the other searches *the workspace*. They share a vocabulary on
purpose: a result names its ``chat_id``, ``chat_name`` and ``ts``, which are the
words the common message schema and the history tool already use, so a result
handed from one tool to the other needs no translation. Neither is the other's
degraded mode, so nothing in this module consults the history toolkit, shares
its gate, or falls back to it: a caller that cannot use search is told why, and
never quietly served history instead.

**Results are references, never content.** This is the organising decision of
the module rather than a matter of taste, so it is stated before anything about
how the thing works.
...
* in the checkpoint database, as pickled agent state carrying the tool call and
...
on disk by construction, and no filter on a write path can change that. The one
place such a filter could be inserted is the transcript append, which covers
the first sink and silently misses the second: a per-tool "do not persist" flag
would test clean against ``history.jsonl`` while the checkpoint went on holding
the same bytes. Nor is any of it short-lived -- there is no expiry, rotation or
cap on either sink anywhere in the deployment, and the redaction filter in
``jiuwenswarm.common.utils`` is a logging filter that never sees persisted
history at all. Neither is offered here as mitigation; they are the reason a
storage-side answer does not exist.

That leaves exactly one lever, and it is this module's output. Returning
references reaches every sink because it changes *what the model sees* rather
than where a result is written. Suppressing the result instead is worse than
useless: the model would forget what it had found the moment the turn ended,
and no follow-up question would work.

So a result carries what identifies and locates the thing, and never what
quotes it. The body a model needs is fetched rather than stored -- it follows
the ``permalink``, or reads that conversation with ``read_slack_conversation``
using the identifiers this tool returned. Content still reaches the model; it
arrives from a tool entitled to serve it, and nothing retrieved from the search
API lands in a file on the way. That is also how the two tools were always
meant to compose.
...
This is a proportionate reading and not compliance by construction, and saying
so is part of the decision. A strict reading of the same clause forbids
retaining a permalink too. The clause sits beside "may not use any of this data
for training" and "may not scrape data unrelated to user queries", so its
evident subject is retention and reuse of content rather than the existence of
a pointer, and a pointer the model must dereference through a permitted path is
a different object from a copy. That judgement is written here so it can be
disagreed with, rather than inferred from the code.
...
* Slack mints an ``action_token`` per inbound event and a bot-token call to
  ...
  event boundary and carries it on the same trusted request metadata that
  already carries the conversation; it is never a model-supplied argument.
* A cron run synthesises its metadata from a stored job. It has no inbound
  event, therefore no token, therefore no search -- structurally, for every
  scheduled turn, and that is reported rather than worked around.

Which conversations a search may reach is Slack's decision and not ours.
Slack scopes every call by the conversation it was invoked from: asked in a
public channel it returns public results alone, *regardless of whether the app
holds more granular scopes*; asked in a private channel or a group DM it
returns that conversation and public channels; asked in a Slack Connect
channel, only that channel. That rule is enforced inside Slack, upstream of
anything written here, so this module neither restates it in the request nor
re-checks it on the way out. An earlier version did both, and the only thing
either could achieve was to drop a result the caller was entitled to -- a
search from a direct message could not find that direct message.
...
Only cross-cutting primitives are borrowed from ``slack_history``: the pass that
keeps a credential out of a tool result, and the rule for when a refusal is
worth retrying. Both are security- or correctness-critical and a fix to one
copy would not reach a second. The redaction was borrowed by halves for a while
-- the regexes imported, the twenty-five lines driving them copied -- which left
exactly the gap the import was reasoned about to close, so it is now one
function and this module supplies its own token and its own cap. No gate, no
availability check and no data path is shared with it.
"""
...
from jiuwenswarm.common.slack_history_policy import slack_config
...
# The Slack method this tool is. Named once, because it appears in the call and
# in nothing else: what the tool is *for* is described in capability terms on
# the card below, where a model reads it.
_SEARCH_METHOD = "assistant.search.context"

# Where the connector leaves the per-event token. The name ends in ``token`` on
# purpose: the log redaction patterns in ``jiuwenswarm.common.utils`` mask any
...
# The operator toggle, under ``channels.slack``. Off by default: the method has
# never returned ``ok`` from this deployment, it needs a scope a typical install
# does not grant, and it may need a paid plan tier. An operator turns it on
# having decided those hold.
...
# result carries a message body -- but a ceiling on names, titles and the like:
...
# The conversation kinds Slack will accept as a filter. Passing none of them is
# the default and the right default: Slack already decides the audience from
# where the call was made, and this argument can only narrow that further. It
# is exposed so a caller can say "only public channels" when that is genuinely
# what they want, not to reinstate a rule of ours.
...
# Its own case, and not one of the refusals above, because it is the only one an
# operator fixes in a minute. Reporting it as "this workspace does not offer
# search" would frame a checkbox and a reinstall as a missing product.
_MISSING_SCOPE_ERROR = "missing_scope"

# Which scope each part of a request needs. The history tool never needs a table
# like this -- the failing method names its own scope and there is nothing to
# infer. Here one method serves four content types and four channel types, so
# ``missing_scope`` alone says nothing about *which* of the six is absent, and
# working it out from what the call asked for is the only way to name it.
...
def _echoed_scopes(value: Any) -> list[str]:
    ...
    carrying ``needed`` and ``provided``, usually comma-separated strings. This
    has **not** been observed from this method: every scope is granted on our
    token, so the refusal is unreachable without revoking one. So this accepts a
    string or a list, tolerates neither being present, and bounds what it keeps
    -- and nothing anywhere claims Slack does send them.
    """
...
def slack_search_request_metadata(
    ...
    * The metadata carries an action token. This is what excludes a cron run
    ...
    * The operator has turned the tool on.
    ...
    Deliberately *not* a condition: ``slack_history_digest_allowed``. That
    allow-list exists to say which conversations may have their record read out;
    search reads no conversation's record, and which conversations it may reach
    is already settled by Slack from where the request was made, so borrowing
    history's gate would couple two tools whose whole point is that they are
    decided separately.
    """
...
class _FieldReader:
    ...
    name are all authored, and any of them can carry a credential, so the
...
class SlackSearchToolkit:
    ...
    def _get_client(self) -> Any:
        ...
        if AsyncWebClient is None:
            raise RuntimeError("slack_sdk_unavailable")
    ...
    def _redact(
        ...
        The pass itself is ``slack_history``'s, imported for the reason its
        regexes already were: it is security-critical, and a credential shape
        added to one copy would not have reached the other.
        """
    ...
    async def _search(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        while True:
            ...
            if data.get("ok", True) is False:
                ...
                # retry. This one carries no Retry-After to read.
    ...
    def _channel_record(
        ...
        ``topic`` and ``purpose`` are kept, and that is the one judgement call
        in these four reductions, so here is the reasoning. They are not
        conversation content. They are the channel's own description of itself,
        the text Slack shows in its channel browser to someone who has not
        joined -- the same class of thing as the name, and effectively a longer
        name. Nothing said *in* the channel appears in either.

        They are also the whole value of searching channels at all. The only
        question this content type answers is "where should I ask about X", and
        a result reading ``#proj-atlas`` with a link says nothing about whether
        Atlas is the billing rewrite or the office move. Dropping them would
        leave the type technically present and practically useless.

        They are still authored text, so they are redacted and capped like any
        other field, at the 250 characters Slack itself allows for a topic.

        ``channel_type`` is kept for the same reason as ``is_archived``, and it
        is the sharper of the two. Both say whether the channel can actually be
        used: an archived one cannot be posted to, and a private one cannot be
        joined by someone not already in it. Sending a person to a conversation
        they have no way to enter is a wrong answer that reads exactly like a
        right one, and neither the name nor the topic gives any clue.
        """
        field = _FieldReader(raw, self._redact)
        record = {
    ...
    def _user_record(
        ...
        ``email`` is dropped, and deliberately. Slack returns it without being
        asked, and a persisted directory record is a different exposure from a
        persisted message: it is durable, it identifies someone outside Slack,
        and it is reusable in a way that a line of chat is not. Nothing this
        tool exists to answer needs it -- ``user_id`` addresses the person
        inside Slack, which is the only place this agent talks to them. The
        same reasoning drops the profile picture, the timezone and the job
        title, none of which help locate anything.
        """
        field = _FieldReader(raw, self._redact)
        record = {
    ...
    async def search_slack_workspace(
        ...
        if not action_token:
            ...
            # a scheduled job above all -- never had a token to carry, and a
        ...
        # Slack answers with a dict keyed by content type and always carries all
