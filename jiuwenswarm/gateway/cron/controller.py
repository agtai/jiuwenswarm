...
def _warn_if_slack_session_unproven(
    *,
    ...
    history tools at run time, and a silent loss of a capability reads as a bug
    in the tools rather than as the gate doing its job. This is the only place
...
class CronController:
    ...
    async def create_job(
        ...
        *,
        ...
        which is what the web and TUI panels genuinely are with respect to Slack,
        and unknown means untrusted: such a job is still created and still
        delivers wherever its ``session_id`` points, it simply carries no Slack
        context the runtime's history gate will honour.
        """
        ...
        # dict that carries ``session_id`` is caller-supplied on every RPC path,
        ...
        # The sibling question, asked in the same breath and answered off the
        # same string: trust decides what a job may *read*, reachability decides
        # whether it has anywhere to *write*. A job can be untrusted and perfectly
        # deliverable, or trusted and -- after a session change -- not.
        warn_if_slack_cron_delivery_unreachable(
            targets=targets,
    ...
    async def update_job(
        ...
        *,
        ...
        A caller-supplied ``slack_session_trusted`` is dropped unread for the
        same reason ``create_job`` ignores one: it would be a way to claim a
        provenance nobody checked. The store clears the flag on any session
        change, and the value re-established here -- from the patching request's
        own channel and session -- is the only one that can put it back.
        """
        patch = dict(patch or {})
        allow_unresolved_project_id = bool(
        ...
        if "session_id" in patch:
            ...
            if str(new_session_id or "").strip() == str(existing.session_id or "").strip():
                # The session is being written back unchanged -- a targets-only
                # edit reaches here through the branch above. The store clears
                ...
                # its history access, which is not a decision anybody made.
