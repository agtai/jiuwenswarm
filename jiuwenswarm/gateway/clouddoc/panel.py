...
class CloudDocPanel:
    ...
    async def write_doc(
        ...
        ledger with the person as executor and can be reverted like anything else --
        which is what keeps a later agent revert from colliding with an unrecorded
        human change. Only markdown is editable here; every other kind has the
        platform's own editor in the frame. The pair is (whole old text, whole new
        text): the anchor criterion then holds trivially, and the receipt's revert
        writes the old text back in full.
        """
        conn = self._reg.find_doc(doc_id)
        if conn is None:
            return {"result": "not_watched"}
    ...
    async def revert(self, receipt_id: str) -> dict[str, Any]:
        """Revert-by-receipt. The mechanics -- eligibility, the anchor criterion in
        both forms, the inverse's application and the chain -- live in the toolkit's
        ``revert`` module (§25.5, cut ⑤); the panel resolves the connection, clears
        inherited highlights, notifies the answered threads and words the outcome.

        **Shielded from the caller's cancellation.** The web channel runs each request
        in its own task and cancels it when the socket closes. A revert on Feishu is
        several CLI round-trips, and a browser tab closing in the middle cancelled the
        task after the platform had taken the write and before the ledger recorded it
        (measured 2026-09-03: the cell read reverted, the revert receipt stayed
        pending, the original stayed applied). A write that has started must run to
        its ledger entry whoever is still listening.
        """
        return await asyncio.shield(self._revert_unshielded(receipt_id))

    async def _revert_unshielded(self, receipt_id: str) -> dict[str, Any]:
        from jiuwenswarm.agents.harness.common.tools.clouddoc.receipts import ReceiptStore
        from jiuwenswarm.agents.harness.common.tools.clouddoc.revert import execute_revert

        store = ReceiptStore()
        pre = store.get(receipt_id)
        doc_id = (pre or {}).get("doc_id", "")
        conn = next(
            (c for c in self._reg.list()
             if doc_id in (getattr(c.watcher, "_docs", []) or [])),
            None,
        ...
        outcome = await execute_revert(prov, store, receipt_id)
        if not outcome.ok:
            return {"ok": False, "detail": outcome.detail}
        r = outcome.receipt or {}
        cleared: dict[str, Any] = {}
        if r.get("highlight"):
            # The restored text may inherit yellow from its neighbors (measured
            # limitation): clear the background over the restored spans, fail-soft.
            try:
                cleared = await prov.clear_highlight(doc_id, [e["old"] for e in r["edits"]])
            except Exception:  # noqa: BLE001
                logger.exception("[clouddoc] revert 后撤高亮失败 doc=%s", doc_id)
        notified = await self._notify_reverted_threads(prov, doc_id, r)
        return {
            "ok": True, "detail": outcome.detail,
            "cleared": cleared, "notified": notified,
        }

    async def _notify_reverted_threads(
        self, prov: Any, doc_id: str, receipt: dict[str, Any]
    ) -> list[str]:
        """Tell every thread the reverted batch answered that it was undone.

        This is what the receipt's many-to-many map is for (D5). One edit may answer
        several comments on the same passage, and the people who wrote them are reading
        their own threads, not the panel: an edit that appears and later disappears
        with nothing said in the thread reads as the agent changing its mind on its own.

        Fail-soft, one thread at a time. The revert itself has already landed and is
        the thing that mattered; a reply that cannot be posted must not turn a
        successful undo into a reported failure. Which threads were reached comes back
        to the caller, so the panel can say so rather than implying all of them were.
        """
        # The mapping lives per edit in the receipt: the provider folds
        # for_comment_ids_by_old into each edit as it writes the WAL entry.
        wanted: list[str] = []
        for edit in receipt.get("edits") or []:
            for cid in (edit or {}).get("for_comment_ids") or []:
        ...
        # Language follows the thread being written to. Listing the comments is one
        # more call and fail-soft like the rest; when it yields nothing the receipt's
        # own edit text stands in, since an edit follows the document's language.
        samples = await self._comment_texts(prov, doc_id)
        fallback = " ".join(
            str((e or {}).get("new") or (e or {}).get("old") or "")
            for e in receipt.get("edits") or []
        )
        notified: list[str] = []
        for cid in wanted:
            try:
                await prov.reply_comment(
                    doc_id, cid,
                    msg("reverted_notice", samples.get(cid) or fallback),
                )
                notified.append(cid)
            except Exception:  # noqa: BLE001 - one thread failing is not a failed revert
                logger.exception(
                    "[clouddoc] 回退后通知线程失败 doc=%s comment=%s", doc_id, cid
                )
        return notified

    @staticmethod
    async def _comment_texts(prov: Any, doc_id: str) -> dict[str, str]:
        """Comment id -> body, for picking a reply language. Empty on any failure."""
        lister = getattr(prov, "list_comments", None)
        if lister is None:
            return {}
        try:
            comments = await lister(doc_id, include_resolved=True)
        except Exception:  # noqa: BLE001 - a wording choice must not fail the revert
            return {}
        return {c.comment_id: (c.content or "") for c in comments or []}
