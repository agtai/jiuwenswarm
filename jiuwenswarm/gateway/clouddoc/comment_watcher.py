    ...
    async def _tick_doc(self, doc_id: str, summary: dict) -> None:
        ...
        for t in find_triggers(
            ...
            if self._registry is not None:
                ...
                # No registry to consult: the strictest tier, as every other
                # missing-or-unknown default in this feature resolves. This was the
                # one place that defaulted the other way.
                turn_mode = "reply_only"
                logger.warning(
                    "[clouddoc] 无授权注册表，doc=%s 的回合按最严档 reply_only 派发", doc_id
                )
