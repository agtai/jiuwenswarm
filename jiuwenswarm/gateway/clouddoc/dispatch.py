...
class CloudDocDispatcher:
    ...
    def _configured_model_name() -> str:
        ...
        Validated on the way out, not only on the way in. ``set_model`` checks the
        name it writes, but a config file is editable by hand and survives every
        upgrade, so a value that no longer resolves -- a renamed model, a removed
        one, an unexpanded ``${VAR}`` left by a template -- would otherwise be
        handed to the agentserver as if it were real, and every unattended turn on
        this deployment would ask for a model that does not exist. An unusable
        value reads as unset: empty means the deployment default, which is the
        outcome the owner gets anyway and the only one that still works.
        """
        try:
            from jiuwenswarm.common.config import get_config

            section = get_config().get("clouddoc") or {}
            name = str(section.get("model_name") or "").strip()
        ...
        try:
            ...
            return validate_cron_model(name) or ""
        except ValueError:
            logger.warning(
                "[clouddoc] clouddoc.model_name=%r 解析不到模型，本回合改用部署默认模型；"
                "请在 设置 → 云文档 重新选择，或清空该项",
                name,
            )
            return ""
