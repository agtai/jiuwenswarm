        ...
        toolkit = self._toolkit_factory(
            {
                ...
                # other, which is exactly what ``origin`` names. Stamped rather
                # than left absent because the toolkit reads an absent policy as
                # *no side that has the configuration settled this request* and
                # refuses -- correctly for a request that arrived from somewhere
                # unknown, and wrongly here, where the connector itself is the
                # caller and the conversation is its own.
                #
                # Deliberately not ``channels.slack.history``. That key governs
                # what the *model* may ask for; this is the connector filling
                # its own context cache for a conversation it is answering in,
                # and holding it to a key about tool reach would turn a setting
                # about the tool into a silent loss of context elsewhere.
