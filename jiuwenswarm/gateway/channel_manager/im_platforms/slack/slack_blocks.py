...
per-message splitter for them -- so a message that will not fit inside Block Kit's
limits is reported as unrenderable and the caller posts it exactly as it would
have before this module existed. Truncating instead would drop content the user
was told had been delivered.
...
Size decided it once, through a row threshold, and no longer does. The two
blocks differ in what they can do as well as in how much they hold, so a row
count could pick between them only by accident -- an operator who wants sorting wants it for the
tables worth sorting, which is not the same set as the tables that are long.
What the threshold was really holding off was ``data_table``'s ``page_size``
default of 5, which would have turned a twelve-row table into five rows and a
pager. That is held off directly now, by ``DATA_TABLE_PAGE_SIZE`` and
``_page_size``: Slack's default is never sent, and neither is a page smaller
than the table it holds.
...
The raised budget is not reachable on every path, and the difference is not this
module's to fix. The connector splits a reply into chunks before it offers any
of them here, and the first chunk of a *streamed* reply is cut at the 4,000
characters ``chat.update`` accepts rather than the 38,000 a fresh post does. A
table cut at that boundary loses its delimiter row, ``_parse_table`` declines
the half that no longer has one, and it reaches the channel as raw pipes. So
20,000 characters is the budget on a fresh post and a table over roughly 4,000
still degrades on the opening chunk of a streamed one. Raising the ceiling here
buys nothing there; only splitting text after rendering rather than before it
would, and the splitter belongs to the connector.
...
``xychart-beta`` is beta, its syntax has moved between mermaid releases, and
supporting it anyway is a decision taken with that in front of us. The risk it
carries is bounded by the one rule every parser here follows: **an unrecognised
shape is declined, never guessed at**. A future mermaid that changes the syntax
does not produce a wrong chart -- it produces a fence that falls through to
source, where the author can see what they wrote and that it was not drawn. The
name is itself the warning, and it is on screen in the reply. The alternative,
leaving mermaid's only bar and line charts unreachable from the portable tier
because their name ends in ``-beta``, costs more than it saves.

The syntax has in fact already moved twice, and both moves are handled -- see
``_XYCHART_OPEN_RE`` and ``_XYCHART_PLOT_RE``, which name the versions.
...
opposite things: one is a ```` ```python ```` fence behaving exactly as intended,
the other is an author who asked for a drawing and did not get one. Both look
identical in the channel -- source on screen -- which is what let a chart that
quietly failed to draw go unnoticed until someone thought to compare specs.
...
Which block types a ``blockkit`` fence may carry is the operator's to decide --
...
allow-list says -- see ``INTERACTIVE_TYPES``. Tier is a portability choice an
author may make for themselves, but an author may not put a forged approval
prompt in front of a reader in a channel where the real ones live.
"""
...
# Declining is designed behaviour and never costs the reader the content, but it
# is invisible from outside: the fence reaches the channel as source and looks
# exactly like source an author meant to write. An operator asked "why was my
# chart not drawn?" had nothing to read. This is the only side effect in the
# module and it stays one -- ``logging`` is the standard library, so nothing
# here is any harder to test or lift than it was.
...
# into the pair above because one message can carry both kinds and Slack holds
...
#                   arrived as, which is what a reader saw before this module
#                   existed and what an unparseable table still falls back to.
...
# holds 100 and 10,000, and a table over those limits is not truncated here but
# declines the whole message back to raw pipes. Its one readability objection --
# Slack paging five rows at a time -- is answered by ``_page_size`` rather than
# by choosing the weaker block, so nothing a plain ``table`` shows is missing
# from what an operator gets by leaving this alone.
...
# Which block types a hand-written fence may carry, and the first of the two
...
# it, and a name missing from a hard-coded list cost an author the whole
# message's rendering for a block Slack would have drawn. An operator who wants
# a narrower surface configures one; nothing is narrowed on their behalf.
#
# This is deliberately *not* where a forged approval prompt is stopped. Type
# names answer a portability question -- which blocks this workspace's clients
# draw well -- and answering a safety question with them is what tied the two
# together before. The safety question is below, and is answered on its own.
...
# carries no ``action_id`` at all -- Slack generates one for a button that omits
...
# every other renderer that reads the reply. Requiring a marker beside it made
# the common case the one that needed extra words, and the marker had to be
# taught to every emitter before any of them could use it.
#
...
# What a rendering was built from, carried alongside the blocks so the connector
...
def combine_block_kinds(kinds: Collection[str]) -> str:
    ...
    One Slack message carries one payload and Slack refuses it whole, so a chunk
...
# chart carries up to twelve series, each of which must hold exactly one point
...
# Stands in when a chart carries no title and sits under no heading.
...
# destination; a link always carries a scheme and none of those do.
...
def render_blocks_with_kind(
    ...
    *,
    ...
    configured as ``basic``. It is a keyword with a default so that a caller
    which has no opinion keeps the behaviour it had.
    ...
    ``allowed_block_types`` restricts what a ``blockkit`` fence may carry.
...
def _segment(
    ...
    *,
    ...
    ``tables=False`` is ``render_tables: off``, and it is expressed by not
    looking for a table rather than by discarding one afterwards. That is what
    makes the mode cost nothing else: the lines fall through to prose exactly as
    a table with no delimiter row already does, so a reply whose only structure
    was a table produces no blocks at all and is posted as the text it always
    was, while a chart fenced beside it still renders on its own language.
    ...
    whose contents turn out to be unusable, stays prose exactly as it always did
    -- which is what makes a fenced pipe table sample text rather than a table,
    and what leaves a broken chart on screen as the source the author can see is
    broken.
    """
    allowed = _normalised_block_types(allowed_block_types)
...
def _fenced_blocks(
    ...
    *,
    ...
    entirely different things: one is a ```` ```python ```` fence behaving
    exactly as intended, the other is an author who asked for a drawing and did
    not get one. Only the second is worth an operator's attention, and until
    this line existed neither was distinguishable from the outside -- the
    channel shows source in both cases, which is why a chart that quietly failed
    to draw could stay unnoticed for as long as it did.

    ``log`` exists because that line is written once per *decision* and this
    function is not called once per decision. ``streamable_prose_prefix`` calls
    it to ask a question -- is this fence structure, or is it prose I may send
    now -- and the streaming path asks that question again on every update,
    rescanning the reply from the start, so one declining fence in a streamed
    answer wrote the line a hundred times or more for a single failure. The
    probe passes ``log=False``; the answer it is asking about is reported once,
    at compose time, by the call that is actually deciding what to render. The
    default is the logging one, so a new caller has to say it is a probe.
    """
    tokens = info.split()
    ...
    # the branch: only ```blockkit carries JSON the author wrote by hand, and
    ...
    *,
    ...
    for block in payload:
        ...
        if not isinstance(kind, str) or not kind.strip():
            return None
        if allowed_block_types and kind.strip().lower() not in allowed_block_types:
            return None
    ...
    for line in lines[1:]:
        ...
        if axis is not None:
            ...
            if axis.group("axis").lower() == "x-axis":
                ...
                # A y-axis carries a title and a range, never a category list.
...
def _xychart_axis(rest: str) -> tuple[str, list[str] | None] | None:
    ...
    line carried none, or ``None`` outright when the line is not an axis this
...
def _vega_lite_pie(
    ...
    apart -- Slack's segment carries a label and there is nowhere else to get
    one.
    """
...
def _vega_lite_series(
    ...
    if colour is not None:
        ...
        if series_field == x_field:
            ...
            # is its own series of exactly one point, so the grouping carries no
            # information the x-axis does not already carry. Vega-Lite draws it
            # as a bar chart in n colours; Slack has no per-category colour, and
            # mapping it literally asks ``data_visualization`` for n series each
            # missing n-1 of the n categories -- a grid that is all gaps, which
            # is refused. Dropping the channel draws the chart the author meant
            # in one colour, which is strictly more than declining it draws.
            #
            # Deliberately the only redundant encoding read this way. This one is
            # decidable by equality of two field names; guessing at intent for
            # any other pair would be the approximation the rest of this module
            # exists to refuse.
            series_field = None
...
def _is_interactive(value: Any) -> bool:
    ...
    * an ``action_id``, wherever it sits. A button, a select, an overflow and an
      accessory all carry one, and nothing this module builds for itself does.
...
def _cell(raw: str, plain: str) -> dict[str, Any]:
    ...
    ``raw_text`` can only carry characters, so a cell holding
    ...
    place. A ``rich_text`` cell can carry the link, so cells that have one use
...
def _link_matches(raw: str) -> list[re.Match[str]]:
    ...
    carry a colon in that position.
    """
    return [
        match
...
def _rich_text_elements(raw: str) -> list[dict[str, Any]]:
    ...
    for match in _link_matches(raw):
        ...
        # Slack shows the url itself when a link carries no text, which is what
...
def _data_table_block(table: _Table, caption: str) -> dict[str, Any] | None:
    ...
    ``column_settings`` is **not** emitted. ``table`` accepts it and whether
    ``data_table`` does was never established -- the probe that would settle it
    was not available. An unknown field risks ``invalid_blocks``, which costs
    the whole message its rendering, while omitting it costs the alignment of
    the rare generated table that asks for one. Until it is probed, the cheaper
    failure is the one to take.
    """
...
def _page_size(data_rows: int) -> int:
    ...
    Never Slack's default of 5, and never more rows than the table has. The
    second half is what keeps ``data_table`` from costing a short table its
    readability: a three-row table gets a page of three and the pager never
    appears, so the block is drawn for its sorting and filtering rather than for
    a paging nobody asked for. Only a table longer than ``DATA_TABLE_PAGE_SIZE``
    is paged at all, and it is paged at exactly the length this module is
    willing to show whole.
    """
...
def _numeric_columns(table: _Table) -> set[int]:
    ...
    A cell carrying a link is excluded too, because the destination is worth
...
def _number_cell(plain: str) -> dict[str, Any]:
    ...
    ``text`` is documented as optional and is not: a post carrying
...
def data_fallback_text(
    ...
    ``already_in`` is the text the same message is about to carry. On the reply
    path that text is the chunk the table was parsed *out* of, so it still holds
    the pipe table verbatim and repeating it underneath would show the same rows
    twice. A block whose header cells all appear there is therefore dropped, and
    only a block the reader would otherwise never see is rebuilt -- which is the
    case that motivated this: a message whose ``text`` is a stub because the
    whole content was the chart.

    Blocks that carry no data of their own -- a ``section``, a ``context`` --
