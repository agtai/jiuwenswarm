...
change, and every one of them carries ``state.values`` -- the complete current
...
class InputRenderError(ValueError):
    ...
    a question declaring more inputs than a message can carry. Posting the rest
    would silently drop something the person was meant to be able to answer
    with, which is exactly the failure the option-button path already refuses.
    """
...
def _build_option(option: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    ``None`` when the option carries nothing to show. An option whose value does
...
def _format_epoch(
    ...
    UTC and no zone name, because that is all the element carries: it answers
...
class InputSpec:
    ...
    carries. It is where validation lives: raising ``InputValueError`` is how an
...
def parse_inputs(question: Mapping[str, Any]) -> list[QuestionInput]:
    ...
    written before this existed: the option-button path is left to handle it,
    unchanged. That is the whole of the backward compatibility contract.

    An entry may be a bare type name -- ``inputs: ["date", "time"]`` -- or a
    mapping carrying a label, an initial value and the type's own knobs. The
...
def divider_block() -> dict[str, Any]:
    """A horizontal rule. Non-interactive and carries nothing."""
    return {"type": "divider"}
...
def render_input_blocks(
    ...
    payload carries their contents in ``state.values`` exactly as a modal's
    submission does.
...
def compose_answer(
    ...
    """Turn the read inputs into the ``selected_options`` an answer carries.
