...
replays across reconnects. The connector has always dropped duplicates, but it
did so with a process-local LRU, so **a restart re-admitted every retry still in
Slack's window** -- the bot answered a message it had already answered, which is
exactly the window a deploy or a crash-restart lands in.

Persisting the window closes that. The storage shape follows ``CronJobStore``:
an ``asyncio.Lock`` for coroutines in this process, a ``portalocker`` sidecar for
other processes, the whole read-modify-write inside both, and an atomic
``tmp.replace()`` so a torn write cannot corrupt the file.
...
class SlackEventDedupStore:
    ...
    That costs a small read plus write per inbound event -- negligible at Slack
    volumes -- and is what makes the answer correct when more than one process
    shares a workspace, where a cached "not seen" would be stale the moment a
    sibling admitted the same message.
    """
    ...
    async def remember(self, key: str) -> bool:
        ...
        in-memory answer and the event is handled. Losing durability is bad;
        silently dropping a user's message because the disk misbehaved is worse.
        """
        if not key:
            return True
    ...
    def _read_unlocked(self) -> list[str]:
        ...
        Starting empty costs at most one duplicated answer; raising would cost
        every message.
        """
        try:
