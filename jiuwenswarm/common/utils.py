...
def prepare_runtime_workspace(*, cleanup_stale_descs: bool = True) -> None:
    ...
    workspace_preparation_needed = any(
        (
            ...
            # An install that predates the zip-seed feature has the directory but an
            # older seed, so "missing" is not enough to notice it needs reseeding.
            mcp_builtins_seed_outdated(mcp_builtins_dir),
        )
    )
