...
remember than a table of per-key rules, and it is the only version that
generalises to keys which do not exist yet: a connector adding a setting gets
its composition rule from the value's type, with no entry to add here.
...
and never see each other's keys, which is what keeps "three sections, three
readers" true of the values as well as of the names: two sections could name the
same key and mean two different things without either fold noticing.

**The result carries only the keys some layer spoke about.** A key no scope set
is absent, not filled in from layer 0. The distinction is the whole contract
where this is consumed: an absent ``mode`` means the connector's own chain still
...
def compose_section(
    ...
    *,
    ...
    applicable = matching_scopes(
        scopes, channel=channel, chat=chat, user=user, section=section
    )
...
def click_rule(
    ...
    *,
    ...
    """Who may make one kind of click in one conversation, or ``None`` for F1.

    ``None`` is the whole of F1 and is not an empty rule: with no ``clicks`` rule
    matching, layer 0 decides -- ``allow_from`` on Slack -- and layer 0 is not
    ...
    **Composed with no sender, and that is not an omission.** The ``clicks``
    ...
    Per gesture, because the ``clicks`` map merges per key (D2/§5.4): a rule
    naming only ``stop`` leaves ``approve`` on whatever the layer above settled,
    and a later layer naming ``approve`` replaces that clause rather than
    widening it by union -- which is the direction D4 requires of a section that
    exists to tighten.
    """
...
def scoped_chats(
    ...
    **``permissions`` and ``clicks`` are deliberately not in that default, and
    ...
    performed by a restriction, which is the one direction (D4) nothing in this
    design may move. ``permissions`` is also resolved in the other process
    entirely (§7.1), and ``clicks`` is read when a button is pressed in a
