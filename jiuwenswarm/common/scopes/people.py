...
platform they are reachable on, so the mapping from a name to an id is exactly
the thing that must not be written once per connector: adding a platform to a
person is one line here, and every role they are in follows. Putting the block
under ``channels.slack`` would give the same human a separate identity per
connector and make "the admins" mean a different set of people on each -- which
is the shape ``roles`` exists to remove.

**The value under a platform is an id, or a list of them.** Identity is an
opaque per-channel bag (section 8.1) and a platform may have several ids for one
...
the event carried. So a person who is reachable under two ids on one platform
...
**Roles carry no permissions, and that is a boundary rather than a gap (D8).**
...
-- does the opposite: authority is in two places, and the escalation problem
the design spent ``override:`` to avoid returns through the role definition. A
``roles:`` entry written that way is refused with that reason, not read for the
part of it that looks familiar.
...
humans legitimately carries ids for platforms this deployment does not run -- a
config shared between two deployments always will -- so a warning there would
fire on every correct file. The warning an operator can act on is emitted where
it is actionable instead: on the scope, when a role it names resolves to nobody
on the channel that scope is written for.
"""
...
def _compile_roles(
    ...
    for raw_name, raw_members in raw.items():
        ...
        if isinstance(raw_members, Mapping):
            # D8, and the reason is worth the sentence: this is the shape every
            # other RBAC system uses, so an author reaching for it is following
            # a habit rather than making a mistake, and being told "not a list"
            # would leave them looking for the syntax error.
            warn(
