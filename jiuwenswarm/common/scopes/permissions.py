...
``permissions`` is the one section a mistake in does not crash: it grants a tool
call that should have been refused. So this module does as little arithmetic of
its own as it can get away with, and borrows the rest.

**There is exactly one merge, and it is not written here.**
...
in fact, because folding several matching scopes into one settled level is also
just ``strictest`` and is done by folding them through ``narrow_permissions``
over an all-``allow`` base rather than by a second merge written by hand.

That is what makes D3 -- *permissions composes order-independently* -- structural
rather than asserted. ``strictest`` is a meet: commutative and associative. Fold
the layers in any order and the settled level is the same, because nothing here
ever takes "the last one wins".

**And it is why ``allow`` in a scope can never widen anything** (D4). A scope
saying ``allow`` folds to ``strictest(base, allow) == base``: it is a no-op by
construction, not by a check that could be forgotten. The loader warns about it
...
**D11 -- an unattended channel degrades ``ask`` to ``deny``.** An ``ask``
...
**Nothing here raises either**, which the package contract already claims for
the whole of ``common/scopes`` and this module could not honour: the one thing
it borrows is imported from agent-core at the point of use, and both of that
import's paths -- and its deliberate re-``raise`` -- ran on every tool check,
inside a permission hook nothing wraps. Both public functions now answer "no
...
def _scope_layers(
    ...
    the way the file does. Nothing downstream depends on it -- that is D3.
    """
...
def _degrade_unattended(
    ...
    """D11: where no human can answer an approval, ``ask`` means ``deny``.
    ...
    the opt-in (§7.2), and guessing here would silently deny on a surface that
...
def _prune_approval_overrides(
    ...
    **This is the caveat that must live in the code and not only in the design.**
    ...
    precisely the failure D3 spent order-independence to avoid.
