...
from openjiuwen.core.foundation.llm.reasoning import is_reasoning_config_intent, resolve_reasoning_plan
...
class OpenAIResponsesClient(OpenAIModelClient):
    ...
    def _request_params(self, messages, tools, *, stream, **kwargs):
        ...
        raw_reasoning = options.pop("reasoning", {}) or {}
        reasoning = deepcopy(raw_reasoning if isinstance(raw_reasoning, dict)
                             else raw_reasoning.model_dump(exclude_none=True))
        if is_reasoning_config_intent(reasoning):
            plan = resolve_reasoning_plan(
                self.model_client_config.model_copy(update={"api_mode": LLMApiMode.Responses}),
                self.model_config, request_model=model, explicit_kwargs={"reasoning": reasoning},
            )
            # GPT's default still reasons; an explicit UI "off" needs effort=none.
            reasoning = ({"effort": "none"} if reasoning.get("mode") == "disabled"
                         else deepcopy(plan.sdk_params.get("reasoning") or {}))
        ...
        for key in ("parser", "output_parser", "tracer_record_data", "session_id",
                    "context_window", "request_purpose", "context_operation_id"):
...
def openai_responses_client_config(client_config: ModelClientConfig, *, model_name: str) -> ModelClientConfig:
    ...
    if (provider == "OpenAI" and client_config.auth_mode == LLMAuthMode.ApiKey
            and model_name in _VERIFIED_MODELS and _official_endpoint(client_config.api_base)):
