...
class HostWorkService:
    ...
    def __init__(self, database_path: str | Path, *, agent_manager: Any) -> None:
        ...
        self.work_runtime = NativeWorkRuntime(
            save=self.journal.save, restored=self.journal.restore(),
        )
    ...
    async def retire_previous_generations_locked(self, scope: Any, generation: int) -> None:
        ...
        """
        if any(not item.execution_settled for item in self.work_runtime.list(scope=scope)):
            return
        for key, (executor, _facade) in tuple(self.executors.items()):
            binding = key[1] if isinstance(key, tuple) and len(key) == 3 and key[0] == "cleanup" else key
            if not (isinstance(binding, tuple) and len(binding) == 2
                    and binding[0] == scope and binding[1] != generation):
                continue
            try:
                snapshot = executor.snapshot()
                if not snapshot.closed and not (
                    getattr(snapshot, "active_requests", None) == ()
                    and getattr(getattr(snapshot, "harness", None), "active_rounds", None) == ()
                ):
                    continue
                await self._close_executor_locked(key)
            except Exception as error:
                logger.warning("Work predecessor cleanup remains pending: %s", type(error).__name__)
        ...
        async with self.executor_lock:
            ...
            try:
                ...
                if any(not item.execution_settled for item in settled):
                    from jiuwenswarm.runtime.service import RuntimeStateError
                    errors.append(RuntimeStateError("work execution settlement is incomplete"))
            except BaseException as error:
                errors.append(error)
            if errors:
                raise errors[0]
