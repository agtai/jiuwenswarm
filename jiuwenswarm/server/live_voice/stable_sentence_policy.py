...
_MAX_NON_TEXT_AGENT_EVENTS = 256
# One non-empty one-byte delta per response byte, a bounded reserve for
# supported barriers/tool events, and the authoritative final.
_MAX_AGENT_EVENTS = _MAX_RESPONSE_UTF8 + _MAX_NON_TEXT_AGENT_EVENTS + 1
_EVENT_RECORD_CHUNK_SIZE = 128
...
class StableSentenceStreamState:
    response_ref: ResponseRef
    observed_text: str = ""
    observed_utf8_end: int = 0
    committed_utf8_end: int = 0
    boundary_scan_character: int = 0
    next_agent_event_seq: int = 0
    non_text_event_count: int = 0
    ...
    event_fingerprints: tuple[tuple[str, ...], ...] = ()
    text_event_spans: tuple[tuple[tuple[int, int, int], ...], ...] = ()
...
def _append_event_fingerprint(
    chunks: tuple[tuple[str, ...], ...], fingerprint: str
) -> tuple[tuple[str, ...], ...]:
    if not chunks or len(chunks[-1]) >= _EVENT_RECORD_CHUNK_SIZE:
        return (*chunks, (fingerprint,))
    return (*chunks[:-1], (*chunks[-1], fingerprint))


def _event_fingerprint_at(
    chunks: tuple[tuple[str, ...], ...], seq: int
) -> str | None:
    chunk_index, offset = divmod(seq, _EVENT_RECORD_CHUNK_SIZE)
    if chunk_index >= len(chunks) or offset >= len(chunks[chunk_index]):
        return None
    return chunks[chunk_index][offset]


def _append_text_event_span(
    chunks: tuple[tuple[tuple[int, int, int], ...], ...],
    span: tuple[int, int, int],
) -> tuple[tuple[tuple[int, int, int], ...], ...]:
    if not chunks or len(chunks[-1]) >= _EVENT_RECORD_CHUNK_SIZE:
        return (*chunks, (span,))
    return (*chunks[:-1], (*chunks[-1], span))
...
def _validate_identity(
    ...
    if event.seq < state.next_agent_event_seq:
        if event.seq >= 0 and _event_fingerprint_at(
            state.event_fingerprints, event.seq
        ) == fingerprint:
            return state
        raise StableSentenceViolation(
    ...
    is_text_event = event.event_type in {"chat.delta", "chat.final"}
    if not is_text_event and state.non_text_event_count >= _MAX_NON_TEXT_AGENT_EVENTS:
        raise StableSentenceViolation(
            "AGENT_NON_TEXT_EVENT_LIMIT_EXCEEDED",
            "Agent non-text event bound was exceeded",
        )
    ...
    return replace(
        ...
        non_text_event_count=(
            state.non_text_event_count
            if is_text_event
            else state.non_text_event_count + 1
        ),
        event_fingerprints=_append_event_fingerprint(
            state.event_fingerprints, fingerprint
        ),
    )
...
def _select_candidate(
...
) -> tuple[StableSentenceCandidate | None, int]:
    if (
        state.active_candidate is not None
        or state.promotion_blocked
        or _has_unclosed_code_fence(state.observed_text)
    ):
        return state.active_candidate, state.boundary_scan_character
    start_character = _character_offset_for_utf8(
        state.observed_text, state.committed_utf8_end
    )
    scan_start = max(start_character, state.boundary_scan_character)
    boundary = next(
        (
            selected
            for index in range(scan_start, len(state.observed_text))
    ...
    if boundary is None:
        unresolved = next(
            (
                index
                for index in range(scan_start, len(state.observed_text))
                if state.observed_text[index] in _SENTENCE_END
                and all(
                    character in _SENTENCE_TRAILER or character.isspace()
                    for character in state.observed_text[index + 1 :]
                )
            ),
            len(state.observed_text),
        )
    ...
    first_event_seq = next(
        (
            event_seq
            for chunk in state.text_event_spans
            for event_seq, _start, end in chunk
...
def observe_agent_event(
    ...
    if event.event_type == "chat.delta":
        ...
        fragment_utf8 = fragment.encode("utf-8")
        source_start = next_state.observed_utf8_end
        source_end = source_start + len(fragment_utf8)
        if source_end > _MAX_RESPONSE_UTF8:
            raise StableSentenceViolation(
                "INVALID_AGENT_TEXT", "Agent text exceeds the closed content boundary"
            )
        ...
        next_state = replace(
            ...
            observed_utf8_end=source_end,
            text_event_spans=_append_text_event_span(
                next_state.text_event_spans,
                (event.seq, source_start, source_end),
            ),
        )
        candidate, scan_character = _select_candidate(next_state, event.seq)
        next_state = replace(
            next_state, boundary_scan_character=scan_character
        )
...
def commit_candidate(
    ...
    return StableSentenceCommit(
        replace(
            ...
            boundary_scan_character=_character_offset_for_utf8(
                state.observed_text, candidate.source_end_utf8
            ),
            active_candidate=None,
        ),
        candidate,
