...
class ScriptedCascadeInteractionEngine:
    ...
    ordering.  Exact duplicates replay the prior intention; conflicting,
    stale, cross-scope, unsupported, or over-capacity observations fail before
    the retained intention set changes.

    Releasing a contiguous prefix only releases replay memory.  It never resets
    the sequence or response generation, so a released observation cannot be
