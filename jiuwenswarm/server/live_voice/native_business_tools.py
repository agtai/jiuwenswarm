...
_DESCRIPTIONS = {
    ...
    "task.adjust": "Native Swarm Tasks only; NOT Atlas targets. Submit the user's complete explicit change to the exact observed Task and revision. For an Atlas claim submission or purchase confirmation use task.approve, not this tool. When supported, a closing or completed Task queues a successor that changes its saved result; do not duplicate that accepted change with another create. A new question is not an adjustment. Dispatched means accepted; execution adoption and saved completion are distinct. Report the exact adjustment observation and final outcome.",
