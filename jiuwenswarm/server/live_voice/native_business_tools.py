...
_DESCRIPTIONS = {
    ...
    "task.approve": "Atlas: execute the user's explicit approval of the exact pending_decision_scope: purchase, directory_listing, or expense_form_submission. Use THIS tool to submit a prepared Paris expense claim when the user confirms submission; do not use task.adjust. Copy the latest target ID and revision from context and match the pending action. Directory approval does not authorize form submission. A question, silence or a requested modification is not approval. Report only the actual scope-specific receipt; purchase approval alone is not purchase completion.",
    ...
    "task.adjust": "Native Swarm Tasks only; NOT Atlas demo targets. Submit the user's complete explicit change to the exact observed Task and revision. For an Atlas claim submission or purchase confirmation use task.approve, not this tool. When supported, a closing or completed Task queues a successor that changes its saved result; do not duplicate that accepted change with another create. A new question is not an adjustment. Dispatched means accepted; execution adoption and saved completion are distinct. Report the exact adjustment observation and final outcome.",
