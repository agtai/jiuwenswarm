...
_DESCRIPTIONS = {
    ...
    "task.adjust": "For an Atlas expense draft, first read task.details, then pass adjustment as JSON with exactly line_id, amount, expected_hash (expense.snapshot_hash). Use the exact observed task and revision. An applied receipt confirms updated form and files, not submission; ask for a new confirmation before task.approve. Other Atlas targets do not support adjustment. For native Swarm Tasks: Submit the user's complete explicit change to the exact observed Task and revision. For an Atlas claim submission or purchase confirmation use task.approve, not this tool. When supported, a closing or completed Task queues a successor that changes its saved result; do not duplicate that accepted change with another create. A new question is not an adjustment. Dispatched means accepted; execution adoption and saved completion are distinct. Report the exact adjustment observation and final outcome.",
...
def _property(field: str, operation: str) -> dict[str, object]:
    ...
    meaning = {
        "name": "A concise name for an ordinary native Task. Only when invoking an advertised Atlas workflow, set exactly atlas:repurchase for coffee repurchase or atlas:expense for Paris reimbursement. Never use these reserved names for weather, itineraries or other native tasks",
