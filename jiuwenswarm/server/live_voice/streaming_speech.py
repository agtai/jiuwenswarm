...
class RecognitionProviderSupport:
    ...
    ordered_events: bool = False
    exact_audio_cursor: bool = False
    provider_cancel_ack: bool = False
    native_partials: bool = False
...
class StreamingSpeechConformance:
    ...
    def start_recognition(
        ...
        with self._lock:
            ...
            self._recognition[_recognition_key(ref)] = _RecognitionState(
                ref=ref, deadline=self._deadline(timeout)
            )
    ...
    def accept_recognition_event(
        ...
        with self._lock:
            ...
            try:
                ...
                if (
                    event.audio_cursor < state.provider_audio_cursor
                    or event.audio_cursor > state.next_audio_cursor
                ):
                    raise StreamingSpeechViolation(
                        "INVALID_RECOGNITION_AUDIO_CURSOR",
                        "recognition Provider cursor must be monotonic and submitted",
                    )
                if (
                    state.output_fenced
                ...
                if event.kind in {
                    ...
                    if event.audio_cursor == 0:
                        raise StreamingSpeechViolation(
                            ...
                            "recognition text must identify consumed audio",
                        )
            ...
            if not self._closed:
                ...
                for state in self._recognition.values():
                    if not state.terminal and not state.cancel_requested:
                        self._request_recognition_cancel(state, "service_close")
                for state in self._synthesis.values():
                    if not state.terminal and not state.cancel_requested:
                        self._request_synthesis_cancel(state, "service_close")
    ...
    def _accept_synthesis_chunk(
        ...
        if event.display_span is None or event.spoken_span is None:
            ...
            raise StreamingSpeechViolation(
                ...
                "every synthesis chunk requires display and spoken provenance",
            )
    ...
    def _require_live_recognition(self, state: _RecognitionState) -> None:
        ...
        if state.output_fenced:
            raise StreamingSpeechViolation(
                "RECOGNITION_INPUT_FENCED",
                "recognition input is fenced after cancel, timeout, or failure",
            )
...
def _validate_recognition_support(support: RecognitionProviderSupport) -> None:
    ...
    for field, value in (
        ...
        if type(value) is not bool:
            raise StreamingSpeechViolation(
                "INVALID_CAPABILITY_BOOLEAN",
                f"recognition.{field} must be a boolean",
            )
    ...
    if streaming and not all(guarantees):
        raise StreamingSpeechViolation(
            "INCOMPLETE_RECOGNITION_STREAM_CAPABILITY",
            "streaming recognition requires ordered partials, cursors, and cancel ACK",
        )
    if not streaming and (
        support.transport is ProviderTransport.NATIVE_STREAM or any(guarantees)
    ):
        raise StreamingSpeechViolation(
...
def _validate_synthesis_support(support: SynthesisProviderSupport) -> None:
    ...
    for field, value in (
        ...
        if type(value) is not bool:
            raise StreamingSpeechViolation(
                "INVALID_CAPABILITY_BOOLEAN",
                f"synthesis.{field} must be a boolean",
            )
    streaming = SpeechMode.STREAM in support.modes
    if streaming and support.transport is ProviderTransport.POLLING:
        raise StreamingSpeechViolation(
    ...
    if streaming and not all(guarantees):
        raise StreamingSpeechViolation(
            ...
            "streaming synthesis requires ordered chunks, cursors, cancel ACK, and spans",
        )
    if not streaming and (
        support.transport is ProviderTransport.NATIVE_STREAM or any(guarantees)
    ):
