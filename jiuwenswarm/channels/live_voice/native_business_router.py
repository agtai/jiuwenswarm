...
from jiuwenswarm.channels.live_voice.native_business_contract import NATIVE_BUSINESS_CONTRACT_VERSION, NativeBusinessProposal, NativeBusinessViolation
from jiuwenswarm.server.runtime.work.native_business_context import NativeBusinessContextStore, formal_context, select_conversation_history
...
class NativeBusinessRouter:
    ...
    def works(self):
        if self._work_owner is None:
            ...
            host = getattr(self.registry, "_runtime", None)
            if not isinstance(host, AgentRuntime):
                raise NativeBusinessViolation("NATIVE_WORK_HOST_UNAVAILABLE", code=ErrorCode.UNAVAILABLE)
            service = host.get_work_service(self.registry._unified_journal.database_path)
    ...
    async def _restore_task_projection(self, route, scope, tasks):
        ...
        """
        from jiuwenswarm.channels.live_voice.product_composition_registry import _VoiceTaskOrigin
        from jiuwenswarm.channels.live_voice.product_p2_interaction_adapter import P2LeaseState
    ...
    async def context_request(self, *, params, request_id, session_id):
        from jiuwenswarm.channels.live_voice.product_composition_registry import _success_result, _error_result
        try:
    ...
    async def handle(self, *, owner, proposal, request_id, retained_route, **unused):
        from jiuwenswarm.channels.live_voice.product_composition_registry import _success_result, _error_result, _VoiceTaskOrigin
            ...
            if result is None:
                try:
                    if invalid:
                        ...
                        from jiuwenswarm.common.schema.native_task_source import SOURCE_OPERATIONS
