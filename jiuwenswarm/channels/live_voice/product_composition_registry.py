...
from jiuwenswarm.channels.live_voice.native_business_contract import NATIVE_BUSINESS_CONTRACT_VERSION, NativeBusinessProposal
...
from jiuwenswarm.channels.live_voice.observability_exporter import ExportRecord
...
if TYPE_CHECKING:
    from jiuwenswarm.channels.live_voice.product_observability_runtime import ProductObservabilityRuntime
...
from jiuwenswarm.server.runtime.formal_tasks.task_semantics import TaskSemanticDecision
from jiuwenswarm.server.runtime.formal_tasks.task_result_context import TaskResultContext, _TASK_RESULT_CONTEXT_MAX_BYTES
from jiuwenswarm.channels.live_voice.product_diagnostic_projection import ProductDiagnosticProjection
...
class AgentServerProductCompositionRegistry(TaskResultContext, ProductDiagnosticProjection):
    ...
    def __init__(
        ...
        *,
        ...
        if runtime is not None and runtime.agent_manager is not agent_manager:
            raise ValueError("product composition must share the host AgentManager")
        ...
        *,
        ...
        try:
            ...
            if "agent_model_selection" in params:
                from jiuwenswarm.server.runtime.agent_adapter.native_agent_model import parse_native_agent_model_selection
            ...
            async def activate_p2(
                ...
                if (
                    ...
                    if self._settings.native_business_enabled:
                        from jiuwenswarm.server.runtime.agent_adapter.native_agent_model import resolve_native_agent_model
                ...
                if (
                    ...
                    and self._runtime.session_coordinator.snapshot_session(routed_session) is None
                ):
                    # Authenticated activation may adopt a never-seen public
                    # Session, but must never reopen a closed generation.
                    await self._runtime.register_session(
                        session_id=routed_session, channel_id=channel_id,
                        allow_reopen=False,
                    )
                ...
                from jiuwenswarm.server.runtime.agent_adapter.runtime_formal import (
                    RuntimeFormalAgentFacade,
                )
                self._pending_p2_agents[pending_key] = RuntimeFormalAgentFacade(
                    runtime=self._runtime, agent=agent, scope=canonical.scope,
                    agent_channel_id=_FORMAL_LIVE_VOICE_AGENT_CHANNEL,
                    mode=_FORMAL_LIVE_VOICE_AGENT_MODE, project_dir=project_dir,
                )
                try:
                    try:
        ...
        *,
        ...
        if decision.route == "task":
            ...
            if decision.proposal.operation in {"task.adjust", "task.status"}:
                from jiuwenswarm.server.runtime.formal_tasks.task_control_presentation import adjustment_status_text, task_status_text, task_subject
    ...
    async def _confirm_production_intent(
        ...
        *,
        ...
        async def claim_continuation() -> None:
            async with self._lock:
                if self._pending_production_task_intents.get(pending.token) is not pending:
                    raise FormalTaskViolation(
                        "TASK_INTENT_CONTINUATION_UNAVAILABLE",
        ...
        async with self._p3_operation_lock:
            ...
            confirmed, result = await self._p3_composition.confirm_and_handle_production_request(
                request=request, preliminary=preliminary, authority=authority,
                origin_authority=origin_authority,
                confirmation_owner=self._p3_confirmation_owner,
                confirmation_forwarder=self._p3_confirmation_forwarder,
                confirmation_id=pending.confirmation_id,
                owner_context=pending.confirmation_owner_context,
                bridge=self._task_intent_bridge,
                clarification_owner=self._production_clarification_owner,
    ...
    async def _run_p3_production_intent(
        ...
        *,
        ...
        native_authority: NativeP3ActivationAuthority | None = None,
        native_request: ProductionTaskIntentRequest | None = None,
    ) -> P3RouteResult:
        bridge = self._task_intent_bridge
        if bridge is None:
            if native_request is not None:
                raise RuntimeError("Native Task intent bridge is unavailable")
            return _error_result(request_id, reason="PRODUCT_P3_INTENT_DISABLED")
        source = str(clean["source"])
        ...
        try:
            ...
            if native_request is not None:
                if (native_authority is None or source != "structured"
                    or native_request.origin is not ProductionIntentOrigin.STRUCTURED
                    or native_request.source_id != clean.get("source_id")
                    or native_request.scope.session_id != clean.get("session_id")
                    or clean.get("continuation_id") is not None):
                    raise FormalTaskViolation(
                        "NATIVE_TASK_REQUEST_BINDING_MISMATCH",
                        "Native Task admission requires its exact internal authority",
                        ErrorCode.PERMISSION_DENIED,
                    )
                proposal = native_request.proposal
            ...
            if native_request is not None:
                intent_request = native_request
            elif pending is not None and pending.kind == "confirmation":
                pending_origin = (
                    ProductionIntentOrigin.STRUCTURED
            ...
            if native_request is not None:
                raise  # Native's immutable call receipt owns its rejection projection.
