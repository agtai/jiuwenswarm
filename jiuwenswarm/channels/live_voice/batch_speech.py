...
from jiuwenswarm.channels.live_voice.speech_http_diagnostics import SpeechHttpDiagnostics
...
def _parse_recognition_segment(
    *,
    ...
    return RecognitionCaptureSegment(
        ...
        capture_id=_required_text(capture["capture_id"], f"{field}.capture.capture_id"),
...
class FormalBatchSpeechService:
    ...
    async def claim_voice_commit_receipt(
        ...
        *,
        ...
        now = self._monotonic()
        # This capability authenticates recognition, not language or intent.
        # Keep the legacy argument wire-compatible, but never let a client
        # boolean confer action authority or relabel speech as explicit text.
    ...
    def _reserve_capture(self, request: RecognitionBatchRequest) -> None:
        ...
        for (capture_scope, capture_id), (_, _, generation) in zip(
            ...
            seen_generation = self._seen_captures.get((capture_scope, capture_id))
