...
import { canLocate, receiptAnchor, receiptsFromExecutions, type ReceiptRow, changedSpan } from '../../features/clouddoc/receipts';
...
export function DocWorkbench({ composer }: { composer: ComposerProps }) {
  ...
  const locateReceipt = useCallback((r: ReceiptRow) => {
    ...
    if (doc.kind === 'markdown') {
      // A person's save replaces the file, so its receipt pair is the whole
      // document; searching for that finds offset 0 and lights every line.
      // Trim it to the lines that differ and look for those.
      const span = changedSpan(r, r.edits?.[0]?.old || '', r.edits?.[0]?.new || '');
      const text = span.new;
      if (!text) { setMdHighlight(null); return; }
