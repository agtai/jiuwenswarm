...
export function DocsPanel({ isConnected }: { isConnected: boolean }) {
  ...
  const actOnReceipt = useCallback(
    async (method: 'clouddoc.revert' | 'clouddoc.unhighlight', receiptId: string) => {
                    ...
                    <div className="rounded-md bg-bg-hover px-3 py-2">
                      ...
                      <p className="mt-1 text-text-muted">
                        {t('docs.usage.summaryLine', {
                          ...
                          reverted: Number(u.reverted_batches ?? 0),
                        })}
                      </p>
                ...
                {receipts.map((r) => (
                  ...
                  >
                    ...
                    {r.status === 'reverted' && r.reverted_by && (
                      ...
                      >
                        {t('docs.history.revertedBy', { id: r.reverted_by.slice(0, 8) })}
                      </p>
                    )}
                    {r.status === 'unknown' && (
                      <p
