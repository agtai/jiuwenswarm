...
class BrowserLiveVoiceOwnershipCoordinator implements BrowserLiveVoiceOwnership {
  ...
  async acquire(onTakeover: BrowserLiveVoiceTakeover): Promise<void> {
    ...
    if (this.#held !== null) {
      ...
      if (latest !== null && compareClaims(latest, held.acquisition.claim) > 0) this.#revoke(held);
    ...
    if (this.#pending !== null) {
      ...
      pending.onTakeover = onTakeover;
      this.#channel?.postMessage(Object.freeze({ type: 'takeover', claim: pending.claim }));
      await this.#awaitCurrentAcquisition(pending);
      return;
    }

    const acquisition: Acquisition = {
      claim: Object.freeze({ issued_at: this.#now(), claim_id: this.#randomId() }),
    ...
    void acquisition.requestPromise.catch(error => {
      ...
      acquisition.granted.reject(error);
    });
