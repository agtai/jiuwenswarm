...
const MARKERS: Readonly<Record<BrowserL0Milestone, Readonly<MarkerRule>>> = Object.freeze({
  ...
  discarded_work: Object.freeze({ event_name: 'fence.stale_dropped', segment_name: 'runtime.response', source_component: 'measurement.runtime.discarded_work', reason_code: 'STALE_GENERATION' }),
});
...
function normalizeBinding(value: Readonly<BrowserL0Binding>): Readonly<BrowserL0Binding> {
  ...
  if (normalized.attempt_id !== null && normalized.task_id === null) throw new TypeError('attempt identity is incomplete');
