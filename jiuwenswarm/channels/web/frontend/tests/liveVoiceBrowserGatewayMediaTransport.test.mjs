...
for (const [field, value] of [
  ...
  test(`attach rejects wrong ${field} with zero downstream effects`, () => {
    ...
    const result = activation.receiver.attach({
      type: 'media.attach',
      binding: binding({ [field]: value }),
    });

    assert.equal(result.reason_id, 'MEDIA_BINDING_MISMATCH');
    assert.equal(activation.receiver.closed, true);
...
test('malformed, oversized, stale generation, and non-finite frames detach before consumption', () => {
  ...
  for (const { reason, mutate } of cases) {
    const effects = { audio: 0, agent: 0, task: 0 };
    const activation = active({ effects });
    activation.receiver.attach({ type: 'media.attach', binding: activation.binding });
    const result = activation.receiver.acceptBinary(mutate(encodeAudioFrame(activation.binding, frame())));
...
test('sender enforces bounded pressure and retains sent frames until ACK', () => {
  ...
  let sender = activation.sender;
  assert.equal(sender.enqueue(frame()).accepted, true);
  assert.deepEqual(sender.enqueue(frame(1, 160)), {
    accepted: false,
    reason_id: 'MEDIA_BACKPRESSURE_LIMIT',
  });
  assert.equal(sender.drain(() => 'backpressured').sent_frames, 0);
  assert.equal(sender.pending_frames, 1);
  assert.equal(sender.acknowledge({
  ...
  assert.equal(sender.drain(() => 'sent').sent_frames, 1);
  assert.equal(sender.acknowledge({
    type: 'media.ack', lease_id: activation.binding.lease_id, generation: 7, through_seq: 0,
  }), null);
  assert.equal(sender.pending_frames, 0);
  assert.equal(sender.pending_bytes, 0);
  assert.equal(sender.enqueue(frame(1, 160)).accepted, true);
});
...
test('sender retains copied immutable metadata and bytes across caller mutation', () => {
  ...
  assert.equal(activation.sender.enqueue(source).accepted, true);
  source.seq = 99;
  source.sample_cursor = 99;
  source.samples[0] = 0.75;
  let sent = null;

  const drained = activation.sender.drain((binary) => {
    sent = binary;
    return 'sent';
  });
  const ack = activation.sender.acknowledge({
    type: 'media.ack',
  ...
  assert.equal(activation.sender.pending_frames, 0);
});

test('semantic detach and close fence the lease without downstream effects', () => {
  const effects = { audio: 0, agent: 0, task: 0 };
  const activation = active({ effects });
  assert.equal(activation.receiver.attach({ type: 'media.attach', binding: activation.binding }), null);
  const detached = activation.receiver.acceptDetach({
    type: 'media.detach',
    lease_id: activation.binding.lease_id,
    generation: activation.binding.generation.value,
  ...
  const late = activation.receiver.acceptBinary(encodeAudioFrame(activation.binding, frame()));
  const repeated = activation.receiver.close();
  ...
  assert.equal(repeated.was_active, false);
  assertZeroDownstream(effects);
});

test('injected transport failure closes without implicit retry', () => {
  const activation = active();
  activation.sender.enqueue(frame());

  const result = activation.sender.drain(() => { throw new Error('private transport detail'); });

  assert.equal(result.sent_frames, 0);
  assert.equal(result.reason_id, 'MEDIA_TRANSPORT_SEND_FAILED');
  assert.equal(activation.sender.closed, true);
});
...
test('arbitrary detach reason is rejected before receiver state change', () => {
  ...
  assert.throws(
    () => activation.receiver.acceptDetach(raw),
    (error) => error instanceof MediaTransportViolation && error.reasonId === 'MEDIA_INVALID_CONTROL',
  );
  assert.equal(activation.receiver.attached, true);
  assert.equal(activation.receiver.closed, false);
  assertZeroDownstream(effects);

  const local = active().sender.close('private-content\nnot-a-stable-reason');
  assert.equal(local.reason_id, 'MEDIA_LOCAL_CLOSE');
  assert.equal(local.detach.reason_id, 'MEDIA_LOCAL_CLOSE');
});
...
test('business cancel delta rejects boolean false before receiver state change', () => {
  ...
  assert.throws(
    () => activation.receiver.acceptDetach(detach),
    (error) => error instanceof MediaTransportViolation && error.reasonId === 'MEDIA_INVALID_CONTROL',
  );
  assert.throws(
