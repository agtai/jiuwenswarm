...
} from '../node_modules/.cache/clouddoc-receipts/features/clouddoc/receipts.js';

// The workbench's markdown editor saves by replacing the file, so the ledger
// records the whole old text against the whole new text. Three surfaces render
// an edit as a fragment -- the side rail summary, the locate highlight and the
// panel preview -- and all three ask changedSpan for the part that moved.
...
test('markdown is always locatable: the editor searches its own text', () => {
  assert.equal(canLocate('markdown', applied), true);
});
...
test('a write that has not landed has nowhere to go', () => {
  for (const status of ['pending', 'aborted', 'reverted', 'unknown']) {
    assert.equal(canLocate('markdown', { ...applied, status }), false);
  }
  assert.equal(canLocate('markdown', { ...applied, status: 'applied_unverified' }), true);
});

test('a share or a trash changed no text, so it has no place to point at', () => {
  assert.equal(canLocate('markdown', { ...applied, op: 'share' }), false);
  assert.equal(canLocate('spreadsheet', { ...anchored, op: 'trash' }), false);
  assert.equal(canLocate('markdown', { ...applied, op: 'edit' }), true);
});
