...
test('real stores receive an idle empty Session with scoped settings and the unsent draft', async () => {
  ...
  useSessionStore.getState().addEnabledPlugin('new', 'plugin-a');
  useSessionStore.getState().addEnabledMcp('new', 'mcp-a');
  useSessionStore.getState().setAgentSelectionIntent('new', { kind: 'clear' });
  const calls = [];
  let attempt = 0;
  const session = await createLiveVoiceConversation({
    request: async (method, params) => {
      ...
      return { session_id: 'voice-created', project_id: 'project-a', project_dir: '/project-a', work_mode: 'code', persist_session: true };
    },
  ...
  assert.equal(session.persist_session, true);
  const promoted = useSessionStore.getState().getRuntime(session.session_id);
  assert.equal(promoted.persistSession, true);
  assert.deepEqual(promoted.enabledPlugins, ['plugin-a']);
  assert.deepEqual(promoted.enabledMcps, ['mcp-a']);
  assert.deepEqual(promoted.agentSelectionIntent, { kind: 'clear' });
  assert.deepEqual(
