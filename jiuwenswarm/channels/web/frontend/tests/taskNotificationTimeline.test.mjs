...
test('each notification keeps its timestamp; foreground metadata and elapsed time end at its reply', () => {
  ...
  assert.deepEqual(
    ...
    ['message', 'turnSummary', 'message', 'message', 'message'],
  );
  assert.deepEqual(
    items.filter(item => item.type === 'message').map(item => item.hideMeta),
    [false, false, false, false],
  );
  assert.equal(items.find(item => item.type === 'turnSummary').endMs - items.find(item => item.type === 'turnSummary').startMs, 3000);
...
test('notifications during foreground streaming have metadata and do not hide the final reply', () => {
  ...
  assert.equal(settled.find(item => item.type === 'turnSummary').endMs - settled.find(item => item.type === 'turnSummary').startMs, 6000);
});
...
test('ordinary text with notification wording keeps normal foreground grouping', () => {
  ...
  assert.equal(items[2].hideMeta, true);
  assert.equal(items[3].hideMeta, false);
  assert.equal(items.find(item => item.type === 'turnSummary').endMs - items.find(item => item.type === 'turnSummary').startMs, 6000);
});
...
test('consecutive empty requests after cancellation, close or disconnect never carry their clocks', () => {
  ...
  const final = render([...messages, { ...msg('a', 'assistant', 25), completedAt: at(30) }]).find(item => item.type === 'turnSummary');
...
test('Goal objective badge and same-text ordinary message cannot establish shared timing', () => {
  for (const isGoalObjectiveMessage of [false, true]) {
    ...
    assert.equal(completedWorkDurationMs(items.findLast(item => item.type === 'turnSummary')), 5000);
...
test('an undated replacement stays before its answer without borrowing an older clock', () => {
  for (const timestamp of ['', 'bad', '0']) {
    ...
    assert.equal(completedWorkDurationMs(items.findLast(item => item.type === 'turnSummary')), 0);
...
test('undated user separates subsequent tools and reasoning from an older request', () => {
  ...
  assert.equal(completedWorkDurationMs(items.findLast(item => item.type === 'turnSummary')), 4000); // Known work only; no invented request time.
});
...
test('a notification before the first reply retains its independent avatar', () => {
  const task = notice(18);
  const items = render([msg('u', 'user', 16), task, msg('a', 'assistant', 22)]);
  assert.equal(items.find(item => item.message?.id === task.id).showAvatar, true);
  assert.equal(items.find(item => item.type === 'turnSummary').showAvatar, true);
  assert.equal(items.find(item => item.message?.id === 'a').showAvatar, false);
});
