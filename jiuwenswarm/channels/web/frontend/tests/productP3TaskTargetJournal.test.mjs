...
test('P3 task target journal round-trips one exact credential-free Session binding', () => {
  ...
  assert.deepEqual(inspectProductP3TaskTarget({ session_id: 'session-a' }, storage), { status: 'absent', record: null });
  assert.equal(persistProductP3TaskTarget(target(), storage), true);
  assert.deepEqual(inspectProductP3TaskTarget({ session_id: 'session-a' }, storage), {
    status: 'valid',
...
test('P3 task target journal rejects cross-correlation, cross-scope, malformed and extra authority', () => {
  const storage = memoryStorage();
  assert.equal(persistProductP3TaskTarget(target({ task_control_binding: { ...target().task_control_binding, session_id: 'session-b' } }), storage), false);
