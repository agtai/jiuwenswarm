...
test('failed prior-owner cleanup keeps the browser lock and blocks the successor', async () => {
  ...
  await ownerA.acquire(async () => {
    ...
    ownerA.disposeAfterRelease();
  });
  assert.equal(typeof ownerA.disposeAfterRelease, 'function');
  const acquireB = ownerB.acquire(async () => {}).then(
    () => 'acquired',
    error => {
  ...
  assert.deepEqual(await Promise.all([acquireB, retryB.then(() => 'acquired')]), ['superseded', 'acquired']);
  assert.equal(cleanupAttempts, 2);
  await flushAsyncWork();
  assert.equal(ownerA.isOwner(), false);
  assert.equal(ownerB.isOwner(), true);
  assert.equal(hub.count('jiuwenswarm.live-voice.capture-owner.v1'), 1);
  await assert.rejects(
    ownerA.acquire(async () => {}),
...
test('pending re-enable mints a newer claim and wins after an intervening tab', async () => {
  const hub = new FakeChannelHub();
  const locks = new FakeLockManager();
  const cleanupHeld = deferred();
  const events = [];
  let nowA = 100;
  const heldOwner = createBrowserLiveVoiceOwnership(
    ownershipEnvironment({ hub, locks, now: 50, randomId: 'held-tab' }),
  );
  const ownerA = createBrowserLiveVoiceOwnership(
    ownershipEnvironment({ hub, locks, now: () => nowA, randomId: 'tab-a' }),
  );
  ...
  await heldOwner.acquire(async () => {
    events.push('held-close');
    await cleanupHeld.promise;
  });
  const firstA = ownerA.acquire(async () => {}).then(
    () => events.push('a-first-acquired'),
    error => {
      assert.equal(error?.message, BROWSER_LIVE_VOICE_OWNERSHIP_SUPERSEDED);
      events.push('a-first-superseded');
    },
  );
  await flushAsyncWork();
  const acquireB = ownerB.acquire(async () => {
  ...
  nowA = 300;
  const latestA = ownerA.acquire(async () => {}).then(() => {
    events.push('a-latest-acquired');
  });
  await flushAsyncWork();
  cleanupHeld.resolve();
  await Promise.all([firstA, acquireB, latestA]);
  ...
  assert.equal(heldOwner.isOwner(), false);
  assert.equal(ownerA.isOwner(), true);
  assert.equal(ownerB.isOwner(), false);

  await ownerA.release();
  await heldOwner.dispose();
  await ownerA.dispose();
  await ownerB.dispose();
});
