...
async def test_mention_dispatches_once_and_dedups(kit):
    ...
    await w.tick()
    # No registry in this rig: the tier falls to the strictest, like every other
    # missing-or-unknown default in the feature.
    assert len(sent) == 1 and sent[0][2]["clouddoc"] == {"doc_id": DOC, "comment_id": "c1", "mode": "reply_only"}
    await w.tick()
...
@pytest.mark.asyncio
async def test_without_a_registry_a_turn_is_dispatched_at_the_strictest_tier(kit):
    """Every other missing-or-unknown default in this feature resolves to the
    strictest tier; the no-registry branch was the one that defaulted to
    apply_scoped."""
    w, prov, store, dispatched = kit
    prov.comments = [C("c1", f"@{SA} 改一下", mentioned=[SA])]
    await w.tick()
    assert dispatched, "应当派发"
