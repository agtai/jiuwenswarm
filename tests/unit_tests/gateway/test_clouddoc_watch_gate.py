...
async def test_ungated_watcher_dispatches_at_the_strictest_tier(tmp_path):
    # registry=None: no mandate can be consulted, so the tier is the strictest
    # one, like every other missing-or-unknown default in the feature (this
    # branch used to default the other way). Production wiring always passes a
    # registry.
    prov = FakeProvider()
    store = CloudDocStore(tmp_path / "s.json", now_fn=Clock())
    dispatched: list[tuple] = []
