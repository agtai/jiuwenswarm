...
@pytest.mark.asyncio
async def test_provider_self_cancelled_cleanup_completes_without_starving_sibling() -> (
    None
):
    """B24/A17: Provider self-cancel is cleanup failure, not caller cancellation."""

    provider = _FakeProvider()
    provider.cancel_error = asyncio.CancelledError()
    owner, handle = await _begin(provider, _request(stream_id="provider-self-cancel"))
    ...
    outcome = await asyncio.wait_for(owner.cancel(handle), timeout=0.5)
    stop_heartbeat.set()
    await heartbeat_task

    assert heartbeat_ticks > 0
    assert outcome.reason is StreamingSynthesisReason.ROUTE_ABORTED
    assert handle.cleanup_complete is False
    assert owner.active_count == 1
    completion = handle.provider_cancel_completion
    assert completion is not None
    assert completion.done() is True
    assert completion.result() is False

    provider.cancel_error = None
    assert await asyncio.wait_for(owner.cancel(handle), timeout=0.5) == outcome
    assert handle.cleanup_complete is True
    assert owner.active_count == 0
    await owner.close()
...
@pytest.mark.asyncio
async def test_real_caller_cancel_wins_when_cleanup_provider_self_cancels() -> None:
    """B24/A17: first caller cancellation survives bounded failed cleanup."""

    provider = _FakeProvider()
    provider.cancel_gate = asyncio.Event()
    provider.cancel_error = asyncio.CancelledError()
    owner, handle = await _begin(
        provider, _request(stream_id="caller-cancel-provider-self-cancel")
    )
    stop_heartbeat = asyncio.Event()
    heartbeat_ticks = 0
    ...
    provider.cancel_error = None
    await asyncio.wait_for(owner.cancel(handle), timeout=0.5)
    assert handle.cleanup_complete is True
    assert owner.active_count == 0
    await owner.close()
