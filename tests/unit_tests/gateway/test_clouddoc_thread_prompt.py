...
def test_the_discussion_reaches_the_turn():
    ...
    assert "不要动" in build_turn_prompt(c, approve_word=("同意",), keep_word=("原文",), mode="apply_scoped").text
...
def test_speakers_are_roles_not_display_names():
    ...
    text = build_turn_prompt(c, approve_word=("同意",), keep_word=("原文",), mode="apply_scoped").text
...
def test_the_discussion_is_fenced_like_every_other_untrusted_text():
    ...
    p = build_turn_prompt(c, approve_word=("同意",), keep_word=("原文",), mode="apply_scoped")
...
def test_the_triggering_reply_is_not_shown_twice():
    ...
    text = build_turn_prompt(
        c, approve_word=("同意",), keep_word=("原文",), mode="apply_scoped", reply_content="最新的要求"
    ).text
