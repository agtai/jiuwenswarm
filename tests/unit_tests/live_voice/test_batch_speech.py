...
@pytest.mark.asyncio
async def test_boundary_timeout_revokes_preterminal_voice_receipt() -> None:
    """L4: a timed-out worker cannot leave a claimable receipt or ledger slot."""

    service = _service(ControlledProvider())
    foreign_receipt = await service.issue_streaming_voice_commit_receipt(
    ...
    original_issue = service._issue_voice_commit_receipt
    minted = asyncio.Event()
    release = asyncio.Event()

    async def issue_then_cross_deadline(request, text):
        token = await original_issue(request, text)
        minted.set()
        try:
            await release.wait()
        except asyncio.CancelledError:
    ...
    service._issue_voice_commit_receipt = issue_then_cross_deadline  # type: ignore[method-assign]
    result = await asyncio.wait_for(
        service.recognize(_recognize_request(timeout_ms=500), CONTEXT), timeout=1.0
    )
    await asyncio.wait_for(minted.wait(), timeout=0.2)
    leaked_receipts = tuple(
        token
        for token, retained in service._voice_commit_receipts.items()
        if retained.operation_id == "operation-r0"
    )
    release.set()
    for _ in range(100):
        worker = next(iter(service._operations.values())).worker_task
    ...
    assert result["error"]["code"] == "TIMEOUT"
    assert leaked_receipts == ()
    assert foreign_receipt in service._voice_commit_receipts
    assert all(
        retained.operation_id != "operation-r0"
        for retained in service._voice_commit_receipts.values()
    )
    assert not hasattr(service, "dispatch_agent")
    assert not hasattr(service, "mutate_task")
    await service.close(timeout_ms=200)


@pytest.mark.asyncio
async def test_receipt_minted_after_timeout_fence_is_immediately_revoked() -> None:
    """L4: a cancellation-defiant worker cannot mint behind the deadline."""

    service = _service(ControlledProvider())
    original_issue = service._issue_voice_commit_receipt
    issue_entered = asyncio.Event()
    release = asyncio.Event()
    issue_settled = asyncio.Event()

    async def cross_deadline_then_issue(request, text):
        issue_entered.set()
        try:
            await release.wait()
        except asyncio.CancelledError:
            await release.wait()
        try:
            return await original_issue(request, text)
        finally:
            issue_settled.set()

    service._issue_voice_commit_receipt = cross_deadline_then_issue  # type: ignore[method-assign]
    operation = asyncio.create_task(
        service.recognize(_recognize_request(timeout_ms=100), CONTEXT)
    )
    await asyncio.wait_for(issue_entered.wait(), timeout=0.2)
    result = await asyncio.wait_for(operation, timeout=0.3)
    ...
    release.set()
    await asyncio.wait_for(issue_settled.wait(), timeout=0.2)
    assert service._voice_commit_receipts == {}
    assert not hasattr(service, "dispatch_agent")
    assert not hasattr(service, "mutate_task")
    await service.close(timeout_ms=200)
