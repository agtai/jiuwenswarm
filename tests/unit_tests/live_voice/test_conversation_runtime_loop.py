...
@pytest.mark.parametrize("worker_exit", ["cancelled", "faulted"])
async def test_worker_exit_settles_existing_close_waiter_with_typed_failure(
    worker_exit: str,
) -> None:
    ...
    runtime._shutdown_future = running.create_future()
    close_waiter = asyncio.create_task(runtime.close())
    await asyncio.sleep(0)

    worker = runtime._worker
    assert worker is not None
    if worker_exit == "cancelled":
        worker.cancel()
        expected_cause: type[BaseException] = asyncio.CancelledError
    else:
        def fail_worker() -> None:
            raise RuntimeError("deterministic worker fault")

        runtime._next_operation = fail_worker  # type: ignore[method-assign]
        assert runtime._wake is not None
        runtime._wake.set()
        expected_cause = RuntimeError

    try:
        with pytest.raises(ConversationRuntimeLoopViolation) as closed:
            await asyncio.wait_for(asyncio.shield(close_waiter), timeout=0.5)
    finally:
    ...
    assert closed.value.reason == "RUNTIME_LOOP_CLOSED"
    assert closed.value.code is ErrorCode.CANCELLED
    assert type(closed.value.__cause__) is expected_cause
    assert worker.done()
    snapshot = runtime.snapshot()
    assert snapshot.closed is True
    assert snapshot.worker_running is False
    assert snapshot.conversation.interactions == ()
    assert snapshot.effects == ()
    with pytest.raises(ConversationRuntimeLoopViolation) as later:
        await runtime.open_interaction("interaction-after-worker-exit")
    assert later.value.reason == "RUNTIME_LOOP_CLOSED"
...
def tracked_package_python_files() -> tuple[pathlib.Path, ...]:
    """Enumerate only Git-tracked Python modules shipped in the package."""

    package_root = pathlib.Path(jiuwenswarm.__file__).resolve().parent
    repository_root = package_root.parent
    try:
        completed = subprocess.run(
            ["git", "ls-files", "--", "jiuwenswarm/**/*.py"],
            cwd=repository_root,
            check=False,
            capture_output=True,
            text=True,
        ...
        raise AssertionError(
            "cannot execute tracked shipped Python enumeration"
        ) from exc
    if completed.returncode != 0:
        diagnostic = completed.stderr.strip() or "git ls-files produced no diagnostic"
        raise AssertionError(
            f"cannot enumerate tracked shipped Python modules: {diagnostic}"
        )
    ...
    if not relative_paths:
        raise AssertionError("tracked shipped Python module enumeration was empty")
    ...
    for relative in relative_paths:
        ...
        try:
            ...
            raise AssertionError(
                f"tracked Python path escaped the shipped package: {relative.as_posix()}"
            ) from exc
        if path.suffix != ".py" or not path.is_file():
            raise AssertionError(
                f"tracked shipped Python path is unreadable: {relative.as_posix()}"
            )
        files.append(path)
    return tuple(files)
...
def package_calls() -> list[tuple[str, ast.Call, ast.FunctionDef | None]]:
    ...
    package_root = pathlib.Path(jiuwenswarm.__file__).resolve().parent
    for path in tracked_package_python_files():
        origin = path.relative_to(package_root).as_posix()
        try:
            source = path.read_text(encoding="utf-8")
        except (OSError, UnicodeError) as exc:
            raise AssertionError(f"cannot read tracked shipped Python: {origin}") from exc
        if not any(family in source for family in RETAINED_VIOLATION_FAMILIES):
            continue
        try:
            tree = ast.parse(source, filename=str(path))
        except SyntaxError as exc:
            raise AssertionError(f"cannot parse tracked shipped Python: {origin}") from exc
        descend(tree, None, origin)
    return found


def test_privacy_oracle_scans_only_tracked_shipped_python() -> None:
    """B42: dependency trees cannot enter the retained-message proof basis."""

    paths = tracked_package_python_files()
    package_root = pathlib.Path(jiuwenswarm.__file__).resolve().parent
    relative_paths = {path.relative_to(package_root).as_posix() for path in paths}
    assert relative_paths
    assert {
        "server/live_voice/agent_conversation_runtime.py",
        "server/live_voice/conversation_runtime.py",
        "server/live_voice/conversation_runtime_loop.py",
        "server/live_voice/presentation_ledger.py",
    } <= relative_paths
    excluded_parts = {"node_modules", ".pnpm", ".cache"}
    assert all(excluded_parts.isdisjoint(path.parts) for path in paths)

    calls = package_calls()
    assert calls
    assert {origin for origin, _, _ in calls} <= relative_paths
