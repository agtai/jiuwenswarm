...
def test_chinese_quoted_create_name_falls_back_only_for_unicode_whitespace() -> None:
    blank = VoiceTaskBridge().resolve(
        committed("创建任务“ \u00a0 ”：检查仓库", commit_id="commit-blank-name"),
        SCOPE,
    )
    named = VoiceTaskBridge().resolve(
        committed("创建任务“仓库检查”：检查仓库", commit_id="commit-chinese-name"),
        SCOPE,
    )

    assert blank.disposition is TaskIntentDisposition.CLARIFICATION
    assert blank.operation == "task.create"
    assert blank.name == "Voice task"
    assert blank.instruction == "检查仓库"
    assert named.name == "仓库检查"
    assert named.instruction == "检查仓库"
