...
def _init_task_store(path: Path) -> None:
    with sqlite3.connect(path) as connection:
        connection.executescript(
            ...
            CREATE TABLE tasks (
                task_id TEXT PRIMARY KEY, state TEXT NOT NULL, outcome TEXT,
            ...
            CREATE TABLE outbox (
                outbox_id TEXT PRIMARY KEY, task_id TEXT NOT NULL,
                attempt_id TEXT NOT NULL, state TEXT NOT NULL,
                claimed_by TEXT, claimed_at TEXT, claim_token TEXT
            );
            CREATE TABLE {s8.DIRECT_EXECUTOR_TABLE} (
                attempt_id TEXT PRIMARY KEY, task_id TEXT NOT NULL,
                project_root TEXT NOT NULL, state TEXT NOT NULL, outcome TEXT,
                raw_status TEXT NOT NULL, owner_id TEXT, lease_expires_at TEXT
            );
            """
        )


def _settle_trace_tasks(context: Context, product_trace: dict[str, object]) -> None:
    records = product_trace["records"]
    assert isinstance(records, list)
    with sqlite3.connect(context.env["S8_TASK_STORE_PATH"]) as connection:
        for index, raw in enumerate(records, start=1):
            ...
            kinds = {
                kind
                for check_id, required in s8._OBSERVATION_IDENTITIES.items()
                if s8._OBSERVATION_SCOPES[check_id] == scope
                for kind in required
            }
            if not {"task", "attempt"}.issubset(kinds):
                continue
            ...
            connection.execute(
                "INSERT INTO tasks(task_id, state, outcome, attempt_id) "
                "VALUES(?, 'terminal', 'completed', ?)",
                (task_id, attempt_id),
            )
            connection.execute(
                "INSERT INTO attempts(attempt_id, task_id, state, outcome) "
                "VALUES(?, ?, 'terminal', 'completed')",
                (attempt_id, task_id),
            )
            connection.execute(
                "INSERT INTO outbox(outbox_id, task_id, attempt_id, state) "
                "VALUES(?, ?, ?, 'delivered')",
                (f"outbox-{index}", task_id, attempt_id),
            )
            connection.execute(
                f"INSERT INTO {s8.DIRECT_EXECUTOR_TABLE}("
                "attempt_id, task_id, project_root, state, outcome, raw_status, "
                "owner_id, lease_expires_at) VALUES(?, ?, ?, 'terminal', "
...
def _bind_report(context: Context) -> None:
...
def _rewrite_bound_files(context: Context) -> None:
    context.report["checks"] = _valid_s7_checks(
        str(context.report["candidate"]["head"]),
        context.report["runtime_declaration"],
    )
    _bind_report(context)
...
def _context(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Context:
    ...
    _run(repo, "git", "add", "pyproject.toml", ".gitignore")
    _run(
        repo,
        "git",
        "-c",
    ...
    env = {
        ...
        **{
        ...
        "JIUWENSWARM_LIVE_VOICE_P3_DATABASE": str(store),
    ...
    monkeypatch.setattr(
        s8, "_validate_frontend_deployment", lambda _origin, **_kwargs: None
    )
    monkeypatch.setattr(
        s8,
        ...
        lambda _env, **_kwargs: (
            ...
            + [
                {
                    "service": s8.PRIVATE_PROXY_SERVICE,
                    "port": s8.PRIVATE_PROXY_PORT,
                    "pid": 90_000_100,
                    "process_ref": "process_ref:sha256-" + "9" * 64,
                    "candidate_head": head,
                }
            ]
        ),
...
def _complete_record(context: Context, record: dict[str, object]) -> dict[str, object]:
    ...
    _settle_trace_tasks(context, product_trace)
...
@pytest.mark.parametrize(
    ("mutation", "reason"),
    [
        ("duplicate", "S7_CHECK_RESULT_INVALID"),
        ("automation-fail", "S7_AUTOMATION_INCOMPLETE"),
        ("latency-order", "S7_REAL_CHECK_CONTRACT_INVALID"),
        ("latency-negative", "S7_REAL_CHECK_CONTRACT_INVALID"),
        ("truncated", "S7_REAL_CHECK_CONTRACT_INVALID"),
        ("boolean-count", "S7_REAL_CHECK_CONTRACT_INVALID"),
    ],
)
def test_s7_report_details_are_rederived_fail_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
    reason: str,
    ...
    context = _context(tmp_path, monkeypatch)
    checks = context.report["checks"]
    assert isinstance(checks, list)
    if mutation == "duplicate":
        checks.append(copy.deepcopy(checks[0]))
    elif mutation == "automation-fail":
        checks[0]["status"] = "FAIL"
    else:
        real = next(item for item in checks if item["check_id"] == "speech-media")
        if mutation == "latency-order":
            real["details"]["probe_p95_ms"] = 0.5
        elif mutation == "latency-negative":
            real["details"]["probe_p50_ms"] = -1.0
        elif mutation == "boolean-count":
            real["details"]["sanitized_result_count"] = True
        else:
            real["details"]["output_truncated"] = True
    _bind_report(context)
...
@pytest.mark.parametrize(
    ("name", "value"),
    [("AGENT_SERVER_PORT", "19000"), ("WEB_PORT", "19001")],
)
def test_frozen_service_port_roles_cannot_be_relabelled(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    name: str,
    value: str,
    ...
    result = _preflight(context, monkeypatch)
    assert result["status"] == "BLOCKED"
    assert result["checks"][-1]["reason"] == "SERVICE_PORT_DECLARATION_MISMATCH"


def test_s8_store_must_equal_product_authoritative_database(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    context = _context(tmp_path, monkeypatch)
    alternate = context.data / "alternate.sqlite3"
    alternate.write_bytes(b"not-authoritative")
    context.env["S8_TASK_STORE_PATH"] = str(alternate)
    result = _preflight(context, monkeypatch)
    assert result["status"] == "BLOCKED"
    assert result["checks"][-1]["reason"] == "TASK_STORE_AUTHORITY_MISMATCH"
...
def test_s7_report_without_frozen_frontend_build_is_blocked(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    context = _context(tmp_path, monkeypatch)
    build = next(
        item
        for item in context.report["checks"]
        if item["check_id"] == "frontend-production-build"
    )
    del build["details"]["artifact_manifest_sha256"]
    _bind_report(context)

    result = _preflight(context, monkeypatch)

    assert result["status"] == "BLOCKED"
    assert result["checks"][-1]["reason"] == "S7_FRONTEND_BUILD_IDENTITY_INVALID"


@pytest.mark.parametrize(
    ("check_id", "field", "value", "reason"),
    [
        (
            "frontend-production-build",
            "vite_modules",
            0,
            "S7_FRONTEND_BUILD_RESULT_INVALID",
        ),
        (
            "candidate-identity-after-run",
            "frontend_build_unchanged",
            False,
            "S7_POSTRUN_IDENTITY_INVALID",
        ),
    ],
)
def test_s7_report_rejects_contradictory_frontend_build_result(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    check_id: str,
    field: str,
    ...
    context = _context(tmp_path, monkeypatch)
    check = next(
        item for item in context.report["checks"] if item["check_id"] == check_id
    )
...
def test_frontend_deployment_rejects_stale_ignored_dist(tmp_path: Path) -> None:
    repo = tmp_path / "candidate"
    root = repo / s8._FRONTEND_DIST_RELATIVE
    root.mkdir(parents=True)
    index = root / "index.html"
    index.write_text("frozen", encoding="utf-8")
    expected, _ = s8._collect_frontend_build(repo)
    index.write_text("stale", encoding="utf-8")

    with pytest.raises(s8.ReadinessError, match="FRONTEND_BUILD_IDENTITY_MISMATCH"):
        s8._validate_frontend_deployment(
            "https://voice.private.test", repo=repo, expected=expected
        )


def test_frontend_deployment_rejects_foreign_root_spa_fallback(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo = tmp_path / "candidate"
    root = repo / s8._FRONTEND_DIST_RELATIVE
    ...
    index_bytes = b"<main>candidate</main>"
    (root / "index.html").write_bytes(index_bytes)
    (assets / "app.js").write_bytes(b"export const candidate=true")
    expected, _ = s8._collect_frontend_build(repo)
    ...
    class Response:
        ...
        def getheader(self, name: str, default: str | None = None) -> str | None:
            if name == "Content-Length":
                return str(len(index_bytes))
            return default

        def read(self, size: int) -> bytes:
            return index_bytes[:size]

    class ForeignRootConnection:
        def __init__(self, *_args: object, **_kwargs: object) -> None:
            pass
    ...
    monkeypatch.setattr(s8.http.client, "HTTPSConnection", ForeignRootConnection)

    with pytest.raises(
        s8.ReadinessError,
        match="FRONTEND_DEPLOYMENT_(LENGTH_INVALID|CONTENT_MISMATCH)",
    ):
        s8._validate_frontend_deployment(
            "https://voice.private.test", repo=repo, expected=expected
        )
...
@pytest.mark.parametrize(
    ("status", "encoding", "reason"),
    [
        (302, "", "FRONTEND_DEPLOYMENT_RESPONSE_INVALID"),
        (200, "gzip", "FRONTEND_DEPLOYMENT_ENCODING_INVALID"),
    ],
)
def test_frontend_deployment_rejects_redirect_or_encoded_content(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    status: int,
    encoding: str,
    ...
    class Response:
        ...
        def getheader(self, name: str, default: str | None = None) -> str | None:
            ...
            if name == "Content-Encoding":
                return encoding
            if name == "Location" and status == 302:
                return "/foreign"
            return default
    ...
    with pytest.raises(s8.ReadinessError, match=reason):
        s8._validate_frontend_deployment(
            "https://voice.private.test", repo=repo, expected=expected
        )
...
def test_listener_process_outside_candidate_is_rejected(
    ...
    listeners = [
        ...
        SimpleNamespace(
            status="LISTEN",
            laddr=SimpleNamespace(port=s8.PRIVATE_PROXY_PORT),
            pid=43,
        )
    ]
...
def test_external_private_proxy_listener_is_bound_separately_from_candidate(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    real_capture = s8._capture_service_processes
    context = _context(tmp_path, monkeypatch)
    monkeypatch.setattr(s8, "_capture_service_processes", real_capture)
    candidate_pids = {
        int(context.env[env_name]): 100 + index
        for index, env_name in enumerate(s8.SERVICE_PORT_ENV.values(), start=1)
    }
    listeners = [
        SimpleNamespace(
            status="LISTEN",
        ...
        for port, pid in candidate_pids.items()
    ] + [
        SimpleNamespace(
            status="LISTEN",
            laddr=SimpleNamespace(port=s8.PRIVATE_PROXY_PORT),
    ...
    class Process:
        ...
        def cwd(self) -> str:
            return str(context.repo if self.pid != 200 else tmp_path)

    monkeypatch.setattr(psutil, "Process", Process)
    monkeypatch.setattr(
        s8,
        "_process_ref",
        lambda pid: "process_ref:sha256-" + f"{pid:064x}",
    )

    result = s8._capture_service_processes(
        context.env,
        repo=context.repo,
        candidate_head=str(context.report["candidate"]["head"]),
    )

    assert {item["service"] for item in result} == s8.SESSION_PROCESS_SERVICES
    proxy = next(item for item in result if item["service"] == s8.PRIVATE_PROXY_SERVICE)
    assert proxy["port"] == 443
    assert proxy["pid"] == 200
...
def test_session_rejects_stale_identity_and_fake_human_pass(
    ...
    placeholder["observations"]["S8-02.P1.HEARD_PLAYOUT"]["reason_code"] = (
        "USER_OBSERVATION_REQUIRED"
    )
    with pytest.raises(
        s8.ReadinessError, match="SESSION_OBSERVATION_REASON_INCONSISTENT"
    ):
        s8._validate_session_record(
            placeholder, trace_identities=record["identities"], final=True
        )
...
def test_session_enforces_scoped_identities_and_complete_task_settlement(
    ...
    nonterminal = copy.deepcopy(record)
    nonterminal["task_settle"][0]["terminal_state"] = "unknown"
    with pytest.raises(s8.ReadinessError, match="SESSION_TASK_NOT_SETTLED"):
        s8._validate_session_record(
            nonterminal, trace_identities=record["identities"], final=True
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "UPDATE tasks SET state='running' WHERE rowid=(SELECT min(rowid) FROM tasks)",
            "TASK_STORE_TASK_NOT_TERMINAL",
        ),
        (
            "UPDATE outbox SET state='claimed', claimed_by='worker', "
            "claimed_at='now', claim_token='token' "
            "WHERE rowid=(SELECT min(rowid) FROM outbox)",
            "TASK_STORE_OUTBOX_NOT_SETTLED",
        ),
        (
            f"UPDATE {s8.DIRECT_EXECUTOR_TABLE} SET raw_status="
            "'completed_cleanup_pending' WHERE rowid=(SELECT min(rowid) FROM "
            f"{s8.DIRECT_EXECUTOR_TABLE})",
            "TASK_STORE_EXECUTOR_NOT_SETTLED",
        ),
        (
            f"UPDATE {s8.DIRECT_EXECUTOR_TABLE} SET owner_id='worker', "
            "lease_expires_at='later' WHERE rowid=(SELECT min(rowid) FROM "
            f"{s8.DIRECT_EXECUTOR_TABLE})",
            "TASK_STORE_EXECUTOR_NOT_SETTLED",
        ),
    ],
)
def test_cleanup_rejects_unsettled_authoritative_task_store(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    sql: str,
    reason: str,
    ...
    context = _context(tmp_path, monkeypatch)
    record = _complete_record(context, _record(context, monkeypatch))
    record_path = context.report_path.parent / "unsettled-session.json"
    _write_json(record_path, record)
    with sqlite3.connect(context.env["S8_TASK_STORE_PATH"]) as connection:
...
def test_cleanup_rejects_untraced_executor_owner(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, foreign_root: bool
) -> None:
    context = _context(tmp_path, monkeypatch)
    record = _complete_record(context, _record(context, monkeypatch))
    record_path = context.report_path.parent / "extra-owner-session.json"
    _write_json(record_path, record)
    with sqlite3.connect(context.env["S8_TASK_STORE_PATH"]) as connection:
        connection.execute(
            f"INSERT INTO {s8.DIRECT_EXECUTOR_TABLE}("
            "attempt_id, task_id, project_root, state, outcome, raw_status, "
            "owner_id, lease_expires_at) VALUES('foreign-attempt', 'foreign-task', "
            "?, 'terminal', 'completed', 'completed', NULL, NULL)",
            (
                str(
                    (tmp_path / "foreign-project").resolve()
                    if foreign_root
                    else context.fixture.resolve()
                ),
            ),
        )
    monkeypatch.setattr(s8, "_port_open", lambda *_args, **_kwargs: False)

    with pytest.raises(s8.ReadinessError, match="TASK_STORE_OWNERSHIP_NOT_EXCLUSIVE"):
        s8.run_cleanup(
            repo=context.repo,
            s7_report_path=context.report_path,
            handoff_path=context.handoff_path,
...
def test_cleanup_rejects_untraced_store_only_task_effect(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    context = _context(tmp_path, monkeypatch)
    record = _complete_record(context, _record(context, monkeypatch))
    record_path = context.report_path.parent / "store-only-session.json"
    _write_json(record_path, record)
    with sqlite3.connect(context.env["S8_TASK_STORE_PATH"]) as connection:
        connection.execute(
            "INSERT INTO tasks(task_id, state, outcome, attempt_id) "
            "VALUES('foreign-task', 'terminal', 'cancelled', 'foreign-attempt')"
        )
        connection.execute(
            "INSERT INTO attempts(attempt_id, task_id, state, outcome) "
            "VALUES('foreign-attempt', 'foreign-task', 'terminal', 'cancelled')"
        )
        connection.execute(
            "INSERT INTO outbox(outbox_id, task_id, attempt_id, state) "
            "VALUES('foreign-outbox', 'foreign-task', 'foreign-attempt', 'suppressed')"
        )
    monkeypatch.setattr(s8, "_port_open", lambda *_args, **_kwargs: False)
...
def test_cleanup_rejects_product_trace_replaced_after_manifest_validation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    context = _context(tmp_path, monkeypatch)
    record = _complete_record(context, _record(context, monkeypatch))
    record_path = context.report_path.parent / "trace-swap-session.json"
    _write_json(record_path, record)
    monkeypatch.setattr(s8, "_port_open", lambda *_args, **_kwargs: False)
    original = s8._load_json_bytes
    reads = 0
    ...
    def swap_after_verified_read(
        ...
        nonlocal reads
        result = original(path, repo, schema)
        if path.resolve() == context.product_trace_path.resolve():
            reads += 1
            if reads == 1:
                tampered = json.loads(
                    context.product_trace_path.read_text(encoding="utf-8")
                )
                task_record = next(
                    item
                    for item in tampered["records"]
                    if item["scope"] == "p3.structured"
                )
                task_record["observation"]["binding"]["task_id"] = "task-swapped"
                _write_json(context.product_trace_path, tampered)
        return result

    monkeypatch.setattr(s8, "_load_json_bytes", swap_after_verified_read)

    with pytest.raises(s8.ReadinessError, match="PRODUCT_TRACE_CHANGED_AFTER_BINDING"):
        s8.run_cleanup(
            repo=context.repo,
            s7_report_path=context.report_path,
            handoff_path=context.handoff_path,
