...
@pytest.mark.asyncio
@pytest.mark.parametrize("effect_ack_first", [False, True])
async def test_acknowledged_effect_claim_survives_presentation_state_change(
    effect_ack_first: bool,
) -> None:
    """B5: either ACK order retains the exact settled effect claim."""
    ...
    lease = current.attach_notification_consumer(
        consumer_id=f"effect-order-{effect_ack_first}", connection_epoch=0
    )
    handle = await dispatch(current, selected)
    notifications = [
        await asyncio.wait_for(current.next_notification_for(lease), timeout=1)
        for _ in range(4)
    ]
    presentation = next(
    ...
    claim = await current.claim_conversation_effects(
        lease, claim_id=f"claim-effect-order-{effect_ack_first}"
    )
    effect_ids = tuple(effect.effect_id for effect in claim.effects)
    assert [effect.effect_type for effect in claim.effects] == ["ui.render"]
    ack = PresentationAck(
        ref=handle.response_ref,
        surface=PresentationSurface.TEXT,
    ...
    if effect_ack_first:
        effect_ack = await current.acknowledge_conversation_effects(
            lease, claim_id=claim.claim_id, effect_ids=effect_ids
        )
        presentation_ack = await current.acknowledge_presentation(ack)
    else:
        presentation_ack = await current.acknowledge_presentation(ack)
        effect_ack = await current.acknowledge_conversation_effects(
            lease, claim_id=claim.claim_id, effect_ids=effect_ids
    ...
    empty_claim = await current.claim_conversation_effects(
        lease, claim_id="claim-after-presented-effect"
    )
    empty_replay = await current.claim_conversation_effects(
        lease, claim_id="claim-after-presented-effect"
    )

    assert presentation_ack.accepted is True
    assert effect_ack.accepted is True
    assert replayed.effects == claim.effects
    assert replayed.acknowledged is True
    assert empty_claim.effects == ()
    assert empty_claim.replayed is False
    assert empty_claim.acknowledged is False
    assert empty_replay.effects == ()
    assert empty_replay.replayed is True
    assert empty_replay.acknowledged is False
...
@pytest.mark.asyncio
async def test_failed_published_task_presentations_release_exact_critical_capacity() -> (
    None
):
    """B4: public Task failure returns its notice slot but retains replay refusal."""
    ...
    await current.start()
    await current.open_interaction("interaction-1")
    failed: list[tuple[str, ResponseRef]] = []

    for index in range(current.snapshot().notification_critical_capacity + 1):
        request_id = f"published-task-failure-{index}"
        handle = await current.present_authoritative_text(
            request_id=request_id,
            response_id=f"published-task-response-{index}",
            correlation_id=f"published-task-correlation-{index}",
            commit=commit(
                turn_id=f"published-task-turn-{index}",
                commit_id=f"published-task-commit-{index}",
                text=f"Published Task failure {index}",
            ),
            text="The background task audio could not be presented.",
            channel_id="web",
            _persist_user_history=False,
            _source_provenance="server.task_notification",
            _presentation_surface=PresentationSurface.AUDIO,
        ...
        assert current.snapshot().queued_critical_notifications == 0
        assert current._task_presentation_reservations == {}
        failed.append((request_id, handle.response_ref))

    successor = await current.present_authoritative_text(
        request_id="published-task-successor",
        response_id="published-task-successor-response",
        correlation_id="published-task-successor-correlation",
        commit=commit(
            turn_id="published-task-successor-turn",
            commit_id="published-task-successor-commit",
            text="Published Task successor",
        ),
        text="The successor background task is ready.",
        channel_id="web",
        _persist_user_history=False,
        _source_provenance="server.task_notification",
        _presentation_surface=PresentationSurface.AUDIO,
    )
    successor_reservation = current.task_presentation_runtime_authority(
        successor.response_ref, None, "reserve"
    )
    assert successor_reservation.active is True
    assert current.snapshot().queued_critical_notifications == 1
    assert await current.fail_task_presentation(
        successor.response_ref,
        successor_reservation.reservation_id,
        reason="task_audio_playout_failed",
    )
    assert current.snapshot().queued_critical_notifications == 0

    # The oldest released identity has moved beyond the exact tombstones into
    # the bounded replay fence.  Reusing it still fails before publication and
    # cannot reach Agent, Tool, Task, history, bridge, or harness authorities.
    oldest_request_id, oldest_ref = failed[0]
    before = current.snapshot()
    replay_notification = presentation_notification(oldest_request_id)
    assert replay_notification.presentation_unit is not None
    with pytest.raises(AgentConversationRuntimeViolation) as replayed:
        current._publish(
            replace(
                replay_notification,
                response_ref=oldest_ref,
                presentation_unit=replace(
                    replay_notification.presentation_unit,
                    ref=oldest_ref,
                ),
            ),
            critical_key=("presentation", oldest_request_id),
        )
    assert replayed.value.reason == "DUPLICATE_CRITICAL_NOTIFICATION"
...
@pytest.mark.asyncio
async def test_interaction_close_waits_for_selected_latest_response_terminal() -> None:
    """D2: an older terminal cannot close the selected latest generation."""

    class OrderedTerminalAdapter(LowerFormalAdapter):
        def __init__(self) -> None:
            super().__init__(final=None)
            self.entered: dict[str, asyncio.Event] = {}
            self.terminal_releases: dict[str, asyncio.Event] = {}
            self.cancel_cleanup_entered: dict[str, asyncio.Event] = {}
            self.cancel_cleanup_releases: dict[str, asyncio.Event] = {}
        ...
        async def process_formal_live_voice_stream_impl(
            ...
            try:
                await self.terminal_releases.setdefault(
                    request.request_id, asyncio.Event()
                ).wait()
                yield AgentResponseChunk(
                    request_id=request.request_id,
                    channel_id=request.channel_id,
                ...
                if request.request_id != "request-close-generation-2":
                    raise
                self.cancel_cleanup_entered.setdefault(
                    request.request_id, asyncio.Event()
                ).set()
                await self.cancel_cleanup_releases.setdefault(
                    request.request_id, asyncio.Event()
                ).wait()
                raise

    lower = OrderedTerminalAdapter()
    history = RecordingHistoryWriter()
    current = runtime(lower, history, max_requests=2, notification_capacity=16)

    async def wait_for_terminal(response_ref: ResponseRef) -> None:
        for _ in range(16):
            notification = await asyncio.wait_for(
                current.next_notification(), timeout=1
            )
            if notification.response_ref != response_ref:
                continue
            if notification.progress_event is None:
                continue
            progress = WorkProgressEventV2.from_dict(
            ...
            if progress.state.value == "terminal":
                return
        raise AssertionError("exact response terminal notification was not published")

    await current.start()
    await current.open_interaction("interaction-close-generations")
    first_commit = commit(
        interaction_id="interaction-close-generations",
        turn_id="turn-close-generation-1",
        commit_id="commit-close-generation-1",
        text="first generation",
    )
    lower.entered["request-close-generation-1"] = asyncio.Event()
    lower.terminal_releases["request-close-generation-1"] = asyncio.Event()
    first = await submit(
        current,
        first_commit,
        request_id="request-close-generation-1",
        response_id="response-close-generation-1",
    )
    await asyncio.wait_for(
        lower.entered["request-close-generation-1"].wait(), timeout=1
    )

    second_commit = commit(
        interaction_id="interaction-close-generations",
        turn_id="turn-close-generation-2",
        commit_id="commit-close-generation-2",
        text="second generation",
    )
    lower.entered["request-close-generation-2"] = asyncio.Event()
    lower.terminal_releases["request-close-generation-2"] = asyncio.Event()
    lower.cancel_cleanup_entered["request-close-generation-2"] = asyncio.Event()
    lower.cancel_cleanup_releases["request-close-generation-2"] = asyncio.Event()
    second = await submit(
        current,
        second_commit,
        request_id="request-close-generation-2",
        response_id="response-close-generation-2",
    )
    await asyncio.wait_for(
        lower.entered["request-close-generation-2"].wait(), timeout=1
    )
    ...
    accepted = await current.close_interaction(
        cancel_command(
            second,
            second_commit,
            command_id="cancel-close-generation-2",
        )
    )
    assert accepted.accepted is True
    await asyncio.wait_for(
        lower.cancel_cleanup_entered["request-close-generation-2"].wait(),
        timeout=1,
    )

    lower.terminal_releases["request-close-generation-1"].set()
    await asyncio.wait_for(first.completion, timeout=1)
    await wait_for_terminal(first.response_ref)
    interaction = next(
        item
        for item in current.snapshot().conversation.conversation.interactions
        if item.interaction_id == "interaction-close-generations"
    )
    state_after_old_terminal = interaction.state
    second_done_after_old_terminal = second.completion.done()
    cancel_effects_after_old_terminal = current.snapshot().harness.cancel_effects

    lower.cancel_cleanup_releases["request-close-generation-2"].set()
    await asyncio.wait_for(second.completion, timeout=1)
    await wait_for_terminal(second.response_ref)
    interaction = next(
        item
        for item in current.snapshot().conversation.conversation.interactions
        if item.interaction_id == "interaction-close-generations"
    )
    final_state = interaction.state
    final_cancel_effects = current.snapshot().harness.cancel_effects
    user_history_count = len(history.users)
    assistant_history = tuple(history.assistant_intents)
    lower_calls = lower.calls
    closed = await current.close(timeout_seconds=1)

    assert final_state is InteractionState.CLOSED
    assert cancel_effects_after_old_terminal == 1
    assert final_cancel_effects == 1
    assert user_history_count == 2
    assert assistant_history == ()
    assert lower_calls == 2
    assert closed.status is (
        AgentConversationShutdownStatus.CLOSED
    )
    assert state_after_old_terminal is InteractionState.CLOSING
    assert second_done_after_old_terminal is False


@pytest.mark.asyncio
async def test_rejected_cancel_commands_have_bounded_replay_state() -> None:
    """L5: 257 wrong-scope identities fail closed without evicting exact replay."""
    ...
    for _ in range(2):
        await asyncio.wait_for(current.next_notification(), timeout=1)
    user_history_before = tuple(history.users)
    assistant_history_before = tuple(history.assistant_intents)

    wrong_scope = scope(session_id="other-session")
    first_command = cancel_command(
        handle,
        selected,
        command_id="cancel-capacity-0",
        current_scope=wrong_scope,
    )
    first = await current.close_interaction(first_command)
    ...
    for index in range(1, 257):
        rejected = await current.close_interaction(
            cancel_command(
                handle,
                selected,
                command_id=f"cancel-capacity-{index}",
                current_scope=wrong_scope,
            )
        )
        assert rejected.accepted is False
    ...
    replay = await current.close_interaction(first_command)
    ...
    overflow_replay = await current.close_interaction(
        cancel_command(
            handle,
            selected,
            command_id="cancel-capacity-256",
            current_scope=wrong_scope,
        )
    )

    assert first.accepted is False
    assert first.reason == "ROUND_CANCEL_SCOPE_MISMATCH"
    assert all(reason in {"ROUND_CANCEL_SCOPE_MISMATCH", "CANCEL_REJECTED"} for reason in reasons)
    assert replay.replayed is True
    assert replay.reason == first.reason
    assert conflict.value.reason == "IDEMPOTENCY_CONFLICT"
    assert overflow_replay.accepted is False
    assert overflow_replay.reason == reasons[-1]
    assert len(current._harness._cancel_results) <= current._harness._max_reservations
    assert current.snapshot().harness.cancel_effects == 0
    assert tuple(history.users) == user_history_before
    assert tuple(history.assistant_intents) == assistant_history_before
    assert lower.calls == 1
