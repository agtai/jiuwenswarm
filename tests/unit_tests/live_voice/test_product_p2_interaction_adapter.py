...
@pytest.mark.asyncio
async def test_failed_successor_allocation_retains_closed_generation_high_water_and_rejects_old_replay() -> None:
    resolver = RecordingResolver((candidate(),))
    runtime_allocations = 0
    engine_allocations = 0

    def make_runtime(_context, _binding):
        nonlocal runtime_allocations
        runtime_allocations += 1
        if runtime_allocations == 2:
            raise RuntimeError("successor runtime unavailable")
        return FakeRuntime()
    ...
    failed_successor = await adapter.activate(
        request(activation_id="activation-2", generation=2)
    )
    stale_replay = await adapter.activate(request())

    assert closed.status is P2LeaseCloseStatus.CLOSED
    assert failed_successor.status is P2ActivationStatus.FAILED
    assert failed_successor.reason is P2ActivationReason.RUNTIME_FACTORY_FAILED
    assert stale_replay.status is P2ActivationStatus.DENIED
    assert stale_replay.reason is P2ActivationReason.ACTIVATION_BINDING_CONFLICT
    assert stale_replay.lease is None
    assert runtime_allocations == 2
    assert engine_allocations == 1
