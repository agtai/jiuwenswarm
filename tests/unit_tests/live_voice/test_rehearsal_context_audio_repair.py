...
from jiuwenswarm.channels.live_voice.product_composition_registry import AgentServerProductCompositionRegistry
...
def test_observability_uses_installed_setup_exports(monkeypatch):
    from openjiuwen.harness.observability import open_agent_run_span
    from openjiuwen.harness.observability import setup as agent_setup
    from openjiuwen.extensions.observability import setup

    assert callable(setup.get_tracer)
    tracer_calls = []

    def forbidden_tracer(*args):
        tracer_calls.append(args)
        raise AssertionError("disabled observability acquired a tracer")

    monkeypatch.setattr(setup, "get_tracer", forbidden_tracer)
    monkeypatch.setattr(setup, "is_initialized", lambda: False)
    monkeypatch.setattr(agent_setup, "is_tracing_enabled", lambda: True)
    assert open_agent_run_span() is None
