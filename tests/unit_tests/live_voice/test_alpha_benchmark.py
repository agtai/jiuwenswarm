...
@pytest.mark.asyncio
async def test_deterministic_runner_closes_sample_latency_and_failure_counts() -> None:
    ticks = iter((0.0, 0.01, 0.01, 0.03, 0.03, 0.10))
    calls: list[str] = []

    def operation(name: str, *, fail: bool = False):
        async def run() -> None:
            calls.append(name)
            if fail:
                raise OSError("private provider detail must not enter the report")

        return run

    execution = await run_alpha_benchmark(
        enabled=True,
        plan=plan(targets=(target(max_failure_count=1),)),
        cases=(
            AlphaBenchmarkCase(
                "case-1", "rm-ingress", "correlation-1", operation("one")
            ),
            AlphaBenchmarkCase(
                "case-2",
                "rm-ingress",
                "correlation-2",
                operation("two", fail=True),
            ),
            AlphaBenchmarkCase(
                "case-3", "rm-ingress", "correlation-3", operation("three")
            ),
        ),
    ...
    assert execution.samples[1].failed is True
...
@pytest.mark.asyncio
async def test_deterministic_runner_flag_off_reads_no_cases_or_clock() -> None:
    class Poison:
    ...
    execution = await run_alpha_benchmark(
        ...
        plan=Poison(),
        cases=Poison(),
        monotonic=cast(object, Poison()),
    )
    assert execution.enabled is False
    assert execution.samples == ()
    assert execution.report.alpha_gate_pass is False
