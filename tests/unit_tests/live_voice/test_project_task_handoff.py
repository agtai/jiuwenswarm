...
from openjiuwen.core.application.tasks.project_executor import (DirectProjectManagedBaselineReader)
...
async def handoff(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    current_time = [NOW]
    monkeypatch.setattr(
        "openjiuwen.core.application.tasks.task_store.utc_now", lambda: current_time[0]
    )
