...
Four words replacing an allow-list of conversation ids. The tests here cover the
...
about how the word is arrived at and carried, which is a separate failure: a
...
def test_the_four_words_are_ordered_and_the_default_is_the_narrowest() -> None:
    ...
    assert HISTORY_POLICY_NAMES_A_TARGET == {HISTORY_MEMBERS, HISTORY_OPEN}
...
def test_a_misspelled_word_lands_on_the_default_and_skips_the_legacy_key() -> None:
    ...
    Falling through would let ``histry: members`` -- or ``shared``, from the
    superseded sketch -- silently buy whatever the old list happened to say,
    which is the reading an operator would never guess from what they wrote.
    """
...
def test_absent_and_empty_both_deny_nothing() -> None:
    ...
    list would have carried: for a list of things that are *forbidden*, writing
