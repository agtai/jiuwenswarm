...
def test_sdk_loggers_are_wired_to_the_handlers_that_already_redact() -> None:
    ...
    Every handler on the ``jiuwenswarm`` logger carries ``SensitiveDataFilter``,
    ...
    try:
        ...
        # tree's level below would not reach one of them. It carries no handlers
...
def test_a_bolt_logger_reaches_the_shared_handlers_exactly_once() -> None:
    ...
    propagating, so a base logger carrying handlers writes every bolt line
