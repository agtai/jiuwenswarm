...
async def test_a_payload_refused_after_validation_passed_still_degrades() -> None:
    ...
    # The whole chunk is delivered, exactly as it was before this feature
    # existed -- and now with the reader told why it looks plain.
...
def test_an_edit_empties_the_blocks_rather_than_removing_them() -> None:
    """The field stays, carrying nothing. Omitting it is what leaves them up."""
...
class _RefusingEditClient(_ValidatingClient):
    """Refuses any payload that still carries a rendering, on either call.

    Distinct from ``_RefusingClient``, which refuses on the presence of the
    field: an edit that has had its rendering taken away still carries it,
...
class _EchoingRefusingClient(_ValidatingClient):
    """Refuses the rendering, then echoes back the text the *first* call carried.
