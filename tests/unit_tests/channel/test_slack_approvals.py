...
def _styles(blocks: list[dict[str, Any]]) -> dict[str, str | None]:
    ...
    ``None`` rather than "" so that a button carrying no ``style`` key is not
    confused with one carrying an empty value, which Slack would refuse.
    """
    return {
...
def _legend(blocks: list[dict[str, Any]]) -> str | None:
    """The options' descriptions, or ``None`` when no block carries them.
...
async def test_each_permission_action_is_styled_by_what_it_does() -> None:
    """The four canonical actions, each carrying the style its effect earns."""
    channel, client = _channel()
...
async def test_only_the_first_question_is_rendered(
    ...
    assert len(client.posts) == 1
    assert "first of 2 questions" in caplog.text
...
async def test_a_message_in_the_same_session_withdraws_the_question() -> None:
    ...
    # The message that withdrew it is still delivered, unchanged.
