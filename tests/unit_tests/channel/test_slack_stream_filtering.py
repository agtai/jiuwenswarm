...
A stream carries the answer and a running report of how it is being built. Only
...
async def test_the_answer_and_its_failures_are_still_delivered(tmp_path) -> None:
    ...
    # this turn, and a message may carry a plan or the reply but not both -- so
...
async def test_a_question_for_the_user_still_reaches_the_channel(tmp_path) -> None:
    ...
    # answer buttons rather than as text, because a question carries its
