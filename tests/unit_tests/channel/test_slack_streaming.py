...
async def test_deltas_post_once_then_edit_that_message(tmp_path) -> None:
    ...
    # The edit carries the whole snapshot, not the increment, and joins the two
...
async def test_an_abandoned_stream_reports_why_at_the_close(
    ...
    """The one exit path that used to be silent is the one that moves the reply.

    ``write`` latched a bare bool and dropped the error, so ``_close_stream``
    returned no handle and said nothing; the reply then posted fresh, below an
    abandoned preview, with nothing in the log connecting the two.
    """
    channel = _channel(tmp_path)
    client = _FakeSlackClient(update_error=RuntimeError("message_not_found"))
    channel._client = client
...
async def test_a_faithful_echo_is_not_reported(
    ...
    Comparing for equality would warn on every message carrying a link, so only
...
async def test_streams_are_bounded_and_aged_out(tmp_path) -> None:
    # A turn whose terminal event carries no text never closes its stream.
    channel = _channel(tmp_path)
    channel._client = _FakeSlackClient()
...
#
# Everything above this line is the "edit" mode the boolean flag has always
# meant, and it is kept exactly as it was: a deployment rolled back onto it has
# to behave the way it behaved before this file grew a second half.
# ---------------------------------------------------------------------------
...
def _text_of(call: dict[str, Any]) -> str:
    """The markdown a start, append or stop call carries.
...
async def test_the_terminal_event_appends_the_tail_and_stops_the_stream(
    ...
    # close, which is what carries it.
...
async def test_a_threaded_report_streams_as_one_message(tmp_path) -> None:
    ...
    # The marker stops the stream where it stands; the close carries the rest.
...
async def test_a_failed_close_that_cannot_be_recovered_from_still_raises(
    ...
    ``close()`` used to be a no-op because ``send()`` made the closing rewrite
    itself and raised on it. The closing call now lives in the surface, and the
    obligation to raise when the reply does not land has to survive the move.
    """
    channel = _streaming_channel(tmp_path)
    client = _FakeStreamingClient(
...
async def test_an_expired_stream_is_rewritten_whole_and_stays_one_message(
    ...
    # blocks its chunks built, and an edit carrying text alone would leave the
...
async def test_a_reply_too_long_to_rewrite_is_posted_beside_the_stream(
    ...
    # message already holds more than an edit can carry, so a rewrite could
...
# model writes before it calls a tool is streamed; the terminal event carries
...
async def test_a_narrated_turn_stays_one_message(tmp_path) -> None:
    ...
    # The close carries the tail alone. Everything before it is on screen.
    assert client.stops == [
        {
            "channel": "C1",
            "ts": "1710000099.000001",
...
async def test_a_rendering_slack_refuses_costs_the_formatting_not_the_reply(
    ...
    # being dropped with the rendering that carried it.
...
def test_only_text_that_cannot_change_its_mind_is_streamable(text, expected) -> None:
    from jiuwenswarm.gateway.channel_manager.im_platforms.slack import slack_blocks
...
async def test_a_channel_with_only_half_a_recipient_is_not_streamed(
    ...
    """A call carrying one of the two is refused with missing_recipient_team_id."""
    channel = _channel(tmp_path, enable_streaming=True)
    client = _FakeStreamingClient()
    channel._client = client
...
async def test_a_slack_token_the_model_wrote_crosses_untouched(tmp_path) -> None:
    ...
    # Two lines, because a line carrying a pipe cannot be judged a table header
...
async def test_every_streaming_call_is_made_in_chunk_mode(tmp_path) -> None:
    ...
    call that has to carry chunks -- so the defect hides until a reply ends in
