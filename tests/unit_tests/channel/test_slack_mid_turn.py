...
def _room_event(*, user: str, ts: str, text: str = "keep going") -> dict:
    return {
        "type": "message",
...
def _accepted(
    *,
    ...
    back as a response whose payload carries the name and may not have been
...
# --------------------------------------------------------------------------
# cancel -- the default, and it must be exactly what was there before
# --------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_cancel_is_what_an_unconfigured_connector_does() -> None:
    # Not the patched seam: the real method, so that "the default is cancel" is
    # a fact about the connector rather than about this file's fixture.
    channel = SlackChannel(
...
async def test_under_cancel_a_second_message_dispatches_and_takes_the_turn() -> None:
    ...
    # Two ordinary requests, neither carrying a steer, and the initiator record
...
async def test_a_steer_leaves_the_initiator_with_whoever_started_the_turn() -> None:
    ...
    # is running as. This is D15's floor: A may stop the turn B steered.
...
async def test_a_full_queue_refuses_the_newest_and_says_so(warnings) -> None:
    ...
    # The newest, never the oldest: every message already waiting carries a mark
...
async def test_the_record_says_which_notice_and_not_what_it_said(records) -> None:
    ...
    These carry user-facing prose, and elsewhere in this connector they quote a
