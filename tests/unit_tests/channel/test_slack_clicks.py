...
* **F1** -- no ``clicks`` rule, so layer 0 decides and nothing here changes. The
  first two tests are the whole of that guarantee: a deployment that has written
  no ``clicks:`` block behaves exactly as it did.
* **F2** -- a rule matches and the clicker cannot be placed, so the click is
  refused with a reason naming the conversation. Two ways to be unplaceable, and
  both arrive as an id that is in no list: none came, or one came and no
  ``people:`` entry maps it into the role the rule names.
* **F3/D15** -- the starter's allowance is a floor. A ``clicks.stop`` rule
  narrows who *else* may stop a turn and can never lock the starter out of
  stopping their own.

One thing these tests do not cover because it is not this section's: the sibling
guard on the answer path skips the allow list when the payload carries no user
...
async def test_with_no_clicks_rule_anyone_in_the_channel_still_answers():
    """F1, and it is the whole compatibility guarantee of this section."""
...
async def test_a_click_that_names_nobody_is_refused_where_a_rule_matches():
    """F2's first case, and the one that meets the known sibling guard.

    ``if user_id and not self.is_allowed(user_id)`` skips the allow list on a
    payload carrying no user, which is the permissive reading of exactly this
...
async def test_a_role_that_names_nobody_here_refuses_every_answer():
    """F2's second case: the rule is honourable and it admits nobody."""
    channel, _client, dispatched = _channel(
...
async def test_clicks_stop_narrows_who_else_may_stop_a_turn():
    ...
    channel._remember_turn_initiator(SESSION, MEMBER, REQUEST_ID, is_dm=False)
...
async def test_the_starter_stops_their_own_turn_through_a_rule_naming_others():
    """D15 as a floor. The rule narrows who *else* may; it cannot reach this.
...
def test_slack_identifies_the_clicker():
    ...
    # which is F2 rather than an ineffective section.
    declared = channel_capabilities("slack")
