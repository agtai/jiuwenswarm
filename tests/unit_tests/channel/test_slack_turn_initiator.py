...
carrying the session and the channel and no user at all, and nothing downstream
carries one back either -- a ``chat.send`` reaching the gateway cancels whatever
...
def _channel(**overrides: Any) -> tuple[SlackChannel, list[Message]]:
...
async def test_an_entry_older_than_any_turn_is_not_reported_as_live() -> None:
    channel, _received = _channel()
    channel._remember_turn_initiator(DM_SESSION, ASKER, "req-1", is_dm=True)
...
async def test_the_map_is_capped_however_many_sessions_are_open() -> None:
    ...
    for index in range(slack_connect._MAX_TURN_INITIATORS + 40):
        channel._remember_turn_initiator(
            f"session-{index}", ASKER, f"req-{index}", is_dm=True
        )
...
async def test_a_thread_turn_records_that_it_came_from_a_channel() -> None:
    """The regime the entry carries real information in.
...
async def test_a_resumed_dm_turn_keeps_the_surface_it_started_on() -> None:
    """Carried through the question, not re-decided at the click.
