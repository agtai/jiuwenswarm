...
``open_slack_file`` is the history gate applied to a file. A file's audience is
...
def _hosted_file(
    ...
    **overrides: Any,
    ...
    by conversation id, each value a list of shares carrying the message ``ts``;
...
class _Workspace:
    ...
    async def users_info(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return {"ok": True, "user": {"id": user_id, "profile": {"display_name": name}}}
...
def _open(toolkit: SlackHistoryToolkit, file_id: str = _FILE) -> dict[str, Any]:
    return json.loads(asyncio.run(toolkit.open_slack_file(file_id)))
...
def test_the_uploader_is_named_the_way_a_messages_author_is(
    ...
    ``files.info`` carries a ``username`` that Slack fills in for an app upload
...
def test_no_directory_call_is_made_when_the_lookup_bound_is_zero(
    ...
    An unresolved author keeps the name the record already carried, which for an
...
def test_an_app_upload_keeps_the_name_the_record_carries(
    ...
    An app upload has no user behind it: the record carries the display name in
...
def test_a_request_without_trusted_slack_context_is_refused(
    ...
    result = json.loads(asyncio.run(toolkit.open_slack_file(_FILE)))
...
# carries them on the same turn, and a rule stated two ways there reads as two
...
def test_the_shared_rules_are_worded_the_way_the_other_cards_word_them() -> None:
    description = _cards(_toolkit(_Workspace()))["open_slack_file"].description
...
def test_the_card_says_it_returns_a_path_and_takes_only_a_file_id() -> None:
    card = _cards(_toolkit(_Workspace()))["open_slack_file"]
...
def test_the_history_card_points_at_this_tool_for_the_id_it_returns() -> None:
    ...
    assert "Each such entry's id is what open_slack_file takes" in description


def test_both_tools_are_mounted_by_one_toolkit_in_a_stable_order() -> None:
    tools = _toolkit(_Workspace()).get_tools()
    names = [
        (getattr(tool, "card", None) or tool._card).name for tool in tools
    ]
    assert names == ["read_slack_conversation", "open_slack_file"]
