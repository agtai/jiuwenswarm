...
"""``agent.history`` -- one key, four words, and one axis it may be written on.
...
*nobody in S learns anything they could not already learn*. The key carries the
...
def test_channels_slack_history_is_registered_as_the_layer_zero_twin():
    # The same question in the same four words, one layer down, and where a
...
def test_anything_that_is_not_one_of_the_four_is_refused(written, warn, warnings):
    ...
    assert [
        ...
        if "is not one of disabled, origin, members, open" in line
    ]


def test_an_append_is_refused_because_there_is_nothing_to_append_to(warn, warnings):
    """Appending to one of four words produces a fifth that is not one of them."""
    scopes = _compile(
