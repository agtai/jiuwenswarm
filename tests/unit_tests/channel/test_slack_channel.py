...
class _FakeStreamResponse:
    def __init__(
        self, status_code: int, content_type: str, chunks: tuple[bytes, ...]
    ) -> None:
...
async def test_external_file_url_never_receives_the_bot_token(
    ...
    assert len(received) == 1
    assert received[0].params["content"] == "have a look at this"
...
async def test_off_slack_download_url_is_refused_whatever_it_looks_like(
    ...
    for url in (
        ...
        "https://files.slack.com@attacker.example/x",
        # http, which httpx would carry the header across on an upgrade.
...
async def test_httpx_drops_the_bearer_header_when_a_redirect_leaves_slack() -> None:
    """Pins the behaviour the per-hop reasoning in _assert_slack_file_url rests on.

    The connector follows redirects and checks only the first hop, which is
    sound exactly as long as httpx keeps stripping ``Authorization`` when a
    redirect leaves the origin. If a future httpx stops doing that, the initial
    check is no longer sufficient and this test is how that is found out.
    """
...
async def test_a_stalled_download_is_cut_off_and_reported(monkeypatch, tmp_path) -> None:
    ...
    # The message itself still reaches the agent, carrying no file records.
    assert len(received) == 1
    assert received[0].params["content"] == "summarize this"
    assert "files" not in received[0].params
    assert list((tmp_path / "slack_T1_D1_U1" / "uploads").glob("*")) == []
...
async def test_the_download_budget_is_shared_across_every_attachment(
    ...
    attachments it carries. Once the budget is gone the remaining files are
...
async def test_bare_mention_with_no_text_and_no_files_stays_unacknowledged(
    ...
    """A mention carrying nothing at all is still ignored outright.
...
async def test_retried_file_share_is_not_acknowledged_twice(
    ...
    that carries attachments is dropped before either the reaction or the
    download is reached.
    """
    _stub_downloads(monkeypatch, tmp_path)
    channel = SlackChannel(
...
def test_resolve_acknowledge_mode_warns_on_an_unknown_mode(slack_logs) -> None:
    ...
    so a config carrying both does not quietly resolve through the deprecated
...
async def test_send_raises_when_no_target_channel_can_be_resolved() -> None:
    ...
    # default_channel_id: there is nowhere to deliver, which used to be a
    # warning and a silent return.
    channel = SlackChannel(SlackChannelConfig(enabled=True), RobotMessageRouter())
...
class _RateLimitError(Exception):
    """Shaped like slack_sdk's SlackApiError: carries a .response."""
...
def test_shipped_config_offers_the_send_file_tool_on_slack() -> None:
    ...
    # A channel that does not declare the flag stays off, which is what Slack
    # was doing before this entry existed.
...
async def test_reply_mode_ignores_a_thread_root() -> None:
    # A root message carries thread_ts == ts once it has replies. It is not itself
