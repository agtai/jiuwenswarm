...
click rather than a message it carries a principal the connector can check. What
...
async def test_the_starter_may_stop_their_own_turn() -> None:
    ...
    await _card(channel, client)
    channel._remember_turn_initiator(SESSION, ASKER, REQUEST_ID, is_dm=False)
...
# ── F3: the allowance needs a recorded starter ───────────────────────────────


@pytest.mark.asyncio
async def test_with_no_recorded_initiator_the_allow_list_decides_alone() -> None:
    """F3. No entry, so no clicker can claim to be the starter.
...
async def test_an_unnamed_clicker_cannot_be_the_starter() -> None:
    ...
    carrying no user compare equal, and "whoever started it may stop it" becomes
    ...
    channel._turn_initiators[SESSION] = slack_connect._SlackTurnInitiator(
        user_id="", request_id=REQUEST_ID, is_dm=False
    )
...
async def test_a_stop_closes_the_open_stream_with_no_terminal_event(
    ...
    Before this, the click dispatched a cancel and returned, and the message
    stayed in streaming state until Slack's own expiry ran out on it. The stop
    is the last thing the connector hears about the turn, so the stop is what
    has to close it.
    """
...
async def test_the_partial_text_stays_and_is_marked_as_stopped(
    ...
    carries the same two words the card's title does, appended below what the
    ...
    # nowhere in what the close carries.
...
async def test_a_close_refused_at_the_stop_is_reported_and_not_raised(
    ...
    have carried is what the cancel destroyed. So a refusal costs the message
...
async def test_a_stop_releases_the_message_already_waiting_behind_it() -> None:
    ...
    person whose message is waiting. Each carries a reaction saying it will run.
    """
    channel, client, dispatched = _queueing_channel()
    await _running_turn(channel, client)
...
async def test_a_stop_drains_one_held_message_and_not_the_queue() -> None:
    ...
    # a conversation.
    assert _sends(dispatched)[0].params["query"].endswith("and one more thing")
...
async def test_a_stop_does_not_retire_a_newer_turns_initiator() -> None:
    ...
    channel._remember_turn_initiator(SESSION, BYSTANDER, "req-newer", is_dm=False)
...
async def test_a_resumed_turns_initiator_is_found_through_the_alias() -> None:
    ...
    resumed = "req-7f3a-resumed"
    channel._remember_activity_alias(resumed, REQUEST_ID)
    channel._remember_turn_initiator(SESSION, ASKER, resumed, is_dm=False)
