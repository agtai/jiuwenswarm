...
that gap: a ``plan`` carrying up to four sections, its thinking, its todos, its
...
``error`` when something fails or the turn does, it never carries a brief, a
...
class _BlockRefusingSlackClient(_RecordingSlackClient):
    """Refuses every message that carries blocks, and takes plain text.
...
def _flatten(field: dict[str, Any] | None) -> str:
    ...
    Only these assertions spell an emoji that way. The block itself carries an
...
async def test_a_tool_call_ends_the_stretch_and_the_total_carries_across() -> None:
    ...
    up beside it. The next chunk opens a fresh stretch, and the number carries
    over rather than restarting: a card whose timer went backwards would read as
    ...
    card = _section(client.updates[-1], "thinking")
    assert card["status"] == "in_progress"
    # Forty-two seconds carried over, plus whatever the new stretch has run for.
...
async def test_the_notification_string_says_how_long_the_turn_has_thought() -> None:
    ...
    await channel.send(_final("Yes."))
    assert client.updates[-1]["text"] == "Turn finished - thought for 1m 15s"
...
@pytest.mark.asyncio
async def test_a_single_type_fan_out_does_not_restate_itself_in_output() -> None:
    """One type's line is the aggregate, character for character."""
    client = _RecordingSlackClient()
    channel = _channel(client)
...
async def test_a_running_tool_is_named_in_output_with_how_long_it_has_been_out() -> None:
    """No payload carries a duration, so the call is clocked when it arrives."""
    client = _RecordingSlackClient()
    channel = _channel(client)
...
async def test_a_tool_the_runtime_classified_as_failed_is_shown_as_failed() -> None:
    """The parent's own tools carry the rail's classification, so it is used."""
    client = _RecordingSlackClient()
    channel = _channel(client)
...
async def test_a_turn_that_died_with_no_card_up_posts_one_standalone() -> None:
    ...
    failure cannot ride along with the reply either -- a message carries a plan
...
async def test_a_failed_turn_never_leaves_the_channel_empty() -> None:
    ...
    for label, client, config in (
        ...
        # The card is up but will not take the edit that would carry the error.
...
async def test_the_error_section_writes_no_string_of_its_own_in_non_ascii() -> None:
    ...
    itself is not one of those, and is carried in the notification line as well
...
async def test_a_dispatch_joining_a_running_fan_out_waits_out_the_floor() -> None:
    ...
    # The completion that follows carries the whole fan-out anyway.
    await channel.send(_result("call-1"))
    assert client.updates[-1]["text"] == "Working... - 2 of 3 subagents running"
