...
the permissions snapshot carries everything a level can say into the engine.
...
def test_the_silent_skills_rebuild_session_stays_above_scopes(deployment):
    ...
    try:
        ...
        assert sites.snapshot()["mode"] == "full_access"
    finally:
...
def test_every_entry_point_binds_the_chat_beside_the_channel():
    ...
    The chat id is bound and released in the same three places the channel id
    is. An entry point that set one and not the other would leave scopes
    matching on ``{channel}`` alone there -- a scope written for one
    conversation quietly applying to every conversation on that platform, which
...
def test_a_role_bearing_permissions_rule_is_refused_rather_than_inverted(deployment):
    """§8.4's gate, closed at the config instead of biting at the tool call.

    The permission rail composes with no sender, so a ``not: {role: admin}``
    exemption used to reach nobody and the restriction landed on exactly the
    ...
    sites = deployment(
        ...
        people={"harenome": {"slack": "U0BLHQQBCCD"}},
...
def test_editing_only_the_roles_block_recompiles(deployment, monkeypatch):
    ...
    The rule carries ``delivery`` rather than ``permissions`` because
    ``permissions`` is barred from the identity axes until the runtime carries a
...
def test_a_compile_that_raises_leaves_the_permissions_block_as_written(
    ...
    ``ToolPermissionHost``. Before this section existed the snapshot was a dict
    lookup and the hook read ContextVars, so neither could fail; an exception
    from a scope now stops the tool call rather than the rule it was written
