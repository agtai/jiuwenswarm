...
carried no explicit ``mode`` (channel connectors never send one), inherited the
...
* ``_prepare_code_mode_chat_turn`` refuses to inherit such a value from disk
  (lets sessions already carrying it recover).
"""
...
class _RecordingAgentManager:
    ...
    async def get_agent(self, *, channel_id, mode, project_dir, sub_mode):
