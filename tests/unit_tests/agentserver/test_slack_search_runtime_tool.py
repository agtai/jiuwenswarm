...
def _card_name(toolkit: Any, func: Callable[..., Any]) -> str:
    ...
    the card that carries it means a rename either arrives here or fails in the
    one place that decides it.

    A toolkit may mount more than one tool -- the history toolkit mounts
    ``open_slack_file`` alongside the conversation reader -- so the one wanted
...
class _FakeToolkit:
    ...
    def __init__(
        ...
        *,
        ...
        # Only the history toolkit is built with one -- open_slack_file needs a
...
async def test_the_toolkit_is_built_once_and_reads_the_live_request() -> None:
    ...
    # Read the provider under a request that carries a different token, and
    # again under one that carries none -- the second is a cron run reaching a
