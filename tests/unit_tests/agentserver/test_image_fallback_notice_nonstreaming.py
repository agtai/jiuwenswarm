...
async def test_notice_stands_alone_when_the_run_produces_no_text(quiet_runtime):
    ...
    payload = response.payload
    # ``_vision_model_config`` is None on this adapter, so the notice is the
    # one that says no image-understanding tool is configured either. Which of
    # the two copies it is does not matter here; that the notice is the whole
    # payload when the run yields no text does.
    assert payload["content"].strip().endswith("且未配置可用的视觉模型工具。")
    assert set(payload) == {"content"}
