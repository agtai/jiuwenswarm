...
@pytest.mark.asyncio
@pytest.mark.parametrize("stream", [False, True])
@pytest.mark.parametrize("level,expected", [("high", {"effort": "high"}), ("off", {"effort": "none"}), ("on", None)])
async def test_develop_model_settings_and_internal_context_stay_compatible(monkeypatch, stream, level, expected):
    sent = []
    ...
    config = {"context_window": 131072, "reasoning_level": level, "temperature": 0.7}
    ...
    async with AsyncOpenAI(api_key="test-key", http_client=httpx.AsyncClient(transport=httpx.MockTransport(respond))) as client:
        monkeypatch.setattr(OpenAIModelClient, "_create_async_openai_client", lambda self, timeout=None: client)
        model = build_model_from_entry(entry(), config)
        options = {"request_purpose": "context_compression", "context_operation_id": "fixture-operation"}
        if stream:
            chunks = [chunk.content or "" async for chunk in model.stream("Fixture", **options)]
            assert "".join(chunks) == "Unchanged answer."
        else:
            assert (await model.invoke("Fixture", **options)).content == "Unchanged answer."
        assert len(sent) == 1
        assert not {"context_window", "request_purpose", "context_operation_id"}.intersection(sent[0])
        assert sent[0].get("reasoning") == expected
        if level == "off":
            assert sent[0]["temperature"] == 0.7
        else:
            assert "temperature" not in sent[0]
        assert model.model_config.context_window == 131072
        assert config == original


@pytest.mark.parametrize("auth_mode", ["openai_account_oauth", "custom_headers", "none"])
def test_non_api_key_authentication_keeps_existing_client_config(auth_mode, monkeypatch):
    from openjiuwen.core.foundation.llm import ModelClientConfig
    from jiuwenswarm.common import openai_responses_client as adapter

    def unexpected_dependency_probe(name):
        pytest.fail("Non-API-key configuration must not require the Responses repair SDK")

    monkeypatch.setattr(adapter, "version", unexpected_dependency_probe)
    config = ModelClientConfig(**{k: v for k, v in entry().items() if k != "model_name"}, auth_mode=auth_mode)
    assert adapter.openai_responses_client_config(config, model_name="gpt-5.6") is config


def test_neutral_reasoning_keeps_explicit_and_legacy_override_precedence():
    from openjiuwen.core.foundation.llm.schema.config import ReasoningConfig

    config = {"reasoning_level": "high", "extra_body": {"reasoning_effort": "none"}}
    model = build_model_from_entry(entry(), config)
    assert request_params(model)["reasoning"] == {"effort": "none"}
    assert config["reasoning_level"] == "high"
    model = build_model_from_entry(entry(), {"reasoning_level": "off"})
    assert request_params(model, reasoning=ReasoningConfig(mode="enabled", effort="low"))["reasoning"] == {"effort": "low"}
    assert request_params(model, reasoning=ReasoningConfig(mode="disabled"))["reasoning"] == {"effort": "none"}
    raw = {"effort": "high", "summary": "auto"}
    assert request_params(model, reasoning=raw)["reasoning"] == raw
