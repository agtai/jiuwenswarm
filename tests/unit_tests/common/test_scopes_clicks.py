...
someone who may not have started it, and it carries its own principal (D13). So
it gets a section of its own rather than a key in ``delivery``, and the section
...
* **F1** -- no ``clicks`` rule matches, so layer 0 decides. Absence of a rule is
  spelled by there being no rule, never by an empty one, and nothing in this
  section may change what a config with no ``clicks:`` block does.
* **F2** -- a rule matches and the clicker cannot be identified, so the click is
...
* **F3** -- lives on the connector, not here: the starter's allowance needs a
  recorded starter.
...
def clicky():
    ...
    register_channel(
        ChannelCapabilities(
            # Renders buttons, names nobody. Not a hypothetical: section 8.4
            # lists connectors that leave the sender in metadata, and the
            # pseudo-channels ship ``identity_keys=()``.
            channel="anonymous",
    ...
    register_channel(
        # Renders none. The other half of section 8.5's distinction.
        ChannelCapabilities(
            channel="silent",
...
def test_user_and_role_are_one_axis_and_they_or(warn):
    # D5, the same union the match axis performs. AND would mean "alice, but
...
# --------------------------------------------------------------------------
# Composition (D2): the map merges per gesture, a clause replaces
# --------------------------------------------------------------------------
...
def test_a_later_clause_replaces_rather_than_widening_the_one_above(warn):
    ...
    # people, which is the direction D4 forbids of a section that exists to
    # tighten.
    scopes = compile_scopes(
        [
            {"match": {"channel": "clicky"}, "clicks": {"approve": {"role": "member"}}},
...
# --------------------------------------------------------------------------
# F1 -- no rule, no change
# --------------------------------------------------------------------------


def test_no_clicks_rule_settles_to_none_rather_than_to_nobody(warn):
    """``None`` is F1 and an empty rule is F2; a caller must not confuse them."""
    scopes = _compile(
...
# --------------------------------------------------------------------------
# F2 -- a rule matches and the clicker cannot be identified
# --------------------------------------------------------------------------
...
def test_a_role_nobody_here_is_in_admits_nobody_here(warn, warnings):
    """F2's second case: section 8.3's rule read at click time.
    ...
    nobody restricts nobody" is the permissive direction F2 settles against.
    """
    scopes = _compile(
...
def test_an_undeclared_role_drops_the_clause_and_says_the_click_is_ungated(
    ...
    """A typo is not F2, and the warning must not read as though it were.
