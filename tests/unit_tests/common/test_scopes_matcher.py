...
def test_specificity_decides_the_layer(demo_channel, warn):
    ...
    assert [scope.layer for scope in scopes] == [1, 2]
...
def test_a_more_specific_scope_wins_even_when_written_first(demo_channel, warn):
    ...
    assert [scope.layer for scope in ordered] == [1, 2]
...
def test_user_and_role_are_one_axis_so_a_not_does_not_add_a_layer(
    ...
    # D5 and D6: user, role and not are one axis between them. Counting the
    # keys would put the second scope at layer 4 and let adding an exemption
    # to a rule silently promote it above a rule that had been winning.
    assert [scope.layer for scope in scopes] == [3, 3]
...
def test_a_not_on_its_own_still_constrains_the_identity_axis(
    ...
    # further layer -- which is what makes section 4.3's read-only-except-admins
    # rule work without an override: directive.
    assert [scope.layer for scope in scopes] == [2, 3]
...
def test_ids_in_an_identity_list_are_settled_at_load(demo_channel, warn, warnings):
    ...
    # Stripped, deduplicated and sorted. A list is a set: its order carries no
    # meaning, so settling one keeps two spellings of the same rule equal.
    assert scopes[0].match.users == ("U1", "U2")
    assert warnings == []


def test_a_user_bearing_scope_and_a_chat_bearing_scope_tie_and_position_decides(
    demo_channel, warn, warnings
):
    """The case the layer numbering has to be pinned on.

    ``{channel, user}`` -- this person anywhere -- and ``{channel, chat}`` --
    everyone in this conversation -- are both layer 2 and neither contains the
    other. Section 6 orders the nested cases and says nothing about this one, so
    they tie and the later line wins, which is the tie-break every other equal
    pair here already uses. Written both ways round: the assertion is that
    position decides, not that one axis is quietly stronger than the other.
    """
    user_first = _compile(
        [
    ...
    assert [scope.layer for scope in user_first] == [2, 2]
    assert [scope.layer for scope in chat_first] == [2, 2]

    def _last(scopes):
        return matching_scopes(scopes, channel="demo", chat="C1", user="U1")[-1]

    assert _last(user_first).section("delivery") == {"prompt": "c"}
    assert _last(chat_first).section("delivery") == {"prompt": "u"}
...
def test_naming_both_a_conversation_and_a_sender_outranks_naming_either(
    ...
    # Written first and still last in the fold: section 6's layer 3 beats both
    # of the layer-2 readings whichever order they appear in.
    ordered = matching_scopes(scopes, channel="demo", chat="C1", user="U1")
    assert [scope.layer for scope in ordered] == [2, 2, 3]
...
def test_the_agent_section_is_stored_rather_than_warned_about(
    ...
    # nothing. A rule carrying agent alone is now a whole rule.
    assert len(scopes) == 1
    assert scopes[0].section("agent") == {"model_name": "m"}
    assert warnings == []
