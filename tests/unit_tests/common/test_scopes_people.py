...
def test_a_platform_may_carry_several_ids_for_one_person(warn, warnings):
    ...
    # whichever the event carried. Listing both is the only shape that can be
...
def test_a_role_carrying_permissions_is_refused_with_that_reason(warn, warnings):
    ...
    # D8. This is the shape every other RBAC system uses, so an author reaching
...
def test_a_role_is_resolved_into_the_ids_user_would_have_enumerated(
    ...
    # The whole of D5, stated as an equality: a role is sugar over user, so the
    # two spellings must compile to the same rule and not merely behave alike.
    assert by_role[0].match.users == by_id[0].match.users
    assert by_role[0].layer == by_id[0].layer
...
def test_a_role_minus_one_person_is_expressible(demo_channel, warn, warnings):
    ...
    # The case section 4.2 says justifies allowing a positive and a negative
    # together: without it, "the team minus one" means enumerating the team,
...
def test_a_role_does_not_outrank_an_id_on_the_same_axis(demo_channel, warn, warnings):
    ...
    # One axis, one count (D5). Counting the keys would put the third scope at
    # layer 4 and let rewriting a list of ids as a role silently reorder which
    # rule wins per key -- a change with no crash and no log line.
    assert [scope.layer for scope in scopes] == [3, 3, 3]

    ordered = matching_scopes(scopes, channel="demo", chat="C1", user="U_HAR")
    assert [scope.index for scope in ordered] == [0, 1, 2]
    assert ordered[-1].section("delivery") == {"prompt": "by role, minus one"}


def test_the_tie_between_a_role_scope_and_a_chat_scope_is_broken_by_position(
    demo_channel, warn, warnings
):
    role_first = _compile(
        [
    ...
    # The same tie {channel, user} and {channel, chat} already have, and it must
    # stay a tie: introducing a role must not quietly weight identity.
    assert [scope.layer for scope in role_first] == [2, 2]
    ...
    assert _last(chat_first).section("delivery") == {"prompt": "r"}
...
def test_a_role_that_resolves_to_nobody_here_still_constrains_identity(
    ...
    # asking. Reading it as unconstrained would promote it from layer 3 to layer
    # 2 for having matched nobody, and it would then start beating rules it had
    # been sitting under.
    assert scopes[0].match.users == ()
    assert scopes[0].layer == 3
    assert not scopes[0].selects(channel="demo", chat="C1", user="U_HAR")
...
def test_no_axis_is_deferred_any_more():
    ...
    assert SUPPORTED_AXES == ("channel", "chat", "user", "role")
