...
is that case for a whole section: a click carries its own principal, which is
...
runtime carries an identity; until then the tests below pin it, because the
...
def restricted():
    """A synthetic channel carrying one restricted key and one unrestricted one.
...
def test_the_refusal_names_the_rule_the_key_and_the_permitted_axes(warn, warnings):
    ...
    # And the reason the declaration gave, carried rather than composed here.
...
def test_permissions_is_refused_on_an_identity_axis(match, axis, warn, warnings):
    ...
    assert "is given no sender" in line
...
def test_the_clicks_bar_is_a_row_in_the_shared_table(warn, warnings):
    ...
    The section was barred from a rule naming a sender before this table
    existed. It is the same bar, read from the same place as every other, which
    is what stops the two drifting into disagreeing about one section.
    """
    ...
    # The sentence the old branch gave is carried in the row's own reason, so
    # the wording an operator has seen before does not change under them.
...
def test_no_key_that_already_existed_has_gained_a_restriction(restricted):
    """The reason a deployment written before this compiles to what it did.

    Stated as the set of keys that were writable on every axis before the table
    existed rather than as "no channel declares anything", so that it goes on
