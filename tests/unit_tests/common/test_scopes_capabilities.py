...
def test_cancel_is_the_first_value_because_it_is_the_default(mid_turn_channel):
    # Not a naming preference. An operator who writes nothing must see exactly
    # what this did before the key existed, and that is cancel.
    assert MID_TURN_VALUES[0] == MID_TURN_CANCEL
...
def test_an_unrecognised_value_is_refused_and_leaves_the_layer_below(
    ...
    assert any("is not one of cancel, steer, queue" in line for line in warnings), warnings
...
def test_a_chat_without_a_channel_is_dropped_rather_than_left_unvalidated(warn, warnings):
    ...
    trigger name is not a trigger name, and before this was refused it survived
    compilation in silence.
    """
    scopes = compile_scopes(
...
def test_declaring_the_user_axis_without_identity_keys_is_reported(registry, caplog):
    """Cross-checked as section 8.1 asks, and reported to the connector author.
